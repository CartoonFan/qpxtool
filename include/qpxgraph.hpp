...
class QPxGraph : public QWidget {
    Q_OBJECT
public:
    QPxGraph(QPxSettings *iset, devlist *idev, QString iname, int ttype,
             QWidget *p = 0, Qt::WindowFlags fl = 0);
    ~QPxGraph();
    //	void	setDataNames(QStringList dn);
    //	void	setZeroPos(float);
    //	float	zeroPos();
    void drawGraph(QPainter *, QSize, device *, int ttype, const QRect &rect,
                   int eflags = 0, bool forceRepaint = 0);
...
protected:
    ...
    void drawTransfer(QPainter *, const QSize &, device *, const QRect &rect,
                      bool rw);
