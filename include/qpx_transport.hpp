...
//
// modified to use with QPxTool http://qpxtool.sf.net (C) 2005-2009,2012,
...
#define ERRCODE(s)                                                             \
  ((s)[0] == 0x70 || (s)[0] == 0x71                                            \
       ? ERRCODE_FIXED(s)                                                      \
       : ((s)[0] == 0x72 || (s)[0] == 0x73 ? ERRCODE_DESCR(s) : 0))
...
typedef enum { NONE = 0, READ = SCCMD_READ, WRITE = SCCMD_WRITE } Direction;
