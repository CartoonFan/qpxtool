...
template <typename T> struct Errc_ARRAY {
  quint64 lba;
  float spdx;
  T err[8];
};

template <typename T> struct Errc_CD {
  quint64 lba;
  float spdx;
  T bler;
  T e11, e21, e31;
...
template <typename T> struct Errc_DVD {
  quint64 lba;
  float spdx;
  T res;
  T pie, pi8, pif;
...
template <typename T> struct Errc_BD {
  quint64 lba;
  float spdx;
  T res0;
  T ldc, res1, res2;
...
template <typename T> union Errc {
  Errc_ARRAY<T> raw;
  Errc_CD<T> cd;
  Errc_DVD<T> dvd;
  Errc_BD<T> bd;
...
public:
  ...
  inline void clearErrc() {
    ...
    errcTOT.raw.spdx = 0.0;
    for (int i = 0; i < 8; i++)
      errcTOT.raw.err[i] = 0;
    errcMAX.raw.spdx = 0.0;
    for (int i = 0; i < 8; i++)
      errcMAX.raw.err[i] = 0;
    errcAVG.raw.spdx = 0.0;
    for (int i = 0; i < 8; i++)
      errcAVG.raw.err[i] = 0.0;
  };
  inline void clearJB() {
    jb.clear();
    jb_time = 0;
...
public:
  ...
  int32_t life_dn;
  QString life_cr, life_cw, life_dr, life_dw;
