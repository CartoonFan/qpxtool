...
static const drivedesclist drivelist =
    ...
    {{"PLEXTOR ", DEV_PLEXTOR, "CD-R   PREMIUM ", PLEXTOR_PREMIUM,
     ...
     {"PLEXTOR ", DEV_PLEXTOR, "DVDR   PX-708A2", PLEXTOR_708A2,
      CHK_ERRC_CD | CHK_JB_CD | CHK_ERRC_DVD | CHK_JB_DVD | CHK_TA_DVD |
          CHK_FETE},
...
// CHK_CD_ERRC | CHK_CD_JB | CHK_FETE_CD | CHK_DVD_ERRC | CHK_DVD_JB |
// CHK_FETE_DVD | CHK_TA_DVD }, CHK_CD_ERRC | CHK_CD_JB | CHK_DVD_ERRC |
// CHK_DVD_JB | CHK_TA_DVD | CHK_FETE },

static const char plugin_name[] = "PLEXTOR";
static const char plugin_desc[] =
    "Scan plugin for PLEXTOR devices (based on SANYO chips)";
...
private:
  ...
  */
  ...
  int build_TA_histogram_px716(unsigned char *response_data, int *dest_pit,
                               int *dest_land, int len);
  int build_TA_histogram_px755(unsigned char *response_data, int *dest_pit,
                               int *dest_land, int len, int dt);
