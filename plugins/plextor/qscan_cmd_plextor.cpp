...
//	dev->cmd[8] = 0x0F;
    ...
    {
        if (!dev->silent) sperror ("PLEXTOR_START_CX", dev->err);
        return dev->err;
    }
#ifdef _debug_cx
...
//	dev->cmd[9] = 0x13;
    ...
    if ((dev->err=dev->cmd.transport() ))
    {
        if(!dev->silent) sperror ("PLEXTOR_START_PIPO",dev->err);
        return dev->err;
    }
    printf("scan init OK!\n");
    return 0;
...
int scan_plextor::cmd_cd_errc_getdata(cd_errc *data)
{
    ...
    printf("| ");
    for (i=0x06; i<0x0A; i++) printf("%02X ", dev->rd_buf[i] & 0xFF);
    printf("| ");
    for (i=0x0A; i<0x1A; i+=2) {
        if (ntoh16(dev->rd_buf+i)) printf("%5d ", ntoh16(dev->rd_buf+i));
        else printf("_____ ");
    }
    printf("|\n");
#endif
