...
int scan_nec::cmd_cd_errc_init() {
  ...
  if ((dev->err = dev->cmd.transport(NONE, NULL, 0))) {
    if (!dev->silent)
      sperror("nec_init_errc_scan", dev->err);
    return 1;
  }
  ...
  if ((dev->err = dev->cmd.transport(NONE, NULL, 0))) {
    if (!dev->silent)
      sperror("nec_set_scan_interval", dev->err);
    return 1;
  }
  return 0;
}
...
int scan_nec::cmd_cd_errc_block(cd_errc *data) {
  ...
  data->uncr = 0;
  lba += 75;
  tlba = ((int)dev->rd_buf[1] * 4500 + (int)dev->rd_buf[2] * 75 +
          (int)dev->rd_buf[3]);
  if (lba < tlba)
    lba = tlba;
  return 0;
