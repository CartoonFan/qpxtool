...
#include "qscan_plugin_benq_dvdrom.hpp"
...
int scan_benqrom::cmd_scan_init() {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 4))) {
        if (!dev->silent)
            sperror("benq_rom_init_scan", dev->err);
        return dev->err;
    }
    cnt = 0;
    return 0;
...
int scan_benqrom::cmd_cd_errc_block(cd_errc *data) {
    ...
    if (!(cnt % cnt_max_cd)) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            //			_lba=((int)dev->rd_buf[0] * 4500 +
            //(int)dev->rd_buf[3] * 75 + (int)dev->rd_buf[2]);
        }
        cnt = 0;
    }
    data->bler = _c1[cnt];
...
scan_commands commands_list_benq_rom = {
        NULL,	benq_rom_init_scan,	benq_rom_cx_do_one_interval,
benq_rom_cx_end_scan, NULL,	benq_rom_init_scan,
benq_rom_jitter_CD_do_one_interval,	benq_rom_cx_end_scan,

        NULL,	NULL,			NULL,
NULL, NULL,	NULL,			NULL,
NULL, NULL,	NULL,			NULL,
NULL, NULL,	benq_rom_init_scan,	benq_rom_pie_pif_do_one_interval,
benq_rom_pi_end_scan,
        NULL,	benq_rom_init_scan,	benq_rom_jitter_DVD_do_16_ecc,
benq_rom_pi_end_scan,
};
