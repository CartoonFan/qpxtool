...
void qpxwriter::setSimul(bool isimul) {
    simul = isimul;
};
int qpxwriter::send_opc() {
    return 0;
}
int qpxwriter::open_session() {
    return 0;
}
int qpxwriter::open_track(uint32_t size) {
    return 0;
}
int qpxwriter::close_track() {
    return 0;
}
int qpxwriter::fixate() {
    return 0;
}
...
int qpxwriter_cd::set_write_parameters_def(bool bfree, bool simul) {
    ...
    */
    ...
    dev->rd_buf[8 + 2] = ((bfree ? 0x40 : 0) |
                          (simul ? 0x10 : 0)); // setting BURN-Free and TestWrite
...
int qpxwriter_cd::send_cue_sheet(uint32_t tsize) {
    ...
    */
    ...
    if ((dev->err = dev->cmd.transport(WRITE, dev->rd_buf, foffs))) {
        sperror("SEND_CUE_SHEET", dev->err);
        return (dev->err);
    }
    return 0;
}
