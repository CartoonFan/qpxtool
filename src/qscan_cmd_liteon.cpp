 ...
 *
 *
 * Thanks to Artur Kalimullin (Kaliy) <kalimullin@gmail.com> for assistance in
 * LiteOn Cx scan implementation
 *
 *
...
#include "qscan_plugin_liteon.hpp"
...
int scan_liteon::cmd_cd_errc_init_old() {
  ...
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_init_A_cx_scan", dev->err);
    return 1;
  }
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_init_B_cx_scan", dev->err);
    return 1;
  }
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_init_C_cx_scan", dev->err);
    return 1;
  }
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_init_D_cx_scan", dev->err);
    return 1;
  }
...
int scan_liteon::cmd_dvd_errc_init() {
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 0x10))) {
    sperror("LiteOn_errc_dvd_probe", dev->err);
    return 1;
  }
  return 0;
}
...
int scan_liteon::cmd_fete_get_data(cdvd_ft *data) {
  ...
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 65536))) {
    sperror("LiteOn_FETE get data", dev->err);
    return 1;
  }
...
int scan_liteon::cmd_fete_get_position() {
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 65536))) {
    sperror("LiteOn_FETE get LBA", dev->err);
    return 1;
  }
  return 0;
}
...
int scan_liteon::cmd_cd_fete_init(int &speed) {
  ...
  if ((dev->err = dev->cmd.transport(WRITE, dev->rd_buf, 16))) {
    sperror("LiteOn_FETE_init scan", dev->err);
    return 1;
  }
  return 0;
}
...
int scan_liteon::cmd_cd_errc_read() {
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_cx_do_one_interval_scan", dev->err);
    return 1;
  }
...
int scan_liteon::cmd_cd_errc_getdata(cd_errc *data) {
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_cx_do_one_interval_readout", dev->err);
    return 1;
  }
...
int scan_liteon::cmd_cd_errc_inteval_rst() {
  ...
  dev->cmd[11] = 0x00;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_cx_do_one_interval_rst", dev->err);
    return 1;
  }
  return 0;
}
...
int scan_liteon::cmd_cd_errc_block_old(cd_errc *data) {
  if (cmd_cd_errc_read())
    return dev->err;
  if (cmd_cd_errc_getdata(data))
    return dev->err;
  lba += 75;
  if (cmd_cd_errc_inteval_rst())
    return dev->err;
...
int scan_liteon::cmd_cd_errc_block(cd_errc *data) {
  return cd_errc_new ? cmd_cd_errc_block_new(data)
                     : cmd_cd_errc_block_old(data);
}
...
int scan_liteon::cmd_cd_fete_block(cdvd_ft *data) {
  // fe/te data redout
  if (cmd_fete_get_data(data))
    return 1;

  // current lba redout
  if (cmd_fete_get_position())
    return 1;
  int tlba =
      (((dev->rd_buf[0] >> 4) & 0x0F) * 10 + (dev->rd_buf[0] & 0x0F)) * 60 *
          75 +
...
int scan_liteon::cmd_cd_errc_end() {
  ...
  dev->cmd[2] = 0x01;
  if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 256))) {
    sperror("LiteOn_end_scan", dev->err);
    return 1;
  }
  return 0;
}
