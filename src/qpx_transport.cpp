...
//
// modified to use with QPxTool http://qpxtool.sf.net (C) 2005-2009, Gennady
// "ShultZ" Kozlov
//
...
const unsigned char scsi_command_size[8] = {6, 10, 10, 12, 16, 12, 10, 10};
...
#define KERNEL_BROKEN 0
    if (use_sg_io) {
        ...
        /* cmd length fix */
        //		printf("opcode: %02X, cmd.len:
        //%d\n",sg_io.cmdp[0],sg_io.cmd_len);
        sg_io.cmd_len = (sg_io.cmd_len < COMMAND_SIZE(sg_io.cmdp[0]))
                        ? COMMAND_SIZE(sg_io.cmdp[0])
                        : sg_io.cmd_len;
...
#ifdef SG_IO
size_t Scsi_Command::residue() {
    return use_sg_io ? sg_io.resid : 0;
}
#else
size_t Scsi_Command::residue() {
    return 0;
    ...
    while ((mb = getmntent(fp)) != NULL) {
        ...
        if (msb.st_rdev == fsb.st_rdev) {
            ...
            if ((pid = fork()) == (pid_t)-1)
                break;
            if (pid == 0)
                execl("/bin/umount", "umount", mb->mnt_dir, NULL);
            while (1) {
...
size_t Scsi_Command::residue() {
    return req.datalen - req.datalen_used;
}
int Scsi_Command::transport(Direction dir, void *buf, size_t sz) {
    int ret = 0;
    /* cmd length fix */
    //	printf("CMD: (%2d)  %02x\n", req.cmdlen, req.cmd[0]);
    req.cmdlen = (req.cmdlen < COMMAND_SIZE(req.cmd[0]))
                 ? COMMAND_SIZE(req.cmd[0])
                 : req.cmdlen;
...
int Scsi_Command::umount(int f) {
    ...
    for (i = 0; i < mntsize; i++) {
        ...
        if ((slash = strrchr(mntbuf[i].f_mntfromname, '/')) == NULL)
            continue;
...
Scsi_Command::Scsi_Command(int f) {
    ...
    if (ioctl(f, CAMGETPASSTHRU, &ccb) < 0)
        return;
...
Scsi_Command::Scsi_Command(void *f) {
    cam = (struct cam_device *)f, autoclose = 0;
    fd = -1;
    filename = NULL;
}
...
void Scsi_Command::timeout(int i) {
    ccb.ccb_h.timeout = i * 1000;
}
size_t Scsi_Command::residue() {
    return ccb.csio.resid;
}
int Scsi_Command::transport(Direction dir, void *buf, size_t sz) {
    int ret = 0;
...
int Scsi_Command::umount(int f) {
    ...
    for (i = 0; i < mntsize; i++) {
        if (stat(mntbuf[i].f_mntfromname, &msb) < 0)
            continue;
        if (msb.st_rdev == fsb.st_rdev) {
            ret = unmount(mntbuf[i].f_mntonname, 0);
            break;
...
int Scsi_Command::transport(Direction dir, void *buf, size_t sz) {
    ...
    p.spt.CdbLength = (p.spt.CdbLength < COMMAND_SIZE(p.spt.Cdb[0]))
                      ? COMMAND_SIZE(p.spt.Cdb[0])
                      : p.spt.CdbLength;
    ...
    if (DeviceIoControl(fd, IOCTL_SCSI_PASS_THROUGH_DIRECT, &p, sizeof(p.spt), &p,
                        sizeof(p), &bytes, FALSE) == 0)
        return -1;
...
int Scsi_Command::umount(int f) {
    ...
    if (DeviceIoControl(h, FSCTL_LOCK_VOLUME, NULL, 0, NULL, 0, &junk, NULL) &&
            DeviceIoControl(h, FSCTL_DISMOUNT_VOLUME, NULL, 0, NULL, 0, &junk,
                            NULL)) {
        if (h == fd)
            autoclose++;
        return 0;
...
static int iokit_err(IOReturn ioret, SCSITaskStatus stat,
    ...
    if (ioret == kIOReturnSuccess)
        ret = 0;
    else if (ioret == kIOReturnNoDevice)
        errno = ENXIO;
    else if (ioret == kIOReturnNoMemory)
        errno = ENOMEM;
    else if (ioret == kIOReturnExclusiveAccess)
        errno = EBUSY;
    else
        errno = EIO;
...
Scsi_Command::Scsi_Command(void *f) {
    taskif = (SCSITaskDeviceInterface **)f, autoclose = 0;
    filename = NULL;
}
...
Scsi_Command::~Scsi_Command() {
    if (autoclose) {
        if (taskif)
            (*taskif)->ReleaseExclusiveAccess(taskif), (*taskif)->Release(taskif),
            taskif = NULL;
        if (mmcdif)
...
int Scsi_Command::associate(const char *file, const struct stat *ref) {
    ...
    if ((match = CFDictionaryCreateMutable(
                     kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks,
                     &kCFTypeDictionaryValueCallBacks)) == NULL)
        return !(errno = ENOMEM);
    if ((bsddev = CFDictionaryCreateMutable(
                      kCFAllocatorDefault, 0, &kCFTypeDictionaryKeyCallBacks,
                      &kCFTypeDictionaryValueCallBacks)) == NULL)
    ...
    while ((kret = IORegistryEntryGetParentEntry(scsiob, kIOServicePlane,
        ...
        scsiob = parent;
        uclient = (CFStringRef)IORegistryEntryCreateCFProperty(
                      scsiob, CFSTR(kIOPropertySCSITaskDeviceCategory), kCFAllocatorDefault,
                      0);
        if (uclient) {
    ...
    SInt32 score = 0;
    if (IOCreatePlugInInterfaceForService(scsiob, kIOMMCDeviceUserClientTypeID,
...
int Scsi_Command::umount(int f) {
    ...
    if ((n = getfsstat(NULL, 0, MNT_NOWAIT)) < 0)
        return -1;
    n += 4, n *= sizeof(struct statfs);

    struct statfs *p = (struct statfs *)alloca(n);
    if ((n = getfsstat(p, n, MNT_NOWAIT)) < 0)
        return -1;

    for (i = 0; i < n; i++, p++) {
        if (stat(p->f_mntfromname, &sb) == 0 && S_ISBLK(sb.st_mode) &&
                sb.st_rdev == ref)
...
#else
        {
            ...
            if (pid == 0) // if diskutil will be proven broken,
                ...
                // attack vector...
                setuid(getuid()),
                       execl("/usr/sbin/diskutil", "diskutil", "unmount", p->f_mntonname,
                             (void *)NULL),
                       exit(errno);
            while (1) {
                rpid = waitpid(pid, &rval, 0);
