...
//  detect_check_capabilities(dev);
    ...
    if ( isPlextor(dev) == 0 && isYamaha(dev) == 0 && isPioneer(dev) == 0) {
        std::cout << dev->device << ": drive not supported, only common controls will work!\n";
//      return 1;
    }
    if ( isPlextor(dev) != 0 ) {
        ...
        if (dev->life.ok) {
            ...
            std::cout << "  CD Rd  : " << dev->life.cr.h << ":" << dev->life.cr.m << ":" << dev->life.cr.s << "\n";
            std::cout << "  CD Wr  : " << dev->life.cw.h << ":" << dev->life.cw.m << ":" << dev->life.cw.s << "\n";
            if ((dev->rd_capabilities & DEVICE_DVD) != 0U) {
                std::cout << "  DVD Rd : " << dev->life.dr.h << ":" << dev->life.dr.m << ":" << dev->life.dr.s << "\n";
            }
            if ((dev->wr_capabilities & DEVICE_DVD) != 0U) {
                std::cout << "  DVD Wr : " << dev->life.dw.h << ":" << dev->life.dw.m << ":" << dev->life.dw.s << "\n";
            }
        }
...
//  for (int i=0; i<256; i++) {if (!plextor_get_mode(dev,i)) std::cout << " MODE 0x" << i << "\n";}


//  std::cout << "Trying opcode ED modes...\n";
//  for (int i=0; i<256; i++) {if (!plextor_get_mode2(dev,i)) std::cout << " MODE 0x" << i << "\n";}


    if ((flags & FL_SUPPORTED) != 0U) {
//      std::cout << "____________________________\n";
        std::cout << "\n** Supported features:\n";
        std::cout << "AudioMaster Q.R.    : " << (((dev->ven_features & YMH_AMQR) != 0U) ? "YES" : "-") << "\n";
        std::cout << "Yamaha ForceSpeed   : " << (((dev->ven_features & YMH_FORCESPEED) != 0U) ? "YES" : "-") << "\n";
        std::cout << "Yamaha DiscT@2      : " << (((dev->ven_features & YMH_TATTOO) != 0U) ? "YES" : "-") << "\n";
        std::cout << "Hide CD-R           : " << (((dev->ven_features & PX_HCDRSS) != 0U) ? "YES" : "-") << "\n";
        std::cout << "SingleSession       : " << (((dev->ven_features & PX_HCDRSS) != 0U) ? "YES" : "-") << "\n";
        std::cout << "SpeedRead           : " << (((dev->ven_features & PX_SPDREAD) != 0U) ? "YES" : "-") << "\n";
        std::cout << "PoweRec             : " << (((dev->ven_features & PX_POWEREC) != 0U) ? "YES" : "-") << "\n";
        std::cout << "GigaRec             : " << (((dev->ven_features & PX_GIGAREC) != 0U) ? "YES" : "-") << "\n";
        std::cout << "VariRec CD          : " << (((dev->ven_features & PX_VARIREC_CD) != 0U) ? "YES" : "-") << "\n";
        std::cout << "VariRec DVD         : " << (((dev->ven_features & PX_VARIREC_DVD) != 0U) ? "YES" : "-") << "\n";
        std::cout << "SecuRec             : " << (((dev->ven_features & PX_SECUREC) != 0U) ? "YES" : "-") << "\n";
        std::cout << "Silent mode         : " << (((dev->ven_features & PX_SILENT) != 0U) ? "YES" : "-") << "\n";
        std::cout << "DVD+R bitsetting    : " << (((dev->ven_features & PX_BITSET_R) != 0U) ? "YES" : "-") << "\n";
        std::cout << "DVD+R DL bitsetting : " << (((dev->ven_features & PX_BITSET_RDL) != 0U) ? "YES" : "-") << "\n";
        std::cout << "DVD+R(W) testwrite  : " << (((dev->ven_features & PX_SIMUL_PLUS) != 0U) ? "YES" : "-") << "\n";
        std::cout << "AutoStrategy        : " << (((dev->ven_features & PX_ASTRATEGY) != 0U) ? "YES" : "-") << ((((dev->ven_features & PX_ASTRATEGY) != 0U) && (dev->dev_ID & (PLEXTOR_755 | PLEXTOR_760))) ? " (EXTENDED)" : "") << "\n";
        std::cout << "PlexEraser          : " << (((dev->ven_features & PX_ERASER) != 0U) ? "YES" : "-") << "\n";
        std::cout << "Pioneer QuietMode   : " << (((dev->ven_features & PIO_QUIET) != 0U) ? "YES" : "-") << "\n";
    }
...
//      std::cout << "Yamaha ForceSpeed   : " << ((dev->yamaha.forcespeed) ? "ON":"OFF") << "\n";
    if (((dev->ven_features & YMH_TATTOO) != 0U) && ((flags & FL_CURRENT) != 0U)) {
        if ((dev->yamaha.tattoo_rows) != 0U) {
            std::cout << "Yamaha DiscT@2      : inner " << dev->yamaha.tattoo_i << "outer " << dev->yamaha.tattoo_o << "image 3744 x " << dev->yamaha.tattoo_rows << "\n";
        } else {
            if ((dev->media.type & DISC_CD) != 0U) {
                std::cout << "DiscT@2: Can't write T@2 on inserted disc! No space left?\n";
            } else {
                std::cout << "DiscT@2: No disc found! Can't get T@2 info!\n";
            }
        }
    }
    if (((dev->ven_features & PX_HCDRSS) != 0U)      && (((flags & FL_CURRENT) != 0U) || ((flags & FL_HCDR) != 0U)))
    {
        std::cout << "Hide-CDR state      : " << (((dev->plextor.hcdr) != 0) ? "ON":"OFF") << "\n";
    }
    if (((dev->ven_features & PX_HCDRSS) != 0U)      && (((flags & FL_CURRENT) != 0U) || (((flags & FL_SSS) != 0U)))) {
        std::cout << "SingleSession state : " << (((dev->plextor.sss) != 0) ? "ON":"OFF") << "\n";
    }
    if (((dev->ven_features & PX_SPDREAD) != 0U)     && (((flags & FL_CURRENT) != 0U) || ((flags & FL_SPDREAD) != 0U))) {
        std::cout << "SpeedRead state     : " << (((dev->plextor.spdread) != 0) ? "ON":"OFF") << "\n";
    }
    if (((dev->ven_features & PX_POWEREC) != 0U)     && (((flags & FL_CURRENT) != 0U) || ((flags & FL_POWEREC) != 0U))) {
        std::cout << "PoweRec state       : " << (((dev->plextor.powerec_state) != 0) ? "ON":"OFF") << "\n";
        if ((dev->media.type & DISC_CD) != 0U) {
            std::cout << "      PoweRec Speed : " << (dev->plextor.powerec_spd / 176) << "X (CD)\n";
        }
        if ((dev->media.type & DISC_DVD) != 0U) {
            std::cout << "      PoweRec Speed : " << (dev->plextor.powerec_spd / 1385) << "X (DVD)\n";
        }
//      show_powerec_speed();
    }
    if (((dev->ven_features & PX_GIGAREC) != 0U)     && (((flags & FL_CURRENT) != 0U) || ((flags & FL_GIGAREC) != 0U))) {
    ...
    if (((dev->ven_features & PX_SILENT) != 0U)      && (((flags & FL_CURRENT) != 0U) || ((flags & FL_SILENT) != 0U))) {
    ...
    if (((dev->ven_features & PX_BITSET_RDL) != 0U)  && (((flags & FL_CURRENT) != 0U) || ((flags & FL_BOOK_RDL) != 0U))) {
        std::cout << "DVD+R DL bitsetting : " << (((dev->book_plus_rdl) != 0) ? "ON":"OFF") << "\n";
    }
    if (((dev->ven_features & PX_SIMUL_PLUS) != 0U)  && (((flags & FL_CURRENT) != 0U) || ((flags & FL_TESTWRITE) != 0U))) {
        std::cout << "DVD+R(W) testwrite  : " << (((dev->plextor.testwrite_dvdplus) != 0) ? "ON":"OFF") << "\n";
    }
    if (((dev->ven_features & PX_ASTRATEGY) != 0U)   && (((flags & FL_CURRENT) != 0U) || ((flags & FL_AS) != 0U))) {
...
void print_opts() {
    ...
    std::cout << "\t-l, --scanbus                list drives (scan IDE/SCSI bus)\n";
    ...
    std::cout << "\t--load                       load media (only for tray devices)\n";
    std::cout << "\t--loej                       load/eject media (toggle tray state)\n";
    std::cout << "\t--loej-immed                 don't wait for media is loaded or ejected\n";

    std::cout << "\noptions for Plextor devices:\n";
    std::cout << "\t--spdread [on|off]           turn SpeedRead on/off (default: off)\n";
    std::cout << "\t--sss [on|off]               turn SingleSession on/off (default: off)\n";
    std::cout << "\t--hcdr [on|off]              turn Hide-CDR on/off (default: off)\n";
    std::cout << "\t--powerec [on|off]           turn PoweREC on/off (default: on)\n";
//  std::cout << "\t--amqr [on|off]              turn Yamaha AMQR on/off (default: off)\n";
//  std::cout << "\t--forcespeed [on|off]        turn Yamaha ForceSpeed on/off (default: off)\n";
    std::cout << "\t--gigarec <state>            set GigaREC rate or turn it off\n";
    std::cout << "\t                             values: 0.6, 0.7, 0.8, 0.9, 1.1, 1.2, 1.3, 1.4, off\n";
    std::cout << "\t--varirec-cd <pwr>           set VariREC power for CD or turn VariREC off\n";
    std::cout << "\t                             values: +4, +3, +2, +1, 0, -1, -2, -3, -4, off\n";
    std::cout << "\t--varirec-cd-strategy <str>  set VariREC strategy for CD\n";
    std::cout << "\t                             values: default, azo, cya, pha, phb, phc, phd\n";
    std::cout << "\t--varirec-dvd <pwr>          set VariREC power for DVD or turn VariREC off\n";
    std::cout << "\t                             values: +4, +3, +2, +1, 0, -1, -2, -3, -4, off\n";
    std::cout << "\t--varirec-dvd-strategy <str> set VariREC strategy for DVD\n";
    std::cout << "\t                             values: default, 0, 1, 2, 3, 4, 5, 6, 7\n";
    std::cout << "\t--securec <passwd>           set SecuREC password and turn it on\n";
    std::cout << "\t                             passwd must be 4..10 characters length\n";
    std::cout << "\t--nosecurec                  turn SecuRec off\n";
    std::cout << "\t--bitset+r [on|off]          turn on/off setting DVD-ROM book on DVD+R media\n";
    std::cout << "\t--bitset+rdl [on|off]        turn on/off setting DVD-ROM book on DVD+R DL media\n";
    std::cout << "\t--mqck <quick|advanced>      run MediaQuality Check\n";
    std::cout << "\t--mqck-speed #               set MQCK speed (default: maximum)\n";
    std::cout << "\t--as-mode <mode>             AS: select AutoStrategy mode: forced,auto,on,off,\n";
    ...
    std::cout << "\t--as-create <q|f> <a|r>      AS: create new strategy for inserted DVD media (PX-755/PX-760 only)\n";
    std::cout << "\t                             mode:    q = quick, f = full\n";
    std::cout << "\t                             action:  a = add,   r = replace\n";
    std::cout << "\t--as-save <file>             AS: save Strategies            / EXPERIMENTAL /\n";
    std::cout << "\t--as-load <file>             AS: load Strategy from file    / EXPERIMENTAL /\n";
    std::cout << "\t--as-clear                   AS: remove ALL strategies from database\n";
    std::cout << "\t--dvd+testwrite [on|off]     turn on/off testwrite on DVD+R(W) media\n";
    std::cout << "\t--silent [on|off]            just turn Silent Mode on/off (default: on)\n";
    std::cout << "\t--sm-nosave                  don't save Silent Mode settings\n";
    std::cout << "\t--sm-cd-rd #                 set max CD READ speed (default: 32X)\n";
    std::cout << "\t                             values: 4, 8, 24, 32, 40, 48\n";
    std::cout << "\t--sm-cd-wr #                 set max CD WRITE speed (default: 32X)\n";
    std::cout << "\t                             values: 4, 8, 16, 24, 32, 48\n";
    std::cout << "\t--sm-dvd-rd #                set max DVD READ speed (default: 12X)\n";
    std::cout << "\t                             values: 2, 5, 8, 12, 16\n";
//  std::cout << "\t--sm-dvd-wr #                set max DVD WRITE speed (default: 8X)\n";
//  std::cout << "\t                             values: 4, 6, 8, 12, 16\n";
    std::cout << "\t--sm-load #                  set tray load speed. spd can be 0 to 80 (default: 63)\n";
    std::cout << "\t--sm-eject #                 set tray eject speed. spd can be 0 to 80 (default: 0)\n";
    std::cout << "\t--sm-access <slow|fast>      set access time, has effect only with CD/DVD speed setting\n";
    std::cout << "\t--destruct <quick|full>      perform PlexEraser function\n";
    std::cout << "\t                             WARNING! data on inserted CD-R/DVD-R will be lost!\n";
    std::cout << "\t                             works on PX-755/PX-760/Premium2 only\n";
...
//  int silent_dvd_wr = SILENT_DVD_WR_8X;
    ...
    std::cout << "**  CDVD Control v" << version << "  (c) 2005-2009  Gennady \"ShultZ\" Kozlov\n";
...
//  ************   GigaREC
            ...
            if (argc>(i+1)) {
                ...
                else if ((strcmp(argv[i],"1.0")) == 0 || ((strcmp(argv[i],"off")) == 0)) {
                    gigarec = GIGAREC_10;
                }
...
//  ************   VariREC  DVD
            ...
            if (argc>(i+1)) {
                ...
                if      (!strcmp(argv[i],"q")) ascre |= ASDB_CRE_QUICK;
                    ...
                    std::cout << "invalid --as-create parameter 1: '" << argv[i] << "'. Only 'q|f' are valid\n";
            }
            if (argc>(i+1)) {
                i++;
                if      (!strcmp(argv[i],"a")) ascre |= ASDB_ADD;
                    ...
                    std::cout << "invalid --as-create parameter 2: '" << argv[i] << "'. Only 'a|r' are valid\n";
            }
        }
...
//  std::cout << "\t--pio-nosave                don't save settings to drive (changes will be lost after reboot)\n";
    ...
    if (flags & FL_SCAN) {
        ...
        if (!drvcnt) std::cout << "ERR: no drives found!\n";
        return 2;
    }
    if (!(flags & FL_DEVICE)) {
...
//  convert_to_ID(dev);
    ...
    if (get_drive_serial_number(dev)) std::cout << "Serial#: " << dev->serial << "\n";

    if (flags) {
//  if (flags & FL_VERBOSE) {
        std::cout << "\nCDVD Control flags : ";
        if (flags & FL_VERBOSE)     std::cout << " VERBOSE";
        if (flags & FL_DEVICE)      std::cout << " DEVICE";
        if (flags & FL_HELP)        std::cout << " HELP";
        if (flags & FL_CURRENT)     std::cout << " CURRENT";
        if (flags & FL_SUPPORTED)   std::cout << " SUPPORTED";
        if (flags & FL_SCAN)        std::cout << " SCAN";
        if (flags & FL_LOCK)        std::cout << " LOCK";
        if (flags & FL_HCDR)        std::cout << " HCDR";
        if (flags & FL_SSS)         std::cout << " SSS";
        if (flags & FL_SPDREAD)     std::cout << " SPDREAD";
        if (flags & FL_POWEREC)     std::cout << " POWEREC";
        if (flags & FL_GIGAREC)     std::cout << " GIGAREC";
        if (flags & FL_VARIREC_CD)  std::cout << " VARIREC_CD";
        if (flags & FL_VARIREC_DVD) std::cout << " VARIREC_DVD";
        if (flags & FL_SECUREC)     std::cout << " SECUREC";
        if (flags & FL_BOOK_R)      std::cout << " BITSET_R";
        if (flags & FL_BOOK_RDL)    std::cout << " BITSET_RDL";
        if (flags & FL_TESTWRITE)   std::cout << " DVD+TESTWRITE";
        if (flags & FL_AS)          std::cout << " AS";
        if (flags & FL_SILENT)      std::cout << " SILENT";
        if (flags & FL_YMH_AMQR)    std::cout << " AMQR";
        if (flags & FL_YMH_FORCESPEED)  std::cout << " FORCESPEED";
        if (flags & FL_PIOLIMIT)    std::cout << " PIOLIMIT";
        if (flags & FL_PIOQUIET)    std::cout << " PIOQUIET";
        if (flags & FL_LOEJ)        std::cout << " LOEJ";
        if (flags_as & FLAS_RETR)   std::cout << " AS_RETR";
        if (flags_as & FLAS_STOR)   std::cout << " AS_STOR";
        if (flags_as & FLAS_CREATE) std::cout << " AS_CREATE";
        if (flags_as & FLAS_DEL)    std::cout << " AS_DEL";
        if (flags_as & FLAS_VIEW)   std::cout << " AS_VIEW";
        if (flags_as & FLAS_ACT)    std::cout << " AS_ACT";
        if (flags_as & FLAS_DEACT)  std::cout << " AS_DEACT";
        if (flags_as & FLAS_CLEAR)  std::cout << " AS_CLEAR";
        std::cout << "\n\n";
    }
//  std::cout << "____________________________\n";
    if (flags & FL_LOCK) {
...
//      std::cout << "PWR = %02X   STR = %02X\n",varirec_dvd_pwr, varirec_dvd_str);
    ...
    if (flags & FL_MQCK) {
        ...
        std::cout << "WR speed max: " << (((float)dev->parms.max_write_speed_kb) / dev->parms.speed_mult) << "X (" << dev->parms.max_write_speed_kb << " kB/s)\n";
        for (int i=0; i<speed_tbl_size && dev->parms.wr_speed_tbl_kb[i] > 0; i++) {
            std::cout << "  speed #" << i << ":" << (((float)dev->parms.wr_speed_tbl_kb[i]) / ((float)dev->parms.speed_mult)) << "X" << "(" << dev->parms.wr_speed_tbl_kb[i] << " kB/s)\n";
        }
        if (as_mqck >= 0) {
            ...
            std::cout << "Starting media check at speed " << (((float)dev->parms.max_write_speed_kb) / dev->parms.speed_mult) << " X...\n";
    ...
    if (flags_as & FLAS_CREATE) {
        std::cout << "AS: Creating new strategy, mode: " << ((ascre & ASDB_CRE_FULL) == ASDB_CRE_FULL? "FULL" : "QUICK") << ", action: " << ((ascre & ASDB_ADD) == ASDB_ADD? "ADD" : "REPLACE") << "...\n";
    ...
    if (flags_as & FLAS_STOR) {
        ...
        std::cout << dev->astrategy.dbcnt << " strategies loaded, sending to drive...\n";
