...
int print_sense (int err) {
    ...
    case 0x1:
        ...
        case 0x5D:
            switch (ASCQ(err)) {
            case 0x01:
                strcpy(str,"FAILURE PREDICTION THRESHOLD EXCEEDED - Predicted Media failure");
                break;
            case 0x02:
                strcpy(str,"LOGICAL UNIT FAILURE PREDICTION THRESHOLD EXCEEDED");
                break;
            case 0x03:
                strcpy(str,"FAILURE PREDICTION THRESHOLD EXCEEDED - Predicted Spare Area Exhaustion");
                break;
            case 0xFF:
    ...
    case 0x5:
        ...
        case 0x6F:
            switch (ASCQ(err)) {
            case 0x00:
                strcpy(str,"COPY PROTECTION KEY EXCHANGE FAILURE - AUTHENTICATION FAILURE");
                break;
            case 0x01:
                strcpy(str,"COPY PROTECTION KEY EXCHANGE FAILURE - KEY NOT PRESENT");
                break;
            case 0x02:
                strcpy(str,"COPY PROTECTION KEY EXCHANGE FAILURE - KEY NOT ESTABLISHED");
                break;
            case 0x03:
            ...
            case 0x04:
                ...
                break;
            case 0x05:
                strcpy(str,"LOGICAL UNIT REGION MUST BE PERMANENT/REGION RESET COUNT ERROR");
                break;
            }
            break;
        case 0x72:
...
int request_sense(drive_info* drive, char add) {
    ...
    {
        if (!drive->silent) sperror ("REQUEST_SENSE",drive->err);
        return (drive->err);
    }
    return 0;
}
...
int mode_sense(drive_info* drive, int page, int page_control, int dest_len) {
    ...
    if ((drive->err=drive->cmd.transport(READ,drive->rd_buf,dest_len) ))
    {
        if (!drive->silent) sperror ("MODE_SENSE(10)",drive->err);
        return (drive->err);
    }
    return 0;
}
...
int mode_select(drive_info* drive, int dest_len) {
    ...
    */
    ...
    if ((drive->err=drive->cmd.transport(WRITE,drive->rd_buf,dest_len) ))
    {
        if (!drive->silent) sperror ("MODE_SELECT(10)",drive->err);
        return (drive->err);
    }
...
int get_configuration(drive_info* drive, int feature_number, unsigned int* data_length, int* current, unsigned char ReqType) {
    ...
    {
        if (!drive->silent) sperror ("GET_CONFIGURATION (LENGTH)",drive->err);
        return (drive->err);
    }
    if (data_length)
    {
        *data_length = ntoh32u(drive->rd_buf);
        ...
        if (*data_length > 4)
            ...
            {
                if (!drive->silent) sperror ("GET_CONFIGURATION",drive->err);
                return (drive->err);
            }
    }
...
int write_buffer(drive_info* drive, uint8_t mode, uint8_t buff_id, uint32_t offs, uint32_t len)
{
    ...
    {
        if (!drive->silent) sperror ("WRITE_BUFFER",drive->err);
        return (drive->err);
    }
...
int read_buffer(drive_info* drive, uint8_t mode, uint8_t buff_id, uint32_t offs, uint32_t len)
{
    ...
    {
        if (!drive->silent) sperror ("READ_BUFFER",drive->err);
        return (drive->err);
    }
...
//	drive->cmd[1] = 0x00;
    ...
    {
        if (!drive->silent) sperror ("READ_ECHO_BUFFER_SIZE",drive->err);
        return (0);
    }
...
//	drive->cmd[0] = SBC_FLUSH_CACHE;
    ...
    if ((drive->err=drive->cmd.transport(NONE, NULL, 0) ))
    {
        if (!drive->silent) sperror ("SBC_FLUSH_CACHE",drive->err);
        return (drive->err);
    }
...
int read_atip(drive_info* drive) {
    ...
    {
        if (!drive->silent) sperror ("READ_ATIP",drive->err);
        drive->media.ATIP_size = 0;
        return 1;
    }
...
//		{sperror ("READ_TOC",drive->err); return 1;}
    ...
    {
        if (!drive->silent) sperror ("READ_TOC",drive->err);
        return 1;
    }
...
int read_disc_info(drive_info* drive, int len) {
    ...
    if ((drive->err=drive->cmd.transport(READ,drive->rd_buf,len) ))
    {
        if (!drive->silent) sperror ("READ_DISC_INFO",drive->err);
        return 1;
    }
    return 0;
}
...
int read_track_info(drive_info* drive, trk* track, unsigned int track_n) {
    ...
    if ((drive->err=drive->cmd.transport(READ,drive->rd_buf,size) ))
    {
        if (!drive->silent) sperror ("READ_TRACK_INFO",drive->err);
        return 1;
    }
...
//			if (!drive->silent) printf("CD-R(W) Capacity: %02d:%02d.%02d (sector %d)\n", lout.m, lout.s, lout.f, ilout);
        ...
        if (drive->media.type & (DISC_DVDmRWR | DISC_DVDmRWS | DISC_DVDmRWDL)) {
            ...
            if ((drive->err = drive->cmd.transport(READ,drive->rd_buf,len)))
            {
                if (!drive->silent) sperror ("READ_FORMAT_CAPACITIES",drive->err);
...
read_total_dvdmr:
            ...
            if ((drive->err = drive->cmd.transport(READ,drive->rd_buf,len)))
            {
                if (!drive->silent) sperror ("READ_DVD_STRUCTURE 10",drive->err);
                return 1;
            }
...
#warning DVD+RW total sectors reading
            ...
            if ((drive->err = drive->cmd.transport(READ,drive->rd_buf,len)))
            {
                if (!drive->silent) sperror ("READ_DVD_STRUCTURE 00",drive->err);
                return 1;
            }
...
int read_mediaid_bd(drive_info* drive) {
    ...
    if ((drive->err = drive->cmd.transport(READ,drive->rd_buf,4)))
    {
        if (!drive->silent) sperror ("READ_BD_STRUCTURE 00",drive->err);
        return 1;
    }
...
//	union { unsigned char _e[4+40],_11[4+256]; } dvd;
    ...
    if ((drive->err = drive->cmd.transport(READ,header,4)))
    {
        if (!drive->silent) sperror ("READ_DVD_STRUCTURE 0E",drive->err);
        return 1;
    }
    len = (header[0]<<8|header[1]) + 2;
    if (len>68) len= 68;
...
int read_mediaid_dvdplus(drive_info* drive) {
    ...
    if ((drive->err = drive->cmd.transport(READ,header,4)))
    {
        if (!drive->silent) sperror ("READ_DVD_STRUCTURE 11",drive->err);
        return 1;
    }
    len = (header[0]<<8|header[1]) + 2;
    if (len>260) len= 260;
...
//	int	i;
    ...
    {
        if (!drive->silent) sperror ("GET_PERFORMANCE",drive->err);
        return (drive->err);
    }
//	return 1;
...
//	*end_lba = ntoh32(0);
    ...
    *write_size = ntoh32(speed_wr);
    ...
    {
        if(!drive->silent) sperror ("SET_STREAMING",drive->err);
        return (drive->err);
    }
    return 0;
}
