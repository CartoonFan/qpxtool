...
static int initCSSCache(drive_info *dev) {
    ...
    if (psz_cache != NULL) {
        ...
        if (strlen(psz_cache) + 1 + 32 + 1 + (DVD_KEY_SIZE * 2) + 10 + 1 >
                PATH_MAX) {
    ...
    if (!dev->silent) {
            ...
            //		&& p_title->p_next != NULL
        {
            printf("STA: %8X,  KEY: %02X:%02X:%02X:%02X:%02X\n", p_title->i_startlb,
                   p_title->p_key[0], p_title->p_key[1], p_title->p_key[2],
...
void *read_disc(void *arg) {
    ...
    if (dev->media.type & DISC_CD) {
        ...
        case 2: // CPRM - protected DVD
            ...
            break;
        default:
            //	printf("Unknown DVD protection scheme:
    ...
    if (dev->media.dvdcss.protection == 0x01) {
        ...
        printf("\nSetting title key: %02X:%02X:%02X:%02X:%02X  for %x-%x, CSS=%d\n",
               dev->media.dvdcss.TK[0], dev->media.dvdcss.TK[1],
               dev->media.dvdcss.TK[2], dev->media.dvdcss.TK[3],
               dev->media.dvdcss.TK[4], 0, title_next, descramble);
    }
    ...
    while (lba < dev->media.capacity) {
        ...
        if (dev->media.type & DISC_CD) {
            ...
            if (dev->media.dvdcss.protection == 0x01) {
                ...
                if (lba >= title_next && p_title) {
                    ...
                    if (!memcmp(dev->media.dvdcss.TK, "\0\0\0\0\0", 5))
                        descramble = 0;
                    else
                        descramble = 1;
                    printf("\nSetting title key: %02X:%02X:%02X:%02X:%02X  for %x-%x, "
                           "CSS=%d\n",
                           dev->media.dvdcss.TK[0], dev->media.dvdcss.TK[1],
                           dev->media.dvdcss.TK[2], dev->media.dvdcss.TK[3],
                           dev->media.dvdcss.TK[4], lba, title_next, descramble);
                    p_title = p_title->p_next;
                }

                read_dvd(dev, dev->rd_buf, lba, scnt,
                         descramble ? DVDCSS_READ_DECRYPT : 0);
            } else {
                read(dev, dev->rd_buf, lba, scnt);
            }
        }
        ...
        if (parm->pass < PASS_RECOVER1) {
            if ((dev->media.type) & DISC_DVD &&
                    (dev->media.dvdcss.protection == 0x01)) {
