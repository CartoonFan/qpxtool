...
int plextor_read_eeprom(drive_info *drive, int *len) {
  ...
  case PLEXTOR_760:
    ...
    break;
  default:
    std::cout << "Don't know how to read eeprom from this device: "
...
int plextor_get_speeds(drive_info *drive) {
  ...
  if (!drive->silent) {
    std::cout << "Selected write speed : " << sel << " kB/s (" << sel / 177
              << " X)\n";
    std::cout << "Max for this media   : " << max << " kB/s (" << max / 177
              << " X)\n";
    std::cout << "Last actual speed    : " << last << " kB/s (" << last / 177
              << " X)\n";
  }
  return 0;
}
...
int plextor_get_powerec(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent) {
      sperror("GET_POWEREC", drive->err);
      return drive->err;
    }
  }
  drive->plextor.powerec_state = drive->rd_buf[2];
  //	drive->plextor.powerec_spd = ((drive->rd_buf[4] & 0xFF)<<8) |
  //(drive->rd_buf[5] & 0xFF);
  drive->plextor.powerec_spd = ntoh16(drive->rd_buf + 4);
  if (!drive->silent) {
    std::cout << "\tPoweRec " << ((drive->plextor.powerec_state) ? "ON" : "OFF")
              << ", Recomended speed: " << drive->plextor.powerec_spd
              << " kB/s\n";
  }
  //	printf("\t"); for (int i=0; i<8; i++) printf("0x%02X
  //",drive->rd_buf[i]&0xFF); printf("\n");
  return 0;
}

int plextor_set_powerec(drive_info *drive) {
  std::cout << "\tTurning PoweRec "
...
void plextor_print_silentmode_state(drive_info *drive) {
  int val = drive->plextor_silent.rd;
  int idx = 0;
  std::cout << "Active SilentMode settings:\n";
  std::cout << "\tSM Read speed  : ";
  if (drive->media.type & DISC_DVD) {
    while ((silent_dvd_rd_tbl[idx].val != 0xFF) &
           (silent_dvd_rd_tbl[idx].val != (val & 0xFF))) {
      idx++;
    }
    std::cout << silent_dvd_rd_tbl[idx].name << "\n";
    //		printf("%s : %d\n",silent_dvd_rd_tbl[idx].name, val);
  } else {
    while ((silent_cd_rd_tbl[idx].val != 0xFF) &
           (silent_cd_rd_tbl[idx].val != (val & 0xFF))) {
      idx++;
    }
    std::cout << silent_cd_rd_tbl[idx].name << "\n";
    //		printf("%s : %d\n",silent_cd_rd_tbl[idx].name, val);
  }
  val = drive->plextor_silent.wr;
  idx = 0;
  ...
  if (drive->media.type & DISC_DVD) {
    ...
    while ((silent_cd_wr_tbl[idx].val != 0xFF) &
           (silent_cd_wr_tbl[idx].val != (val & 0xFF)))
      idx++;
    std::cout << silent_cd_wr_tbl[idx].name << "\n";
    //		printf("%s : %d\n",silent_cd_wr_tbl[idx].name, val);
  }
  std::cout << "\tSM Access time : "
            << ((drive->plextor_silent.access) ? "SLOW" : "FAST") << "\n";
  std::cout << "\tSM Load speed  : " << drive->plextor_silent.load << "\n";
  std::cout << "\tSM Eject speed : " << drive->plextor_silent.eject << "\n";

  if (!drive->plextor_silent.psaved) {
    std::cout << "Saved SilentMode settings not found\n";
    return;
  }

  std::cout << "Saved SilentMode settings:\n";

  std::cout << "\tPSM Silent State   : "
            << ((drive->plextor_silent.pstate) ? "ON" : "OFF") << "\n";

  std::cout << "\tPSM CD Read speed  : " << drive->plextor_silent.prd_cd
            << "X\n";
  std::cout << "\tPSM CD Write speed : " << drive->plextor_silent.pwr_cd
            << "X\n";
  std::cout << "\tPSM DVD Read speed : " << drive->plextor_silent.prd_dvd
            << "X\n";
  //	printf("\tPSM DVD Write speed: %d\n",drive->plextor_silent.pwr_dvd);

  std::cout << "\tPSM Access time : "
            << ((drive->plextor_silent.paccess) ? "SLOW" : "FAST") << "\n";
  std::cout << "\tPSM Load speed  : " << drive->plextor_silent.pload << "\n";
  std::cout << "\tPSM Eject speed : " << drive->plextor_silent.peject << "\n";
}

void plextor_get_silentmode_saved(drive_info *drive) {
  int len;
  if (plextor_read_eeprom(drive, &len) || len < 0x110)
    return;
...
int plextor_get_silentmode(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("GET_SILENT_MODE", drive->err);
    return drive->err;
  }
  if ((drive->err =
           drive->cmd.transport(READ, (void *)&(drive->plextor_silent), 8))) {
    if (!drive->silent)
      sperror("GET_SILENT_MODE", drive->err);
    return drive->err;
  }
...
int plextor_set_silentmode_tray(drive_info *drive, int disc_type,
                                int permanent) {
  drive->cmd[0] = PLEXTOR_MODE;
  drive->cmd[1] = PLEX_GET_MODE;
  ...
  if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
    if (!drive->silent)
      sperror("SET_SILENT_MODE_DISC", drive->err);
    return drive->err;
  }
  //	printf("\t"); for (int i=0; i<8; i++) printf("0x%02X
  //",drive->rd_buf[i]&0xFF); printf("\n");
...
int plextor_set_silentmode_disc(drive_info *drive, int disc_type,
                                int permanent) {
  drive->cmd[0] = PLEXTOR_MODE;
  drive->cmd[1] = PLEX_GET_MODE;
...
void print_gigarec_value(drive_info *drive) {
  ...
  int g = 0;
  int i = drive->plextor.gigarec;
  printf("GigaRec state       : ");
  while ((gigarec_tbl[g].val != 0xFF) & (gigarec_tbl[g].val != (i & 0xFF)))
    g++;
  printf("%s\nDisc GigaRec rate   : ", gigarec_tbl[g].name);
  i = drive->plextor.gigarec_disc;
  g = 0;
  while ((gigarec_tbl[g].val != 0xFF) & (gigarec_tbl[g].val != (i & 0xFF)))
    g++;
  printf("%s\n", gigarec_tbl[g].name);
...
void print_varirec(drive_info *drive, int disc_type) {
  ...
  v = 0;
  while ((varirec_pwr_tbl[v].val != 0xFF) &
         (varirec_pwr_tbl[v].val != (i & 0xFF)))
    v++;
  printf("\tVariRec %s power    : %s\n",
         (disc_type == VARIREC_DVD) ? "DVD" : "CD ", varirec_pwr_tbl[v].name);
  printf("\tVariRec %s strategy : %s [%d]\n",
         (disc_type == VARIREC_DVD) ? "DVD" : "CD ",
         (disc_type == VARIREC_DVD) ? varirec_str_dvd_tbl[s]
...
int plextor_get_securec_state(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_SECUREC", drive->err);
    return drive->err;
  }
...
int plextor_set_securec(drive_info *drive, char len, char *passwd) {
  ...
  if ((passwd) && (len)) {
    ...
    if ((drive->err = drive->cmd.transport(WRITE, drive->rd_buf, 0x10))) {
      if (!drive->silent)
        sperror("PLEXTOR_SET_SECUREC", drive->err);
      return drive->err;
    }
  } else {
...
int plextor_set_speedread(drive_info *drive, int state) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("SET_SPDREAD", drive->err);
    return drive->err;
  }
  //	printf("** SPDREAD: "); for (int i=0; i<8; i++) printf("0x%02X
  //",drive->rd_buf[i]&0xFF); printf("\n");
...
int plextor_get_hidecdr_singlesession(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("GET_HCDR_SSS", drive->err);
    return drive->err;
  }
...
int plextor_set_hidecdr_singlesession(drive_info *drive, int hidecdr_state,
                                      int singlesession_state) {
  if (plextor_get_hidecdr_singlesession(drive))
    return 1;
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("SET_HCDR_SSS", drive->err);
    return drive->err;
  }
  //	printf("** HCDR_SSS: "); for (int i=0; i<8; i++) printf("0x%02X
  //",drive->rd_buf[i]&0xFF); printf("\n");
  drive->plextor.hcdr = !!(drive->rd_buf[2] & 0x02);
  drive->plextor.sss = (drive->rd_buf[2]) & 0x01;
  return 0;
...
int plextor_set_hidecdr(drive_info *drive, int state) {
  ...
  return plextor_set_hidecdr_singlesession(drive, drive->plextor.hcdr,
                                           drive->plextor.sss);
}
...
int plextor_get_bitset(drive_info *drive, int disc_type) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_BITSET", drive->err);
    return drive->err;
  }
  switch (disc_type) {
  case PLEX_BITSET_R:
    drive->book_plus_r = ((drive->rd_buf[2] & 0x02) == 0x02);
    break;
  case PLEX_BITSET_RDL:
    drive->book_plus_rdl = ((drive->rd_buf[2] & 0x01) == 0x01);
    break;
  }
  return 0;
}
...
int plextor_set_bitset(drive_info *drive, int disc_type) {
  ...
  switch (disc_type) {
  case PLEX_BITSET_R:
    book = (drive->book_plus_r);
    break;
  case PLEX_BITSET_RDL:
    book = (drive->book_plus_rdl);
    break;
  default:
    printf("PLEXTOR_SET_BITSET: Invalid disc_type");
    return 1;
  }
  drive->cmd[0] = PLEXTOR_MODE;
  drive->cmd[1] = PLEX_SET_MODE;
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_SET_BITSET", drive->err);
    return drive->err;
  }
  return 0;
}
...
int plextor_get_testwrite_dvdplus(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_TESTWRITE_DVDPLUS", drive->err);
    return drive->err;
  }
...
int plextor_set_testwrite_dvdplus(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
    if (!drive->silent)
      sperror("PLEXTOR_SET_TESTWRITE_DVDPLUS", drive->err);
    return drive->err;
  }
  return 0;
}
...
int plextor_plexeraser(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
    if (!drive->silent)
      sperror("PLEXTOR_DO_PLEXERASER", drive->err);
    return drive->err;
  }
...
int plextor_get_autostrategy(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_AUTOSTRATEGY", drive->err);
    return drive->err;
  }
  //	printf("** GET AS: "); for (int i=0; i<8; i++) printf("0x%02X
  //",drive->rd_buf[i]&0xFF); printf("\n");
...
int plextor_set_autostrategy(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_SET_AUTOSTRATEGY", drive->err);
    return drive->err;
  }
  //	printf("** SET AS: "); for (int i=0; i<8; i++) printf("0x%02X
  //",drive->rd_buf[i]&0xFF); printf("\n");
  drive->astrategy.state = (drive->rd_buf[2] & 0x0F);
...
int plextor_get_autostrategy_db_entry_count(drive_info *drive) {
  ...
  drive->cmd[10] = 0x08;
  if ((drive->err =
           drive->cmd.transport(READ, (void *)&(drive->astrategy), 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_ASDB_ENTRY_COUNT", drive->err);
    return drive->err;
  }
...
int plextor_get_autostrategy_db(drive_info *drive) //, void* database)
{
  ...
  if ((drive->err =
           drive->cmd.transport(READ, (void *)&(drive->astrategy), size))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_ASDB", drive->err);
    return drive->err;
  }
  printf("** AS DB entries: %d\n", drive->astrategy.dbcnt);
  //*
  for (j = 0; j < drive->astrategy.dbcnt; j++) {
    ...
    printf("S#%02d |%c| DVD%cR [%02X] | %3dX | %12s | %d\n",
           ...
           (drive->astrategy.entry[j].type == 0xA1) ? '+' : '-',
           drive->astrategy.entry[j].type, drive->astrategy.entry[j].speed,
           drive->astrategy.entry[j].MID,
           (drive->astrategy.entry[j].counter[0] << 8) |
               drive->astrategy.entry[j].counter[1]);
  }
  //*/
  /*
      char ch;
      ...
      printf("\n");
      for (j=0; j<drive->astrategy.dbcnt; j++) {
              for (i=0; i<32; i++) printf("%02X ", drive->rd_buf[j*32+i+8] &
     0xFF); printf("\n"); for (i=0; i<32; i++) { if (drive->rd_buf[j*32+i+8] >
     0x1F) ch = drive->rd_buf[j*32+i+8]; else ch = 0x20; printf("%c", ch);
              }
              printf("\n");
      }
  */
...
int plextor_get_strategy(drive_info *drive) {
  ...
  drive->cmd[10] = 0x10;
  if ((drive->err =
           drive->cmd.transport(READ, drive->rd_buf, cnt * 0x100 + 0x10))) {
    if (!drive->silent)
      sperror("PLEXTOR_GET_STRATEGY_DATA", drive->err);
    return drive->err;
  }
  ...
  std::cout << "DB HDR0:\n";
  for (i = 0; i < 8; i++) {
    std::cout << drive->rd_buf[i];
    std::cout << "\n";
  }
  for (s = 0; s < cnt; s++) {
    offs = 8 + s * 0x20;
    for (i = 0; i < 0x20; i++) {
      std::cout << drive->rd_buf[offs + i];
    }
    std::cout << "\n";
  }
  offs = 8 + cnt * 0x20;
  ...
  for (s = 0; s < cnt; s++) {
    ...
    offs = 8 + s * 0x20;
    for (i = 0; i < 0x20; i++)
      entry[i] = drive->rd_buf[offs + i];

    offs = 0x10 + cnt * 0x20 + s * 0xE0;
    for (i = 0; i < (0x20 * 7); i++)
      entry_data[i] = drive->rd_buf[offs + i];
  }
  return 0;
}
...
int plextor_add_strategy(drive_info *drive) {
  ...
  for (i = 0; i < cnt * 0x20; i++)
    drive->rd_buf[8 + i] = entry[i];
  for (i = 0; i < cnt; i++)
    drive->rd_buf[8 + i * 0x20] = (unsigned char)((i + 1) & 0xFF);
  ...
  if ((drive->err =
           drive->cmd.transport(WRITE, drive->rd_buf, cnt * 0x20 + 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_ADD_STRATEGY_HDR", drive->err);
    return drive->err;
  }

  // sending strategy parameters
  for (i = 0; i < (8 + cnt * 0xE0); i++)
    drive->rd_buf[i] = 0;
  ...
  for (int s = 0; s < cnt; s++) {
    ...
    for (int s1 = 0; s1 < 7; s1++) {
      ...
      for (i = 0; i < 0x20; i++)
        printf("%02X ", drive->rd_buf[offs + i]);
      printf("\n");
    }
  }
#endif
  ...
  if ((drive->err =
           drive->cmd.transport(WRITE, drive->rd_buf, cnt * 0xE0 + 8))) {
    if (!drive->silent)
      sperror("PLEXTOR_ADD_STRATEGY_DATA", drive->err);
    return drive->err;
  }
  return 0;
}
...
int plextor_clear_autostrategy_db(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(WRITE, drive->rd_buf, 0x08))) {
    if (!drive->silent)
      sperror("PLEXTOR_CLEAR_ASTRATEGY_DB", drive->err);
    return drive->err;
  }
  return 0;
}
...
int plextor_modify_autostrategy_db(drive_info *drive, int index, int action) {
  ...
  if ((drive->err = drive->cmd.transport(WRITE, drive->rd_buf, 0x0A))) {
    if (!drive->silent)
      sperror("PLEXTOR_MODIFY_ASDB", drive->err);
    return drive->err;
  }
  return 0;
}
...
int plextor_create_strategy(drive_info *drive, int mode) {
  ...
  if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
    if (!drive->silent)
      sperror("PLEXTOR_CREATE_STRATEGY_START", drive->err);
    return drive->err;
  }
  ...
  if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, 0x12))) {
    if (!drive->silent)
      sperror("PLEXTOR_AS_GET_STATUS", drive->err);
    return drive->err;
  }
...
int plextor_media_check(drive_info *drive, int mode) {
  ...
  if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
    if (!drive->silent)
      sperror("PLEXTOR_MEDIA_QUALITY_CHECK_START", drive->err);
    return drive->err;
  }
