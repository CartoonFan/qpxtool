...
int plextor_read_eeprom(drive_info* drive, int* len) {
    ...
    case PLEXTOR_760:
        ...
        break;
    default:
        std::cout << "Don't know how to read eeprom from this device: " << drive->ven_ID << ":" << drive->dev_ID << "\n";
        if (len) *len=0;
        break;
    }
    if (len) *len=l;
...
//	last = (drive->rd_buf[9]<<8) | drive->rd_buf[8];
    ...
    if (!drive->silent) {
        std::cout << "Selected write speed : " << sel << " kB/s (" << sel/177 << " X)\n";
        std::cout << "Max for this media   : " << max << " kB/s (" << max/177 << " X)\n";
        std::cout << "Last actual speed    : " << last << " kB/s (" << last/177 << " X)\n";
    }
    return 0;
}
...
//	drive->plextor.powerec_spd = ((drive->rd_buf[4] & 0xFF)<<8) | (drive->rd_buf[5] & 0xFF);
    ...
    if (!drive->silent) {
        std::cout << "\tPoweRec " << ((drive->plextor.powerec_state) ? "ON" : "OFF") << ", Recomended speed: " << drive->plextor.powerec_spd << " kB/s\n";
    }
...
int plextor_set_powerec(drive_info* drive) {
    std::cout << "\tTurning PoweRec " << ((drive->plextor.powerec_state) ? "ON" : "OFF") << "\n";
    drive->cmd[0]=PLEXTOR_MODE2;
...
void plextor_print_silentmode_state(drive_info* drive) {
    ...
    if ( drive->media.type & DISC_DVD ) {
        while ((silent_dvd_rd_tbl[idx].val!=0xFF) & (silent_dvd_rd_tbl[idx].val!=(val & 0xFF))) {
            idx++;
        }
        std::cout << silent_dvd_rd_tbl[idx].name << "\n";
//		printf("%s : %d\n",silent_dvd_rd_tbl[idx].name, val);
    } else {
        while ((silent_cd_rd_tbl[idx].val!=0xFF) & (silent_cd_rd_tbl[idx].val!=(val & 0xFF))) {
            idx++;
        }
...
//		printf("%s : %d\n",silent_cd_wr_tbl[idx].name, val);
    ...
    std::cout << "\tSM Access time : " << ((drive->plextor_silent.access) ? "SLOW" : "FAST") << "\n";
    ...
    std::cout << "\tPSM Silent State   : " << ((drive->plextor_silent.pstate) ? "ON":"OFF") << "\n";

    std::cout << "\tPSM CD Read speed  : " << drive->plextor_silent.prd_cd << "X\n";
    std::cout << "\tPSM CD Write speed : " << drive->plextor_silent.pwr_cd << "X\n";
    std::cout << "\tPSM DVD Read speed : " << drive->plextor_silent.prd_dvd << "X\n";
//	printf("\tPSM DVD Write speed: %d\n",drive->plextor_silent.pwr_dvd);

    std::cout << "\tPSM Access time : " << ((drive->plextor_silent.paccess)?"SLOW":"FAST") << "\n";
...
//	drive->plextor.gigarec_disc = (drive->rd_buf[4] & 0xFF);
    ...
    for (int i=0; i<8; i++) printf("0x%02X ",drive->rd_buf[i]&0xFF);
    printf("\n");
#endif // DEBUG_SECUREC
    return 0;
}
...
//	return 0;
    ...
    if ((drive->err=drive->cmd.transport(NONE, NULL, 0) ))
    {
        if (!drive->silent) sperror ("PLEXTOR_DO_PLEXERASER",drive->err);
        return drive->err;
    }
...
int plextor_get_autostrategy(drive_info* drive)
{
    ...
    if ((drive->err=drive->cmd.transport(READ,drive->rd_buf,8) ))
    {
        if (!drive->silent) sperror ("PLEXTOR_GET_AUTOSTRATEGY",drive->err);
        return drive->err;
    }
...
int plextor_set_autostrategy(drive_info* drive)
{
    ...
    if ((drive->err=drive->cmd.transport(READ,drive->rd_buf,8) ))
    {
        if (!drive->silent) sperror ("PLEXTOR_SET_AUTOSTRATEGY",drive->err);
        return drive->err;
    }
...
int plextor_get_autostrategy_db_entry_count(drive_info* drive)
{
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_GET_ASDB_ENTRY_COUNT",drive->err);
        return drive->err;
    }
...
int plextor_get_autostrategy_db(drive_info* drive)//, void* database)
{
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_GET_ASDB",drive->err);
        return drive->err;
    }
    printf("** AS DB entries: %d\n",drive->astrategy.dbcnt);
//*
...
int plextor_get_strategy(drive_info* drive) {
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_GET_STRATEGY",drive->err);
        return drive->err;
    }
    acnt= drive->rd_buf[0];
    cnt=  drive->rd_buf[6];
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_GET_STRATEGY_DATA",drive->err);
        return drive->err;
    }
...
// sending strategy headers
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_ADD_STRATEGY_HDR",drive->err);
        return drive->err;
    }
...
// sending strategy parameters
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_ADD_STRATEGY_DATA",drive->err);
        return drive->err;
    }
    return 0;
}
...
int plextor_clear_autostrategy_db(drive_info* drive) {
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_CLEAR_ASTRATEGY_DB",drive->err);
        return drive->err;
    }
    return 0;
}
...
int plextor_modify_autostrategy_db(drive_info* drive, int index, int action)
{
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_MODIFY_ASDB",drive->err);
        return drive->err;
    }
    return 0;
}
...
//	if (!drive->silent) printf("AS create: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n",		(CMD_PLEX_AS_RD) & 0xFF, 4, mode & 0xFF, 0, 0, 0, 0, 0, 0, 0, 0, 0);
    ...
    if ((drive->err=drive->cmd.transport(NONE, NULL, 0) ))
    {
        if (!drive->silent) sperror ("PLEXTOR_CREATE_STRATEGY_START",drive->err);
        return drive->err;
    }
    ...
    {
        if (!drive->silent) sperror ("PLEXTOR_AS_GET_STATUS",drive->err);
        return drive->err;
    }
...
//		(PLEXTOR_AS_RD) & 0xFF, 1, mode & 0xFF, 0, 0, 0, 0, 0, 0, 0, 0, 0);
    ...
    if ((drive->err=drive->cmd.transport(NONE, NULL, 0)))
    {
        if (!drive->silent) sperror ("PLEXTOR_MEDIA_QUALITY_CHECK_START",drive->err);
        return drive->err;
    }
...
// Waiting until Media is checked
    ...
    if(!drive->silent)
    {
        ...
        for (i=0; i<0x12; i++) printf("%02X ", drive->rd_buf[i]);
        printf("\n");
    }
    return 0;
}
