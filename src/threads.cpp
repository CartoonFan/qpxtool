...
char **add_arg(char **args, int *argc, const char *arg) {
...
int WIN32_thread_create(HANDLE *tid, void *attr, void *(*func)(void *),
                        void *arg) {
...
int createChildProcess(char **argv, pipe_t *rdpipe, pipe_t *wrpipe) {
  ...
  if (!cpid) {
    ...
    if (rdpipe)
      close((*rdpipe)[1]); // unused write end
    if (wrpipe)
      close((*wrpipe)[0]); // unused read end
  }
  return cpid;
  ...
  if (wrpipe)
    si.hStdInput = opiper;
  if (rdpipe) {
    si.hStdOutput = ipipew;
    si.hStdError = ipipew;
  }

  for (int i = 0; argv[i] != 0; i++) {
    if (i != 0)
      string_append_c(args, ' '); // don't prepend a space before the path!!!
    quoted_arg = protect_arg(argv[i]);
    string_append(args, quoted_arg);
    free(quoted_arg);
  }

  if (!CreateProcessA(NULL, args, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL,
                      NULL, &si, &pi)) {
    if (rdpipe) {
      close((*rdpipe)[0]);
      close((*rdpipe)[1]);
