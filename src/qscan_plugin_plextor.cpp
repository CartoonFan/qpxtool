...
static const int SPEEDS_ERRC_CD[] = {4 * CD_SPEED_MULT,  8 * CD_SPEED_MULT,
                                     24 * CD_SPEED_MULT, 32 * CD_SPEED_MULT,
                                     40 * CD_SPEED_MULT, 0};

static const int SPEEDS_ERRC_DVD[] = {2 * DVD_SPEED_MULT, 5 * DVD_SPEED_MULT,
                                      8 * DVD_SPEED_MULT, 12 * DVD_SPEED_MULT,
                                      0};

static const int SPEEDS_JB_CD[] = {4 * CD_SPEED_MULT, 0};

static const int SPEEDS_JB_DVD[] = {2 * DVD_SPEED_MULT, 0};

scan_plugin *plugin_create(drive_info *idev) { return new scan_plextor(idev); }
...
int scan_plextor::errc_data() {
  ...
  if (dev->media.type & DISC_CD) {
    return (ERRC_DATA_BLER | ERRC_DATA_E11 | ERRC_DATA_E21 | ERRC_DATA_E31 |
            ERRC_DATA_E12 | ERRC_DATA_E22 | ERRC_DATA_E32 | ERRC_DATA_UNCR);
  } else if (dev->media.type & DISC_DVD) {
    return (ERRC_DATA_PIE | ERRC_DATA_PIF | ERRC_DATA_POE | ERRC_DATA_POF |
            ERRC_DATA_UNCR);
  }
  return 0;
}
