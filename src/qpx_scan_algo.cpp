...
void qscanner::calc_cur_speed(long sects) {
    ...
    //	float spdX;
    btime =
        (blke.tv_sec - blks.tv_sec) + (blke.tv_usec - blks.tv_usec) / 1000000.0;
...
int qscanner::run_rd_transfer() {
    ...
    for (lba = lba_sta; (!stop_req) && !err && lba < lba_end; lba += rsize) {
        ...
        //		printf("lba=%d +%d\n",lba,rsize);
        if (lba > lba_sta &&
                (!(lba % bsize) || lba + rsize == lba_end || stat_req)) {
            gettimeofday(&blke, NULL);
            /*
            btime=(blke.tv_sec - blks.tv_sec) + (blke.tv_usec -
            blks.tv_usec)/1000000.0; spdKB = (bsize << 1) / btime; spdX  =
            spdKB/(float)spd1X;
            */
            calc_cur_speed(br);
...
int qscanner::run_wr_transfer() {
    ...
    if (dev->media.type & (DISC_CD)) {
        ...
        printf(
            "Internal write transfer rate test not implemented for mounted media!");
        return -1;
    }
    //	lba_end = 4096;
    ...
    for (lba = lba_sta; !stop_req && !err && lba < lba_end; lba += wsize) {
        ...
        if (!(lba % bsize) || lba + wsize == lba_end || stat_req || stop_req ||
                !wsize) {
...
int qscanner::run_cd_errc() {
    ...
    printf("\nTesting %ld sectors: %ld - %ld\n", lba_end - lba_sta + 1, lba_sta,
           lba_end);
    printf("         lba |        speed        |  BLER |  E11   E21   E31  |  "
           "E12   E22   E32  |  UNCR\n");
    for (; (!stop_req) && lba < lba_end;) {
        ...
        printf("cur : %6ld | %6.2f X %5d kB/s | %5ld | %5ld %5ld %5ld | %5ld %5ld "
               "%5ld | %5ld\r",
               lba, spdX, spdKB, err.bler, err.e11, err.e21, err.e31, err.e12,
               err.e22, err.e32, err.uncr);
        err_tot += err;
        err_max.EMAX(err);
#ifdef USE_FFLUSH
        fflush(stdout);
    ...
    printf("tot : %5ld | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_tot.bler,
           err_tot.e11, err_tot.e21, err_tot.e31, err_tot.e21, err_tot.e22,
           err_tot.e32, err_tot.uncr);
    printf("max : %5ld | %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_max.bler,
           err_max.e11, err_max.e21, err_max.e31, err_max.e21, err_max.e22,
           err_max.e32, err_max.uncr);
    printf("avg : %5.2f | %5.2f %5.2f %5.2f | %5.2f %5.2f %5.2f | %5.2f\n",
           err_tot.bler / (float)lba, err_tot.e11 / (float)lba,
           err_tot.e21 / (float)lba, err_tot.e31 / (float)lba,
           err_tot.e21 / (float)lba, err_tot.e22 / (float)lba,
           err_tot.e32 / (float)lba, err_tot.uncr / (float)lba);
#ifdef USE_FFLUSH
    fflush(stdout);
#endif
    return 0;
...
int qscanner::run_cd_jb() {
    ...
    for (; (!stop_req) && lba < lba_end;) {
        ...
        gettimeofday(&blke, NULL);
        calc_cur_speed(lba - lbao);
        printf("cur : %6ld | %6.2f X %5d kB/s | %6.2f | %6.2f\r", lba, spdX, spdKB,
               jb.jitter / 1000.0, jb.asymm / 10.0);
        jb_min.EMIN(jb);
        jb_max.EMAX(jb);
#ifdef USE_FFLUSH
        fflush(stdout);
    ...
    printf("         max : %6.2f | %6.2f\n", jb_max.jitter / 100.0,
           jb_max.asymm / 10.0);
//    printf("         avg : %6.2f | %6.2f", lba, jb_max.jitter/100.0,
//    jb_max.asymm/10.0);
#ifdef USE_FFLUSH
    fflush(stdout);
#endif
...
int qscanner::run_dvd_errc() {
    ...
    printf("\nTesting %ld sectors: %ld - %ld\n", lba_end - lba_sta + 1, lba_sta,
           lba_end);
    printf("          lba |        speed        |  PIE   PI8   PIF  |  POE   PO8 "
           "  POF  |  UNCR\n");
    for (; (!stop_req) && lba < lba_end;) {
        lbao = lba;
        gettimeofday(&blks, NULL);
        ...
        if ((lba - lbas) >= 0x80) {
            ...
            if (po8_max < po8)
                po8_max = po8;
            printf("cur : %7ld | %6.2f X %5d kB/s | %5ld %5ld %5ld | %5ld %5ld %5ld "
                   "| %5ld\r",
                   lba, spdX, spdKB, err.pie, pi8, err.pif, err.poe, po8, err.pof,
                   err.uncr);
            pi8 = 0;
            po8 = 0;
            lbas = lba;
            ...
            printf("cur : %7ld | %6.2f X %5d kB/s | %5ld %5ld %5ld | %5ld %5ld %5ld "
                   "| %5ld\r",
                   lba, spdX, spdKB, err.pie, (long)-1, err.pif, err.poe, (long)-1,
                   err.pof, err.uncr);
        }
#ifdef USE_FFLUSH
        fflush(stdout);
    ...
    printf("tot : %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_tot.pie,
           err_tot.pie, err_tot.pif, err_tot.poe, err_tot.poe, err_tot.pof,
           err_tot.uncr);
    printf("max : %5ld %5ld %5ld | %5ld %5ld %5ld | %5ld\n", err_max.pie, pi8_max,
           err_max.pif, err_max.poe, po8_max, err_max.pof, err_max.uncr);
    printf("avg : %5.2f %5.2f %5.2f | %5.2f %5.2f %5.2f | %5.2f\n",
           err_tot.pie / (float)(lba >> 4), err_tot.pie / (float)(lba >> 7),
           err_tot.pif / (float)(lba >> 4), err_tot.poe / (float)(lba >> 4),
           err_tot.poe / (float)(lba >> 7), err_tot.pof / (float)(lba >> 4),
...
int qscanner::run_bd_errc() {
    ...
    for (; (!stop_req) && lba < lba_end;) {
        ...
        calc_cur_speed(lba - lbao);
        //		if (!(lba & 0x7F)) {
        printf("cur : %7ld | %6.2f X %5d kB/s | %5ld %5ld | %5ld\r", lba, spdX,
               spdKB, err.ldc, err.bis, err.uncr);
#ifdef USE_FFLUSH
        fflush(stdout);
#endif
    }
    ...
    printf("Test summary:\n");
    printf("       LDC   BIS  |  UNCR\n");
    printf("tot : %5ld %5ld | %5ld\n", err_tot.ldc, err_tot.bis, err_tot.uncr);
    printf("max : %5ld %5ld | %5ld\n", err_max.ldc, err_max.bis, err_max.uncr);
    printf("avg : %5.2f %5.2f | %5.2f\n", err_tot.ldc / (float)(lba >> 4),
           err_tot.bis / (float)(lba >> 4), err_tot.uncr / (float)(lba >> 4));
#ifdef USE_FFLUSH
    fflush(stdout);
#endif
    return 0;
