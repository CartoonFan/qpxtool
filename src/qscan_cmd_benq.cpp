...
int scan_benq::cmd_check_mode_init() {
    ...
    if ((dev->err = dev->cmd.transport(NONE, NULL, 0))) {
        if (!dev->silent)
            sperror("benq_check_mode_init", dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_benq::cmd_check_mode_exit() {
    ...
    if ((dev->err = dev->cmd.transport(NONE, NULL, 0))) {
        if (!dev->silent)
            sperror("benq_check_mode_exit", dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_benq::cmd_get_result() {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 2))) {
        if (!dev->silent)
            sperror("BENQ_CHECK_RESULT", dev->err);
        return dev->err;
    }
...
int scan_benq::cmd_set_speed(unsigned char sidx) {
    ...
    if ((dev->err = dev->cmd.transport(WRITE, dev->rd_buf, 4))) {
        if (!dev->silent)
            sperror("BENQ_SET_ERRC_SPEED", dev->err);
        return dev->err;
    }
...
int scan_benq::cmd_start_errc(int lba) {
    ...
    if ((dev->err = dev->cmd.transport(WRITE, dev->rd_buf, 6))) {
        if (!dev->silent)
            sperror("BENQ_ERRC_SEEK", dev->err);
        return dev->err;
    }
    return (cmd_get_result());
}
...
int scan_benq::cmd_start_fete(int lba) {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 8))) {
        if (!dev->silent)
            sperror("BENQ_START_FETE", dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_benq::cmd_cd_errc_init(int &speed) {
    ...
    if ((dev->err = dev->cmd.transport(WRITE, dev->rd_buf, 10))) {
        if (!dev->silent)
            sperror("benq_init_cx_scan_3", dev->err);
        return dev->err;
    }
    cmd_get_result();
...
int scan_benq::cmd_dvd_errc_init(int &speed) {
    ...
    if ((dev->err = dev->cmd.transport(WRITE, dev->rd_buf, 10))) {
        if (!dev->silent)
            sperror("benq_init_pi_scan_3", dev->err);
        return dev->err;
    }
    if (cmd_get_result())
        return 1;
...
int scan_benq::cmd_dvd_fete_init(int &speed) {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 8))) {
        if (!dev->silent)
            sperror("benq_fete_init_1", dev->err);
        return dev->err;
    }
...
int scan_benq::cmd_cd_errc_block(cd_errc *data) {
    ...
    do {
        cmd_read_block();
        cmd_getdata();
        if ((!dev->rd_buf[0]) && (dev->rd_buf[1] == 0x63) &&
                (dev->rd_buf[2] == 0x64) && (dev->rd_buf[3] == 0x6E)) {
            found = 1;
#ifdef _BENQ_DEBUG2
            printf("\nData block found...\n");
        } else {
...
/*
int benq_jitter_CD_do_one_interval(dev_info* dev, int* lba, int* jitter, short
int* beta, int blklen)
{
        benq_scan_block(dev);
        benq_read_err(dev);
        ...
        *jitter = 0;
        *beta = 0;
//	*lba=((int)dev->rd_buf[0] * 4500 + (int)dev->rd_buf[3] * 75 +
(int)dev->rd_buf[2]); *lba = (((dev->rd_buf[7] & 0xF0)*10 + (dev->rd_buf[7] &
0x0F))*60 +  (dev->rd_buf[7] & 0xF0))*10 + (dev->rd_buf[7] & 0x0F); return 0;
}
*/
...
//	int pif;
    ...
    do {
        cmd_read_block();
        cmd_getdata();
        if ((!dev->rd_buf[0]) && (dev->rd_buf[1] == 0x64) &&
                (dev->rd_buf[2] == 0x76) && (dev->rd_buf[3] == 0x64)) {
            found = 1;
#ifdef _BENQ_DEBUG2
            printf("\nData block found...\n");
        } else {
...
//			return 0;
    ...
    data->pie = ntoh16(dev->rd_buf + 0x0C) + ntoh16(dev->rd_buf + 0x0E) +
                ntoh16(dev->rd_buf + 0x10) + ntoh16(dev->rd_buf + 0x12) +
                ntoh16(dev->rd_buf + 0x14);
    data->pif = ntoh16(dev->rd_buf + 0x16);
    data->poe =
        //		+ntoh16(dev->rd_buf+0x18)
        +ntoh16(dev->rd_buf + 0x1A) + ntoh16(dev->rd_buf + 0x1C) +
        ntoh16(dev->rd_buf + 0x1E) + ntoh16(dev->rd_buf + 0x20) +
        ntoh16(dev->rd_buf + 0x22);
    data->pof = ntoh16(dev->rd_buf + 0x38);
    plba = lba;
    lba = (((dev->rd_buf[7] - 3) << 16) & 0xFF0000) +
          ((dev->rd_buf[8] << 8) & 0xFF00) + (dev->rd_buf[9] & 0xFF);
...
int scan_benq::cmd_dvd_fete_block(cdvd_ft *data) {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 8))) {
        if (!dev->silent)
            sperror("benq_fete_block", dev->err);
        return dev->err;
    }
