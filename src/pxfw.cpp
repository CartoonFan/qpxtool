...
int op_blacklisted(unsigned char opcode) {
    if (op_blacklist_sz)
        for (int i = 0; i < op_blacklist_sz; i++) {
            if (op_blacklist[i] == opcode)
                return 1;
        }
    return 0;
}
...
void FW_convert_to_ID(int *dev_ID, char *buf, int *FWSZ_CRC) {
    *FWSZ_CRC = 0;
    if (!strncmp(buf, "PLEXTOR CD-R   PX-W4824A", 24)) {
        *dev_ID = PLEXTOR_4824;
        *FWSZ_CRC = 0x79FFE;
    } else if (!strncmp(buf, "PLEXTOR CD-R   PX-W5224A", 24)) {
        *dev_ID = PLEXTOR_5224;
        *FWSZ_CRC = 0x79FFE;
    } else if (!strncmp(buf, "PLEXTOR CD-R   PREMIUM2", 23)) {
        *dev_ID = PLEXTOR_PREMIUM2;
        *FWSZ_CRC = 0xEFFFE;
    } else if (!strncmp(buf, "PLEXTOR CD-R   PREMIUM", 22)) {
        *dev_ID = PLEXTOR_PREMIUM;
        *FWSZ_CRC = 0x7C7FE;
    } else if (!strncmp(buf, "PLEXTOR DVDR   PX-708A", 22)) {
        *dev_ID = PLEXTOR_708;
        *FWSZ_CRC = 0xEFFFE;
    } else if (!strncmp(buf, "PLEXTOR DVDR   PX-712A", 22)) {
        *dev_ID = PLEXTOR_712 | PLEXTOR_708A2;
        *FWSZ_CRC = 0xEFFFE;
        //		*FWSZ_CRC=0xFAD9E;
        //		*FWSZ_CRC=0x0F0000;
    } else if (!strncmp(buf, "PLEXTOR DVDR   PX-716A ", 23)) {
        *dev_ID = PLEXTOR_716 | PLEXTOR_714;
        *FWSZ_CRC = 0xEFFFE;
    } else if (!strncmp(buf, "PLEXTOR DVDR   PX-716AL", 23)) {
        *dev_ID = PLEXTOR_716AL;
        *FWSZ_CRC = 0xEFFFE;
    } else if (!strncmp(buf, "PLEXTOR DVDR   PX-760A", 22)) {
    ...
    *FWSZ = 0;
    if (!strncmp(dev->ven, "PLEXTOR ", 8)) {
        ...
        if (!strncmp(dev->dev, "CD-R   PX-W4824A", 16)) {
            ...
            *FWSZ = 524288;
        } else if (!strncmp(dev->dev, "CD-R   PX-W5224A", 16)) {
            dev->dev_ID = PLEXTOR_5224;
            *FWSZ = 524288;
        } else if (!strncmp(dev->dev, "CD-R   PREMIUM ", 15)) {
            dev->dev_ID = PLEXTOR_PREMIUM;
            *FWSZ = 524288;
        } else if (!strncmp(dev->dev, "CD-R   PREMIUM2", 15)) {
            dev->dev_ID = PLEXTOR_PREMIUM2;
            *FWSZ = 983040;
        } else if (!strncmp(dev->dev, "DVDR   PX-708A ", 15)) {
            dev->dev_ID = PLEXTOR_708;
            *FWSZ = 983040;
        } else if (!strncmp(dev->dev, "DVDR   PX-708A2", 15)) {
            dev->dev_ID = PLEXTOR_708A2;
            *FWSZ = 1028096;
        } else if (!strncmp(dev->dev, "DVDR   PX-712A", 14)) {
            dev->dev_ID = PLEXTOR_712;
            *FWSZ = 1028096;
        } else if (!strncmp(dev->dev, "DVDR   PX-714A", 14)) {
            dev->dev_ID = PLEXTOR_714;
            *FWSZ = 983040;
        } else if (!strncmp(dev->dev, "DVDR   PX-716A ", 15)) {
            dev->dev_ID = PLEXTOR_716;
            *FWSZ = 983040;
        } else if (!strncmp(dev->dev, "DVDR   PX-716AL", 15)) {
            dev->dev_ID = PLEXTOR_716AL;
            *FWSZ = 983040;
        } else if (!strncmp(dev->dev, "DVDR   PX-755A", 14)) {
            dev->dev_ID = PLEXTOR_755;
            *FWSZ = 2031616;
        } else if (!strncmp(dev->dev, "DVDR   PX-760A", 14)) {
...
void usage(char *bin) {
    ...
    printf("\t-f\t\t force flashing, even if device not recognized or checksum "
           "error\n");
...
int main(int argc, char *argv[]) {
    ...
    printf("**  Plextor FirmWare updater v%s (c) 2006-2009  Gennady \"ShultZ\" "
           "Kozlov **\n",
           VERSION);
    /*
        if (argc<2) {
    ...
    */
        ...
        for (unsigned int opcode = 0x0000; opcode < 0x0100; opcode++) {
            ...
            if (!op_blacklisted((unsigned char)opcode)) {
                ...
                if (!err)
                    ...
                    printf("command didn't passed to dev");
                else if (err == 0x52000)
    ...
    for (i = 0; i < (FB / HH); i++) {
        printf("| ");
        for (j = 0; j < HH; j++)
            printf("%02X ", dev->rd_buf[i * HH + j] & 0xFF);
        printf(" | ");
        for (j = 0; j < HH; j++)
...
//		printf ("%02X:  %4dB CRC=%04X\n",i,blen,CRC16blk);
    ...
    */
    ...
    if (flags & FL_UPDATE) {
        ...
        if (CSUM_diff) {
            if (!(flags & FL_FORCE)) {
                printf("*** CheckSum incorrect, you can try -f option AT YOUR RISC to "
                       "Force flashing ...\n");
                delete dev;
                fclose(fwfile);
                exit(3);
                ...
                printf(
                    "*** CheckSum incorrect, but -f option found. Force flashing...\n");
            }
        }
