...
void qscanner::setTestSpeed(int ispeed) {
    speed = ispeed;
}
    ...
    if (dev->media.type & DISC_DVDplus) {
        if (isPlextor(dev)) {
            if (isPlextorLockPresent(dev) || !plextor_px755_do_auth(dev)) {
                ...
                printf(
                    "Found locked PLEXTOR drive. Can't handle DVD+R(W) TestWrite!\n");
                return 1;
            }
        } else {
            if (simul) {
        ...
        if ((dev->media.type & DISC_CD) &&
                (dev->capabilities & CAP_TEST_WRITE_CD)) {
            printf("Turning TestWrite (CD) %s\n", simul ? "ON" : "OFF");
            WT_simul = simul;
            return 0;
        } else if ((dev->media.type & DISC_DVDminus) &&
                   (dev->capabilities & CAP_TEST_WRITE_DVD)) {
...
int qscanner::run(char *test) {
    ...
    //	set_speed(dev,speed);
    if (!strcmp(test, "rt")) {
        if (lba_end < 0 || lba_end > dev->media.capacity)
            lba_end = dev->media.capacity - 1;
        r = run_rd_transfer();
    } else if (!strcmp(test, "wt")) {
        if (lba_end < 0 || lba_end > dev->media.capacity_total)
            lba_end = dev->media.capacity_total - 1;
        r = run_wr_transfer();
...
int qscanner::plugin_attach_fallback() {
    return plugin_attach(FALLBACK_PLUGIN_NAME);
}
...
int qscanner::plugin_attach(char *pname, bool probe_enable, bool no_detach,
                            bool silent) {
    bool blacklisted = 0;
    drivedesc *devlist;
    if (attached)
...
//	if (!dev->silent) printf("pluginlib = %p\n", pluginlib);
...
#endif
    if (!dev->silent)
        printf("plugin lib opened: %s\n", pname);
    plugin_create =
        (scan_plugin * (*)(drive_info *)) dlsym(pluginlib, "plugin_create");
#ifndef _WIN32
    if (dlerror()) {
#else
    if (!plugin_create) {
    ...
    */
    ...
    if (!probe_enable && plugin->devlist) {
        ...
        for (int d = 0; !listed && devlist[d].ven_ID > 0 && devlist[d].dev_ID > 0;
                d++) {
            if (!strncmp(dev->ven, devlist[d].ven, strlen(devlist[d].ven)) &&
                    !strncmp(dev->dev, devlist[d].dev, strlen(devlist[d].dev))) {
    ...
    if (!no_detach && !listed &&
            (!probe_enable || plugin->probe_drive() == DEV_FAIL)) {
        if (!dev->silent) {
            if (probe_enable)
