...
#include "csstables.hpp"

#define DVD_DISCKEY_SIZE 2048

// device commands
int report_key(drive_info *drive, unsigned char key_class,
               unsigned char key_format, int len, unsigned int lba = 0);
...
static int css_RecoverTitleKey(int i_start, unsigned char const *p_crypted,
                               ...
                               unsigned char const *p_sector_seed,
                               unsigned char *p_key);
static void css_DecryptTitleKey(dvd_key_t p_disc_key, dvd_key_t p_titlekey);
static int css_CrackTitleKey(drive_info *, int i_pos, int i_len,
                             dvd_key_t p_titlekey);
static int css_AttackPattern(unsigned char const p_sec[DVDCSS_BLOCK_SIZE],
                             int i_pos, unsigned char *p_key);
...
int read_dvd(drive_info *drive, unsigned char *data, int lba, int sector_count,
             int flags)
...
//                                          int i_flags )
{
    ...
    if (!read(drive, data, lba, sector_count)) {
        ...
        if (drive->err == 0x52100)
            i_ret = 0;
    }

    if (i_ret <= 0 || drive->media.dvdcss.protection != 0x01 ||
...
int read_disc_regions(drive_info *drive) {
    ...
    if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, len))) {
        if (!drive->silent)
            sperror("READ_DISC_REGIONS", drive->err);
        return drive->err;
    }
#if 0
    if (!drive->silent) {
...
int read_disc_key(drive_info *drive, unsigned char DK[DVD_DISCKEY_SIZE]) {
    ...
    if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, len))) {
        if (!drive->silent)
            sperror("READ_DISC_KEY", drive->err);
        return drive->err;
    }
...
int report_key(drive_info *drive, unsigned char key_class,
               unsigned char key_format, int len, unsigned int lba) {
    if (!(drive->rd_capabilities & DEVICE_DVD) ||
            !(drive->capabilities & CAP_DVD_CSS))
        return -1;
    ...
    if ((drive->err = drive->cmd.transport(READ, drive->rd_buf, len))) {
        if (!drive->silent)
            sperror("MMC REPORT KEY", drive->err);
        return drive->err;
    }
    return 0;
}
    ...
    */
    ...
    if (drive->err) {
        if (drive->err == 0x52400) {
            ...
            if (!drive->silent)
                sperror("READ_RPC_STATE", drive->err);
            drive->rpc.phase = 0;
            return drive->err;
        }
    }
...
int css_invalidate_agid(drive_info *drive) {
    if (!(drive->capabilities & CAP_DVD_CSS))
        return -1;
    ...
    if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
        if (!drive->silent)
            sperror("MMC REPORT KEY (INVALIDATE AGID)", drive->err);
        return drive->err;
    }
    return 0;
}
...
int css_report_challenge(drive_info *drive) {
    ...
    printf(
        "Report Challenge: %02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X\n",
...
int css_send_challenge(drive_info *drive) {
    ...
    drive->cmd[9] = len & 0xFF;
    drive->cmd[10] = (drive->media.dvdcss.agid & 0xC0) |
                     0x01; // key format = 1 : Challenge key
    drive->cmd[11] = 0;
    ...
    if ((drive->err = drive->cmd.transport(WRITE, drive->rd_buf, len))) {
        if (!drive->silent)
            sperror("MMC SEND KEY (CHALLENGE)", drive->err);
        return drive->err;
    }
...
int css_report_key1(drive_info *drive) {
    ...
    printf("Report KEY1: %02X:%02X:%02X:%02X:%02X\n", drive->media.dvdcss.K1[0],
           drive->media.dvdcss.K1[1], drive->media.dvdcss.K1[2],
           drive->media.dvdcss.K1[3], drive->media.dvdcss.K1[4]);
    return 0;
}
...
int css_send_key2(drive_info *drive) {
    ...
    printf("Send KEY2: %02X:%02X:%02X:%02X:%02X\n", drive->media.dvdcss.K2[0],
           drive->media.dvdcss.K2[1], drive->media.dvdcss.K2[2],
           drive->media.dvdcss.K2[3], drive->media.dvdcss.K2[4]);
    ...
    drive->cmd[9] = len & 0xFF;
    drive->cmd[10] =
        (drive->media.dvdcss.agid & 0xC0) | 0x03; // key format = 2 : KEY 2
    drive->cmd[11] = 0;
    ...
    if ((drive->err = drive->cmd.transport(WRITE, drive->rd_buf, len))) {
        if (!drive->silent)
            sperror("MMC SEND KEY (KEY2)", drive->err);
        return drive->err;
    }
...
int css_unscramble(dvd_key_t p_key, unsigned char *p_sec) {
    ...
    i_t3 = (p_key[2] | (p_key[3] << 8) | (p_key[4] << 16)) ^
           (p_sec[0x56] | (p_sec[0x57] << 8) | (p_sec[0x58] << 16));
    i_t4 = i_t3 & 7;
    i_t3 = i_t3 * 2 + 8 - i_t4;
    p_sec += 0x80;
    i_t5 = 0;
...
int css_get_bus_key(drive_info *drive) {
    ...
    memcpy(drive->media.dvdcss.CK + DVD_KEY_SIZE, drive->media.dvdcss.K2,
           DVD_KEY_SIZE);
...
static void css_CryptKey(int i_key_type, int i_variant,
    ...
    unsigned char pp_perm_challenge[3][10] = {{1, 3, 0, 7, 5, 2, 9, 6, 4, 8},
        {6, 1, 9, 3, 8, 5, 7, 4, 0, 2},
        {4, 0, 3, 5, 7, 2, 8, 6, 1, 9}
    };
    ...
    unsigned char pp_perm_variant[2][32] = {
        {   0x0a, 0x08, 0x0e, 0x0c, 0x0b, 0x09, 0x0f, 0x0d, 0x1a, 0x18, 0x1e,
            0x1c, 0x1b, 0x19, 0x1f, 0x1d, 0x02, 0x00, 0x06, 0x04, 0x03, 0x01,
            0x07, 0x05, 0x12, 0x10, 0x16, 0x14, 0x13, 0x11, 0x17, 0x15
        },
        {   0x12, 0x1a, 0x16, 0x1e, 0x02, 0x0a, 0x06, 0x0e, 0x10, 0x18, 0x14,
    ...
    unsigned char p_variants[32] = {
        0xB7, 0x74, 0x85, 0xD0, 0xCC, 0xDB, 0xCA, 0x73, 0x03, 0xFE, 0x31,
     ...
     */
    i_lfsr0 = (p_tmp1[0] << 17) | (p_tmp1[1] << 9) | ((p_tmp1[2] & ~7) << 1) | 8 |
              (p_tmp1[2] & 7);
    ...
    do {
        for (i_bit = 0, i_val = 0; i_bit < 8; ++i_bit) {
            ...
            i_lfsr0_o = ((i_lfsr0 >> 24) ^ (i_lfsr0 >> 21) ^ (i_lfsr0 >> 20) ^
                         (i_lfsr0 >> 12)) &
                        1;
...
static void css_DecryptKey(unsigned char invert, unsigned char const *p_key,
    ...
    i_lfsr0 = ((p_key[4] << 17) | (p_key[3] << 9) | (p_key[2] << 1)) + 8 -
              (p_key[2] & 7);
    ...
    for (i = 0; i < DVD_KEY_SIZE; ++i) {
        ...
        o_lfsr0 =
            (((((((i_lfsr0 >> 8) ^ i_lfsr0) >> 1) ^ i_lfsr0) >> 3) ^ i_lfsr0) >> 7);
...
static const dvd_key_t player_keys[] = {
    ...
    {0x2c, 0xb2, 0xc1, 0x09, 0xee}, {0x2f, 0x25, 0x9e, 0x96, 0xdd},
    {0x33, 0x2f, 0x49, 0x6c, 0xe0}, {0x35, 0x5b, 0xc1, 0x31, 0x0f},
    {0x36, 0x67, 0xb2, 0xe3, 0x85}, {0x39, 0x3d, 0xf1, 0xf1, 0xbd},
    {0x3b, 0x31, 0x34, 0x0d, 0x91}, {0x45, 0xed, 0x28, 0xeb, 0xd3},
    {0x48, 0xb7, 0x6c, 0xce, 0x69}, {0x4b, 0x65, 0x0d, 0xc1, 0xee},
    {0x4c, 0xbb, 0xf5, 0x5b, 0x23}, {0x51, 0x67, 0x67, 0xc5, 0xe0},
    {0x53, 0x94, 0xe1, 0x75, 0xbf}, {0x57, 0x2c, 0x8b, 0x31, 0xae},
    {0x63, 0xdb, 0x4c, 0x5b, 0x4a}, {0x7b, 0x1e, 0x5e, 0x2b, 0x57},
    {0x85, 0xf3, 0x85, 0xa0, 0xe0}, {0xab, 0x1e, 0xe7, 0x7b, 0x72},
    {0xab, 0x36, 0xe3, 0xeb, 0x76}, {0xb1, 0xb8, 0xf9, 0x38, 0x03},
    {0xb8, 0x5d, 0xd8, 0x53, 0xbd}, {0xbf, 0x92, 0xc3, 0xb0, 0xe2},
    {0xcf, 0x1a, 0xb2, 0xf8, 0x0a}, {0xec, 0xa0, 0xcf, 0xb3, 0xff},
 ...
 *****************************************************************************/
static int css_DecryptDiscKey(drive_info *drive,
                              unsigned char const *p_struct_disckey,
    ...
    for (n = 0; n < sizeof(player_keys) / sizeof(dvd_key_t); n++) {
        ...
        for (i = 1; i < 409; i++) {
            ...
            css_DecryptKey(0, player_keys[n], p_struct_disckey + 5 * i, p_disc_key);
        ...
        if (i == 10) {
            ...
            for (i = 0; i < 4; i++) {
                ...
                for (j = 0; j < 256; j++) {
                    ...
                    i_t6 = (((((((i_t3 >> 3) ^ i_t3) >> 1) ^ i_t3) >> 8) ^ i_t3) >> 5) &
                           0xff;
                    if (i_t6 == i_t1) {
                        break;
                    }
...
static int css_AttackPattern(unsigned char const p_sec[DVDCSS_BLOCK_SIZE],
    ...
    for (i = 2; i < 0x30; i++) {
        ...
        for (j = i + 1; j < 0x80 && (p_sec[0x7F - (j % i)] == p_sec[0x7F - j]);
                j++) {
    ...
    if ((i_best_plen > 3) && (i_best_plen / i_best_p >= 2)) {
        ...
        res = css_RecoverTitleKey(
                  0, &p_sec[0x80], &p_sec[0x80 - (i_best_plen / i_best_p) * i_best_p],
...
int css_title(drive_info *drive, int lba) {
    ...
    if (p_title != NULL && p_title->i_startlb == lba) {
 ...
 *****************************************************************************/
int css_titlekey(drive_info *drive, int lba, dvd_key_t p_title_key) {
    //    static unsigned char p_garbage[ DVDCSS_BLOCK_SIZE ];  /* we never read
    //    it back */
    unsigned char p_key[DVD_KEY_SIZE];
    int i, i_ret = 0;

    if (drive->media.dvdcss.method == DVDCSS_METHOD_KEY ||
            drive->media.dvdcss.method == DVDCSS_METHOD_DISC) {
