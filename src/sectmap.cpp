...
int smap::is_done() { return (sectors - get_done()); }
...
unsigned int smap::get_next(unsigned int *lba, mape state, int *count) {
  ...
  //	printf("smap::get_next(%x,%d,%d)\n",lba ? *lba : -1,state, count ?
  //*count : -1);

  if (lba)
    offs = *lba;
  if (icount < 2) {
    while ((get(offs) != state) && (offs < sectors))
      offs++;
  } else {
...
void smap::fill(mape state) {
  for (unsigned int i = 0; i < blocks; i++)
    for (unsigned int j = 0; j < map_block_sz; j++)
      arr[i][j] = state;
}
...
int smap::load() {
  ...
  for (unsigned i = 0; (i < blocks) && (!feof(f)); i++) {
    ...
    for (unsigned j = 0;
         (j < map_block_sz) && (i * map_block_sz + j) < (s / sizeof(mape)); j++)
