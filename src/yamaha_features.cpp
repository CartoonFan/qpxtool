...
//	int	i;
    ...
    */
    ...
    offs=0;
    while (((drive->rd_buf[offs]) & 0x3F) != MODE_PAGE_WRITE_PARAMETERS) offs++;
...
int yamaha_check_forcespeed(drive_info* drive) {
    ...
    offs=0;
    while (((drive->rd_buf[offs]) & 0x3F) != MODE_PAGE_MM_CAP_STATUS) offs++;
...
int yamaha_f1_get_tattoo(drive_info* drive) {
    ...
    if (ilen < 22) return 1;
    offs=0;
    while (((drive->rd_buf[offs]) & 0x3F) != MODE_PAGE_YAMAHA_TATTOO) offs++;
    /*
    	unsigned char len=0;
    	int	r;
    	int	i;
...
//	drive->cmd[2] = 0;
    ...
    {
        if (!drive->silent) sperror ("YAMAHA_WRITE", drive->err);
        return (drive->err);
    }
    return 0;
}
...
int yamaha_set_tattoo_speed(drive_info* drive) {
    ...
    if ((drive->err=drive->cmd.transport(NONE,NULL,0) ))
    {
        if (!drive->silent) sperror ("YAMAHA_SET_SPEED",drive->err);
        return (drive->err);
    }
    return 0;
}
