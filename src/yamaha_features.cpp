...
int yamaha_check_amqr(drive_info *drive) {
  ...
  */
  ...
  offs = 0;
  while (((drive->rd_buf[offs]) & 0x3F) != MODE_PAGE_WRITE_PARAMETERS)
    offs++;
  /*	len=drive->rd_buf[offs+1];

      printf("Page:\n");
      for (i=0; i<(len+2); i++) { if (!(i%8)) printf("\n"); printf("
     %02X",drive->rd_buf[i+8] & 0xFF); } printf("\n");
  */
...
int yamaha_set_amqr(drive_info *drive) { return 1; }
...
int yamaha_check_forcespeed(drive_info *drive) {
  ...
  offs = 0;
  while (((drive->rd_buf[offs]) & 0x3F) != MODE_PAGE_MM_CAP_STATUS)
    offs++;
...
int yamaha_set_forcespeed(drive_info *drive) {
  ...
  if (drive->parms.read_speed_kb)
    speed = drive->parms.read_speed_kb;
...
int yamaha_f1_get_tattoo(drive_info *drive) {
  ...
  if (ilen < 22)
    return 1;
  offs = 0;
  while (((drive->rd_buf[offs]) & 0x3F) != MODE_PAGE_YAMAHA_TATTOO)
    offs++;
  /*
      unsigned char len=0;
      int	r;
     ...
     %02X",drive->rd_buf[i+8] & 0xFF); } printf("\n");
  */
  drive->yamaha.tattoo_i = (drive->rd_buf[offs + 4] << 16) |
                           (drive->rd_buf[offs + 5] << 8) |
                           drive->rd_buf[offs + 6];
  drive->yamaha.tattoo_o = (drive->rd_buf[offs + 7] << 16) |
                           (drive->rd_buf[offs + 8] << 8) |
                           drive->rd_buf[offs + 9];
...
int yamaha_write(drive_info *drive, char mode, char bufid, int offs, int plen) {
  ...
  if ((drive->err = drive->cmd.transport(WRITE, drive->rd_buf, plen << 11))) {
    if (!drive->silent)
      sperror("YAMAHA_WRITE", drive->err);
    return (drive->err);
  }
  return 0;
}
...
int yamaha_set_tattoo_speed(drive_info *drive) {
  ...
  if ((drive->err = drive->cmd.transport(NONE, NULL, 0))) {
    if (!drive->silent)
      sperror("YAMAHA_SET_SPEED", drive->err);
    return (drive->err);
  }
  return 0;
}

int yamaha_f1_do_tattoo(drive_info *drive, unsigned char *iimage,
                        uint32_t bsize) {
  ...
  if (iimage) {
    ...
    printf(
        "yamaha_f1_do_tattoo(): got no image buffer! writing test image...\n");
    ...
    for (i = 0; i < drive->yamaha.tattoo_rows; i++) {
      for (j = 0; j < sizeof(tattoo_row); j++) {
        if (j < 20)
          imagec[i * 3744 + j] = 0xFF;
        else if ((j > 1000) && (j < 1040))
          imagec[i * 3744 + j] = 0xFF;
        else
          imagec[i * 3744 + j] = 0;
        //				image[i][j]=0xFF;
  ...
  while (i < tattoo_size) {
    ...
    //		printf("block %d\n",i);
    if (!i) {
      if (yamaha_write(drive, 1, 0, drive->yamaha.tattoo_i, maxbuf / 2048))
        goto tattoo_err;
    } else {
      //	blen = (blen+2047) / 2048 * 2048;
      //	yamaha_write(drive, 1, 0, 0, 10, blen);
      if (yamaha_write(drive, 1, 0, 0, maxbuf / 2048))
        goto tattoo_err;
    }
    //		b++;
    printf(".");
  ...
  if (yamaha_write(drive, 1, 0, drive->yamaha.tattoo_o, 0))
    goto tattoo_err;
  printf(".\n");

  printf("Burning T@2...\n");
  //	printf("\nwait_unit_ready()...\n");
  if (wait_unit_ready(drive, 1000))
    printf("Error %05X...\n", drive->err);
  //	printf("\nwait_fix()...\n");
  if (wait_fix(drive, 1000))
    printf("Error %05X...\n", drive->err);
