...
void show_available_errc_data(qscanner *scanner) {
  ...
  if (dev->media.type & DISC_CD) {
    for (int i = 0; i < 8; i++)
      if (errc_data & (1 << i))
        printf(" %s", errc_names_cd[i]);
  } else if (dev->media.type & DISC_DVD) {
    for (int i = 0; i < 8; i++)
      if (errc_data & (1 << i))
        printf(" %s", errc_names_dvd[i]);
  } else if (dev->media.type & DISC_BD) {
    for (int i = 0; i < 8; i++)
      if (errc_data & (1 << i))
        printf(" %s", errc_names_bd[i]);
  }
  printf("\n");
}
  ...
  */
  ...
  while (1) {
    ...
    case 'h':
      ...
      printf("-s --speed #        test media at given speed (default: "
             "maximum). Don't use with -r or -w\n");
      printf("-S --speeds         detect available read/write speeds\n");
      printf("-r --rspeed #       set read speed\n");
      printf("-w --wspeed #       set write speed\n");
      printf("-l --scanbus        scan IDE/SATA/SCSI buses for MMC-compliant "
             "devices\n");
      ...
      printf("-W --write          do real write instead simulation (for use "
             "with --test wt)\n");
      printf("-p --plugins        list all available plugins\n");
      printf("-f --force PLUGIN   force using specified plugin (default: "
             "autodetect)\n");
      printf("-I --shortinfo      print device info\n");
      printf("-i --info           print device info (with supported features "
             "list)\n");
      printf("-m --media          print media info\n");
      printf("-M --mediashort     print media info without raw media "
             "identification data\n");
  ...
  if (dev->wr_capabilities) {
    ...
    for (int i = 0; wr_modes[i].id; i++) {
      if ((dev->wr_modes & wr_modes[i].id) == wr_modes[i].id)
        printf(" %s", wr_modes[i].name);
    }
    printf("\n");
  }
  ...
  if (flags & FL_DINFO) {
    printf("\nDevice capabilities:\n");
    for (int idx = 0; capabilities[idx].id || strlen(capabilities[idx].name);
         idx++) {
...
//			printf("Yamaha ForceSpeed   : %s\n", drive->yamaha.forcespeed ? "ON":"OFF");
    ...
    for (int idx = 0;
         rw_capabilities[idx].id || strlen(rw_capabilities[idx].name); idx++) {
  ...
  if (!pname && ((test && (strcmp(test, "rt") && strcmp(test, "wt"))) ||
    ...
    //	scanner = new qscanner(dev);
    if (scanner->plugins_probe(!test && (flags & FL_LPLUGIN), false) &&
        !(flags & FL_LPLUGIN)) {
      printf(
          MSGPREF
          "Device not found in any plugin support list, trying to probe...\n");
      if (scanner->plugins_probe(!test && (flags & FL_LPLUGIN),
                                 !(flags & FL_LPLUGIN))) {
  ...
  if (flags & FL_MINFO) {
    ...
    while (MEDIA[i].id != 0xFFFFFFFF &&
           (dev->media.type & (~DISC_CDRWSUBT)) != MEDIA[i].id)
      i++;
    ...
    if (scanner && scanner->is_plugin_attached()) {
      printf(IMEDIA "Available quality tests:%s%s%s%s%s%s\n",
             dev->media.capacity ? " rt" : "", dev->media.dstatus ? "" : " wt",
    ...
    printf(IMEDIA "Read capacity : %d sectors/%dMB/%02d:%02d.%02dMSF\n",
           dev->media.capacity, dev->media.capacity / 512,
           dev->media.capacity / 4500, ((dev->media.capacity) / 75) % 60,
           dev->media.capacity % 75);
    printf(IMEDIA "Free capacity : %d sectors/%dMB/%02d:%02d.%02dMSF\n",
           dev->media.capacity_free, dev->media.capacity_free / 512,
           dev->media.capacity_free / 4500,
           ((dev->media.capacity_free) / 75) % 60,
           dev->media.capacity_free % 75);
    printf(IMEDIA "Total capacity: %d sectors/%dMB/%02d:%02d.%02dMSF\n",
           dev->media.capacity_total, dev->media.capacity_total / 512,
           dev->media.capacity_total / 4500,
           ((dev->media.capacity_total) / 75) % 60,
           dev->media.capacity_total % 75);
    ...
    if (dev->media.type & (DISC_DVDRAM | DISC_HDDVD_RAM)) {
      ...
      printf(IMEDIA "Primary SA    : %d (%dMB) / %d (%dMB)\n",
             dev->media.spare_psa_free, dev->media.spare_psa_free / 512,
             dev->media.spare_psa_total, dev->media.spare_psa_total / 512);
      printf(IMEDIA "Supplement. SA: %d (%dMB) / %d (%dMB)\n",
             dev->media.spare_ssa_free, dev->media.spare_ssa_free / 512,
             dev->media.spare_ssa_total, dev->media.spare_ssa_total / 512);
    }
    ...
    if ((flags & FL_MID_RAW) && (dev->media.type & DISC_CD) &&
        dev->media.ATIP_size) {
      for (int i = 0; i < ((dev->media.ATIP_size - 4) >> 2); i++) {
        printf(IMEDIA "ATIP data %X   : ", i);
        printf(" %02X %02X %02X %02X", dev->media.ATIP[(i << 2) + 4],
               dev->media.ATIP[(i << 2) + 5], dev->media.ATIP[(i << 2) + 6],
               dev->media.ATIP[(i << 2) + 7]);
        printf(" | %3d %3d %3d %3d\n", dev->media.ATIP[(i << 2) + 4],
               dev->media.ATIP[(i << 2) + 5], dev->media.ATIP[(i << 2) + 6],
               dev->media.ATIP[(i << 2) + 7]);
      }
    }
    if (dev->media.MID_size) {
      if (dev->parms.wr_speed_tbl_media[0] > 0) {
        ...
        for (int i = 0;
             i < speed_tbl_size && dev->parms.wr_speed_tbl_media[i] > 0; i++)
          // for (int i=0; i<speed_tbl_size; i++)
          printf(SMEDIA "M WR speed #%02d: %4.1fX\n", i,
                 dev->parms.wr_speed_tbl_media[i]);
      }
      if (flags & FL_MID_RAW) {
        for (int i = 0; i < (dev->media.MID_size >> 4); i++) {
          unsigned char c;
          printf(IMEDIA "MID raw data %X0: ", i);
          for (int ii = 0; ii < 16 && ((i << 4) + ii) < dev->media.MID_size;
               ii++) {
            printf("%02X ",
                   (unsigned char)dev->media.MID_raw[(i << 4) + ii + 4]);
          }
          printf(" | ");
          for (int ii = 0; ii < 16 && ((i << 4) + ii) < dev->media.MID_size;
               ii++) {
    ...
    printf(IMEDIA "Track:     Session   TMode   DMode      Start        End    "
                  "   Size       Free   Next Writable\n");
    printf("-------------------------------------------------------------------"
           "-------------------------------\n");
    if (dev->media.tracks)
      for (i = 0; i < dev->media.tracks; i++) {
        printf(IMEDIA "Track# %2d       %2d      %2d      %2d   %8d   %8d   "
                      "%8d   %8d   %8d\n",
               dev->media.track[i].n, dev->media.track[i].session,
               dev->media.track[i].track_mode, dev->media.track[i].data_mode,
               dev->media.track[i].start, dev->media.track[i].last,
               dev->media.track[i].size, dev->media.track[i].free,
  ...
  if (test && !strcmp(test, "wt")) {
    ...
    if (dev->media.dstatus &
        !(dev->media.type & (DISC_DVDRAM | DISC_DVDpRW | DISC_DVDpRWDL))) {
...
//		printf("setting signal handlers...\n");
    ...
    if (!strcmp(test, "wt")) {
      if ((dev->media.dstatus || !dev->media.capacity_free) &&
          !(dev->media.type & (DISC_DVDRAM | DISC_DVDpRW | DISC_DVDpRWDL))) {
        //	sprintf(linef, "tsize=%dk", 2295104 * 2);
        printf(
            "Media should be blank to run write transfer rate. Aborting...\n");
        result = 4;
        goto end;
      }
  ...
  if (scanner) {
    if (!dev->silent)
      printf(MSGPREF "destroing scanner...\n");
    delete scanner;
  }
  if (!dev->silent)
    printf(MSGPREF "destroing dev...\n");
  delete dev;
  return result;
}
