...
cdvdreader::cdvdreader(int itidx, int iparent, drive_info *idev, smap *imap,
                       imgwriter *iiso, int ipass, int itries) {
  parm.running = 0;
  parm.stop = 0;
  parm.tidx = itidx;
  parm.parent = iparent;
  ...
  //    printf("%02d: %s %s %s\n", parm.tidx, parm.dev->ven, parm.dev->dev,
  //    parm.dev->fw);
}
