...
#include "qscan_plugin_generic.hpp"
#include <cstdio>

scan_plugin *plugin_create(drive_info *idev) { return new scan_generic(idev); }
...
scan_generic::scan_generic(drive_info *idev) : scan_plugin() {
  dev = idev;
  if (!dev->silent)
    printf("scan_generic()\n");
