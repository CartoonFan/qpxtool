...
#include "qscan_plugin_plextor.hpp"
...
int scan_plextor::cmd_cd_errc_init() {
    ...
    if ((dev->err = dev->cmd.transport())) {
        if (!dev->silent)
            sperror("PLEXTOR_START_CX", dev->err);
        return dev->err;
    }
#ifdef _debug_cx
    printf("00 18 01 01 00 4B |      LBA    |  BLER   E31   E21   E11   E32   "
           "???   E22   E12\n");
#endif
    printf("scan init OK!\n");
    return 0;
...
int scan_plextor::cmd_dvd_errc_init() {
    ...
    if ((dev->err = dev->cmd.transport())) {
        if (!dev->silent)
            sperror("PLEXTOR_START_PIPO", dev->err);
        return dev->err;
    }
    printf("scan init OK!\n");
    return 0;
...
int plextor_start_pie(drive_info* dev) {
    ...
    if ((dev->err=dev->cmd.transport() ))
    {
        sperror ("PLEXTOR_START_PISUM8",dev->err);
        return dev->err;
    }
    return 0;
}
...
int plextor_start_pie_poe(drive_info* dev) {
    ...
    if ((dev->err=dev->cmd.transport() ))
    {
        sperror ("PLEXTOR_START_PISUM8_POE",dev->err);
        return dev->err;
    }
    return 0;
}
...
int plextor_start_pif(drive_info* dev) {
    ...
    if ((dev->err=dev->cmd.transport() ))
    {
        sperror ("PLEXTOR_START_PIF",dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_plextor::cmd_cd_jb_init() {
    ...
    //	dev->cmd[11]= 0x00;
    if ((dev->err = dev->cmd.transport())) {
        sperror("PLEXTOR_START_JB_CD", dev->err);
        return dev->err;
    }
    printf("scan init OK!\n");
    return 0;
...
int scan_plextor::cmd_dvd_jb_init() {
    ...
    //	dev->cmd[11]= 0x00;
    if ((dev->err = dev->cmd.transport())) {
        sperror("PLEXTOR_START_JB_DVD", dev->err);
        return dev->err;
    }
    printf("scan init OK!\n");
    return 0;
...
//	start address
    ...
    if ((dev->err = dev->cmd.transport())) {
        sperror("PLEXTOR_START_FETE", dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_plextor::cmd_scan_end() {
    ...
    dev->cmd[11] = 0x00;
    if ((dev->err = dev->cmd.transport())) {
        sperror("PLEXTOR_END_SCAN", dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_plextor::cmd_fete_end() {
    ...
    dev->cmd[9] = 0x00;
    if ((dev->err = dev->cmd.transport())) {
        sperror("PLEXTOR_END_FETE", dev->err);
        return dev->err;
    }
    return 0;
}
...
int scan_plextor::cmd_cd_errc_getdata(cd_errc *data) {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 0x1A))) {
        sperror("PLEXTOR_READ_CD_ERROR_INFO", dev->err);
        return dev->err;
    }
    ...
    for (i = 0x06; i < 0x0A; i++)
        printf("%02X ", dev->rd_buf[i] & 0xFF);
    printf("| ");
    for (i = 0x0A; i < 0x1A; i += 2) {
        if (ntoh16(dev->rd_buf + i))
            printf("%5d ", ntoh16(dev->rd_buf + i));
        else
            printf("_____ ");
    }
    printf("|\n");
...
int scan_plextor::cmd_dvd_errc_getdata(dvd_errc *data) {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 0x34))) {
        sperror("PLEXTOR_READ_DVD_ERROR_INFO", dev->err);
        return dev->err;
    }
    /*
    test 0x12:
...
int plextor_read_pi_info(drive_info* dev) {
    ...
    if ((dev->err=dev->cmd.transport(READ,dev->rd_buf,0x34)))
    {
        sperror ("PLEXTOR_READ_PI",dev->err);
        return dev->err;
    }
    /*
    	printf("READ PI:");
    ...
    */
    for (i=0x00; i<0x06; i++) printf("%02X ", dev->rd_buf[i] & 0xFF);
    printf("|");
...
int scan_plextor::cmd_jb_getdata(cdvd_jb *data) {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 0x10))) {
        sperror("PLEXTOR_READ_JB", dev->err);
        return dev->err;
    }
#ifdef _debug_jb
    int i;
...
int scan_plextor::cmd_dvd_errc_block(dvd_errc *data) {
    if (read_one_ecc_block(dev, dev->rd_buf, lba))
        data->uncr++;
...
int scan_plextor::cmd_fete_getdata() {
    ...
    if ((dev->err = dev->cmd.transport(READ, dev->rd_buf, 0xCE))) {
        sperror("PLEXTOR_FETE_READOUT", dev->err);
        return dev->err;
    }
#ifdef _debug_fete
...
int scan_plextor::cmd_fete_block(cdvd_ft *data) {
    ...
    if (lba > (dev->media.capacity_total - 1))
        lba = dev->media.capacity_total - 1;

    while ((data->fe == 0 || data->te == 0) && (fete_idx <= 99) &&
            (rdy == 0x20408)) {
        if (cmd_fete_getdata())
            return -1;
        data->te = dev->rd_buf[offs];
...
int scan_plextor::build_TA_histogram_px716(unsigned char *response_data,
        int *dest_pit, int *dest_land,
        int len) {
...
int scan_plextor::build_TA_histogram_px755(unsigned char *response_data,
        int *dest_pit, int *dest_land,
        int len, int dt) { //, int spd) {
    int *dest[] = {dest_land, dest_pit};
    int count = ntoh16(response_data + 2);
...
int scan_plextor::evaluate_histogramme(cdvd_ta *data, int **peaks, int **mins) {
    ...
    for (int k = 0; k < 2; k++) {
        ...
        local_max = 0;
        for (i = 40; i < 330; i++) {
            if (src[k][i - 1] <= src[k][i] && src[k][i + 1] <= src[k][i] &&
                    src[k][i] > 20 && src[k][i] > local_max) {
                peaks[k][j1] = i;
                local_max = src[k][i];
                next_peak = 1;
            } else if (peak_found)
...
int scan_plextor::cmd_dvd_ta_block(cdvd_ta *data) {
    ...
    int i, r, m, j;
    float sum;
    unsigned char scan_cmd[6][2] = {{0x04, 0x00}, {0x10, 0x00}, {0x20, 0x00},
        {0xFA, 0x28}, {0xEA, 0x28}, {0xDE, 0x28}
    };
    const char *scan_txt[] = {
        "Running TA on L0 inner zone ", "Running TA on L0 middle zone",
        "Running TA on L0 outer zone",  "Running TA on L1 inner zone ",
        "Running TA on L1 middle zone", "Running TA on L1 outer zone"
    };
    //	int ta_response_pit[6][512]; int ta_response_land[6][512];
    int peaks_lands[15], peaks_pits[15];
    int mins_lands[15], mins_pits[15];
    ...
    for (i = 0; i < 9; i++) {
        ...
        if (!strncmp(dev->dev, "DVDR   PX-714A", 14) ||
            ...
            build_TA_histogram_px755(dev->rd_buf, data->pit, data->land, 512,
                                     dev->media.type);
    ...
    sum = 0;
    for (m = 0; m < 10; m++) {
        r = (int)((float)peaks_pits[m] - 21.5454 * ((m < 9) ? m : 11) - 64);
        sum += sqrt(abs(r));
        printf("%4d", r);
    }
    printf("  sum %f \n", sum);
    ...
    sum = 0;
    for (m = 0; m < 10; m++) {
        r = (int)((float)peaks_lands[m] - 21.5454 * ((m < 9) ? m : 11) - 64);
        sum += sqrt(abs(r));
        printf("%4d", r);
    }
    printf("  sum %f \n", sum);
