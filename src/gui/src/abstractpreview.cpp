 ...
 * Copyright (C) 2009 Maxim Aldanov <aldmax@mail.ru>, Gennady "ShultZ" Kozlov
 * <qpxtool@mail.ru> Copyright (C) 2008 Gennady "ShultZ" Kozlov
 * <qpxtool@mail.ru>
 *
...
public:
  ...
  bool isValid() { return (bool)(index == -1); }
...
private:
  class Tile {
  public:
    Tile(QPixmap *pix, int p, int x, int y)
        : data(pix), page(p), xidx(x), yidx(y){};
    ~Tile() {
      if (data)
        delete data;
    };
...
private:
  ...
  qreal mm_left, mm_right, mm_top, mm_bottom;
  // то же, но в пикселах
  int mp_left, mp_right, mp_top, mp_bottom;
  // с учетом масштаба
  int ms_left, ms_top;
  //  int	  ms_right,
  //    	  ms_bottom;
  ...
  //Мастштаб
  double scale, scaleStep, scaleMax, scaleMin;
...
#ifndef QT_NO_DEBUG
  qDebug() << "TilesCache::insertPages() start: " << start
           << " count: " << count;
#endif
  for (int i = tiles.size() - 1; i >= 0; i--)
    if (tiles[i]->page >= start)
      tiles[i]->page += count;
}
...
#ifndef QT_NO_DEBUG
  qDebug() << "TilesCache::deletePages() start: " << start
           << " count: " << count;
#endif
  for (int i = tiles.size() - 1; i >= 0; i--) {
    if (tiles[i]->page > end)
      tiles[i]->page -= count;
    else if (tiles[i]->page >= start)
      delete tiles.takeAt(i);
  }
}
...
QPixmap *TilesCache::getTile(int page, int xidx, int yidx) {
  Tile *tile;
#ifndef QT_NO_DEBUG
//	qDebug() << "TilesCache::getTile(): " << page << " x:" << xidx << " y:"
//<< yidx;
#endif
...
//--------------------------------------------------------------
AbstractPreviewPrivate::AbstractPreviewPrivate(AbstractPreview *preview,
                                               QPrinter *printer) {
...
//--------------------------------------------------------------
AbstractPreviewPrivate::~AbstractPreviewPrivate() { clear(); }
//--------------------------------------------------------------
...
void AbstractPreviewPrivate::changeScale() {
  ...
  if (fw - ((qreal)(int)fw) > 0.0)
    paperSizeScaled.setWidth((int)fw + 1);
  else
    paperSizeScaled.setWidth((int)fw);

  fh = paperSize.height() * scale;
  if (fh - ((qreal)(int)fh) > 0.0)
    paperSizeScaled.setHeight((int)fh + 1);
  else
    paperSizeScaled.setHeight((int)fh);

  pw = pageSize.width() * scale;
  if (pw - ((qreal)(int)pw) > 0.0)
    pageSizeScaled.setWidth((int)pw + 1);
  else
    pageSizeScaled.setWidth((int)pw);

  ph = pageSize.height() * scale;
  if (ph - ((qreal)(int)ph) > 0.0)
    pageSizeScaled.setHeight((int)ph + 1);
  else
    pageSizeScaled.setHeight((int)ph);
...
void AbstractPreviewPrivate::setPageDefaults() {
  ...
  printer->setPageMargins(mm_left, mm_top, mm_right, mm_bottom,
                          QPrinter::Millimeter);
...
void AbstractPreviewPrivate::setup() {
  ...
  //    if(paperSizeMM.width() -  ((qreal)(int)paperSize.width()) > 0.0)
  //    paperSize.setWidth ((int)paperSize.width() + 1); if(paperSizeMM.height()
  //    - ((qreal)(int)paperSize.height()) > 0.0)
  //    paperSize.setHeight((int)paperSize.height() + 1);

  //    if(pageSizeMM.width() -  ((qreal)(int)pageSize.width()) > 0.0)
  //    pageSize.setWidth ((int)pageSize.width() + 1); if(pageSizeMM.height() -
  //    ((qreal)(int)pageSize.height()) > 0.0)
  //    pageSize.setHeight((int)pageSize.height() + 1);

  printer->getPageMargins(&mm_left, &mm_top, &mm_right, &mm_bottom,
                          QPrinter::Millimeter);
  recalcMarginsFromMM();

#if (PRINTER_USE_FULLPAGE == 1) || (QT_VERSION < 0x040500)
  pageSizeMM = QSize(paperSizeMM.width() - mm_left - mm_right,
...
#ifndef QT_NO_DEBUG
  qDebug() << "device DPI (phy) X:" << currentDevice->physicalDpiX()
           << " Y:" << currentDevice->physicalDpiY();
  qDebug() << "device DPI (log) X:" << currentDevice->logicalDpiX()
           << " Y:" << currentDevice->logicalDpiY();
  qDebug() << "paperSize (mm): " << paperSizeMM.width() << "x"
           << paperSizeMM.height();
  qDebug() << "pageSize  (mm): " << pageSizeMM.width() << "x"
           << pageSizeMM.height();
...
//	qDebug() << "calcParamRedraw(): preview @ "<< preview;
#endif
  const int startRow =
      (preview->verticalScrollBar()->value() - pageMargin) /
  ...
  yy = ((paperHeightToScale() + interPageSpacing + pageLabelHeight) * startRow +
        pageMargin);
...
//--------------------------------------------------------------
void AbstractPreviewPrivate::flushCache() { cache.clear(); }
//--------------------------------------------------------------
...
//--------------------------------------------------------------
void AbstractPreviewPrivate::flushPage(int page) { cache.deletePages(page, 1); }
//--------------------------------------------------------------
...
//--------------------------------------------------------------
AbstractPreview::~AbstractPreview() { delete d; }
//--------------------------------------------------------------
int AbstractPreview::sizeToCountPage() {
  ...
  if (d->viewMode == AbstractPreview::Mode_Normal) {
    cols =
        (int)(viewport()->width() - 2 * d->pageMargin + d->interPageSpacing) /
...
void AbstractPreview::autoscaleThumbs() {
  //	qDebug() << "autoscaleThumbs(): w=" << d->w << " viewport()->width()="
  //<< viewport()->width();
  ...
  if (scale != d->scale) {
    d->scale = scale;
    d->changeScale();
    d->pageLabelRect = QRect(1, d->paperSizeScaled.height() + 3,
                             d->paperSizeScaled.width(), d->pageLabelHeight);
  }
  sizeToCountPage();
}
//--------------------------------------------------------------
...
//	qDebug() << "Tile not found in cache, rendering new one";
#endif
    // tile->data = new QPixmap(d->paperWidthToScale() + 1,
    // d->paperHeightToScale() + 1);
    pix = new QPixmap(
        qMin(d->tileWidth, (d->pageSizeScaled.width() - d->tileWidth * tx)),
        qMin(d->tileHeight, (d->pageSizeScaled.height() - d->tileHeight * ty)));
    //                qDebug() << "page " << page << "  tile: " << ty << "," <<
    //                tx << " " << pix->width() << "x" << pix->height();
    ...
    paintPage(&pp, page,
              QRect(QPoint(d->tileWidth * tx, d->tileHeight * ty) / d->scale,
                    pix->size() / d->scale));
  ...
  for (int ty = ty_sta; ty < ty_end; ty++) {
    for (int tx = tx_sta; tx < tx_end; tx++) {
      ...
      if (!px)
        continue;
      QPainter pp(px);
      pp.drawPixmap(sec.topLeft() - tileRect.topLeft(), pix,
                    QRect(sec.topLeft() - rect.topLeft(), sec.size()));
    }
  }
...
void AbstractPreview::paintEvent(QPaintEvent *) {
  ...
  for (int y = 0; y < d->countPagesRow; y++) {
    ...
    for (int x = 0; x < d->cols; x++) {
      ...
      for (int ty = ty_sta; ty < ty_end; ty++) {
        for (int tx = tx_sta; tx < tx_end; tx++) {
          p->drawPixmap(d->tileWidth * tx, d->tileHeight * ty,
                        *(getTile(page, tx, ty)));
        }
      }
#ifdef DEBUG_PAINT_TIME
      gettimeofday(&te, NULL);
      qDebug() << "Page: " << page << "  " << tx_sta << "." << ty_sta << " - "
               << tx_end << "." << ty_end
               << QString("  time: %1 s")
                      .arg(te.tv_sec - tb.tv_sec +
      ...
      mrect =
          QRect(QPoint(-xx + d->ms_left, -yy + d->ms_top), viewport()->size());
      ...
      QPageInfo pg(page,
                   QRect(xx - d->ms_left, yy - d->ms_top,
                         d->paperWidthToScale(), d->paperHeightToScale()));
      ...
      if (d->pageLabelHeight) {
        ...
        p->drawText(d->pageLabelRect, Qt::AlignCenter,
                    QString::number(page + 1));
      }

      page++;
      if (page == d->pagesData.count())
        break;
    ...
    if (page == d->pagesData.count())
      break;
    p->translate(0, d->paperHeightToScale() + d->interPageSpacing +
                        d->pageLabelHeight);
...
//--------------------------------------------------------------
void AbstractPreview::additionalPaintEvent(QPainter *p, int page,
                                           const QRect &rect) {
  Q_UNUSED(p);
  Q_UNUSED(page);
}
...
QPoint AbstractPreview::toCurrentPage(QPoint point) {
  if (!d->currentPageRect.contains(point))
    return QPoint();
...
QPageInfo AbstractPreview::pageInfo(const QPoint &point) {
  ...
  if (!d->enterPageInfo.rect().contains(point)) {
    ...
    if (page == -1)
      ...
      d->enterPageInfo = QPageInfo(page, rect);
  }
  return d->enterPageInfo;
}
//--------------------------------------------------------------
QVector<QPageInfo> AbstractPreview::visiblePages() { return d->pagesRect; }
//--------------------------------------------------------------
...
void AbstractPreview::moveContext(const QPoint &point) {
  if (!d->mousePressData.press && d->scrollBarValueOnMousePress.isNull())
    return;

  horizontalScrollBar()->setValue(d->scrollBarValueOnMousePress.x() -
                                  point.x() + d->mousePressData.pos.x());
  verticalScrollBar()->setValue(d->scrollBarValueOnMousePress.y() - point.y() +
                                d->mousePressData.pos.y());
  d->calcParamDraw();
...
#warning disabled due to enable multiple selection of same page (i.e. to return to viewed page)
    ...
    mousePressPageEvent(e, d->selectPageIndex, d->currentPageRect,
                        toCurrentPage(e->pos()));

    emit pageSelected(d->selectPageIndex);
    updatePreview();
  } else {
    if (d->selectPageIndex == -1)
      return;
    d->selectPageIndex = -1;
    updatePreview();
  }
...
//--------------------------------------------------------------
void AbstractPreview::mouseMovePageEvent(QMouseEvent *event,
                                         const QPageInfo &info) {
  Q_UNUSED(event);
  Q_UNUSED(info);
}
//--------------------------------------------------------------
...
//--------------------------------------------------------------
int AbstractPreview::interPageSpacing() { return d->interPageSpacing; }
//--------------------------------------------------------------
void AbstractPreview::setPageMargin(int margin) {
  if (d->pageMargin == margin)
    return;
  d->pageMargin = margin;
  sizeToCountPage();
  updatePreview();
...
//--------------------------------------------------------------
int AbstractPreview::pageMargin() { return d->pageMargin; }
//--------------------------------------------------------------
int AbstractPreview::pageCount() { return d->pagesData.count(); }
//--------------------------------------------------------------

void AbstractPreview::setMargins(qreal ml, qreal mt, qreal mr, qreal mb) {
  if (d->mm_left == ml && d->mm_right == mr && d->mm_top == mt &&
      d->mm_bottom == mb)
    return;
  d->mm_left = ml;
  d->mm_right = mr;
...
void AbstractPreview::setPageData(int index, QVariant data) {
  if (index < 0 || index >= d->pagesData.count())
    return;
...
//--------------------------------------------------------------
AbstractPreview::PreviewMode AbstractPreview::viewMode() { return d->viewMode; }
//--------------------------------------------------------------
#ifdef PRINTER_CHANGE_DEVICE
void AbstractPreview::changeDevice(QPaintDevice *device) {
  if (device == d->printer) {
    if (!dp)
      dp = new AbstractPreviewPrivate(this, d->printer);
    d = dp;
...
void AbstractPreview::print(QPrinter *printer) { printPages(printer); }
...
#ifdef DEBUG_PRINT_TIME
  qDebug() << "Setup time: "
           << QString(" %1 s").arg(tpb.tv_sec - tsb.tv_sec +
                                       (tpb.tv_usec - tsb.tv_usec) / 1000000.0,
                                   0, 'f', 4);
#endif
#endif
...
#ifdef DEBUG_PRINT_TIME
  qDebug() << "Pages time: "
           << QString(" %1 s").arg(tpe.tv_sec - tpb.tv_sec +
                                       (tpe.tv_usec - tpb.tv_usec) / 1000000.0,
                                   0, 'f', 4);
#endif
}
//--------------------------------------------------------------
...
//--------------------------------------------------------------
void AbstractPreview::setScaleStep(double step) { d->scaleStep = step; }
//--------------------------------------------------------------
double AbstractPreview::scaleStep() { return d->scaleStep; }
//--------------------------------------------------------------
...
//--------------------------------------------------------------
double AbstractPreview::scaleMaximum() { return d->scaleMax; }
//--------------------------------------------------------------
double AbstractPreview::scaleMinimum() { return d->scaleMin; }
//--------------------------------------------------------------
void AbstractPreview::scaleEvent(const double &scaleOld,
                                 const double &scaleNew) {
  Q_UNUSED(scaleOld);
...
void AbstractPreview::gotoPage(int index) {
  ...
  int y = row *
          (d->paperHeightToScale() + d->interPageSpacing + d->pageLabelHeight);
...
//--------------------------------------------------------------
QSize AbstractPreview::paperSize() { return d->paperSize; }
//--------------------------------------------------------------
...
//--------------------------------------------------------------
int AbstractPreview::currentPage() { return d->selectPageIndex; }
//--------------------------------------------------------------
qreal AbstractPreview::scale() { return d->scale; }
//--------------------------------------------------------------
...
//--------------------------------------------------------------
QPixmap *AbstractPreview::grabPage(int page, qreal left, qreal top, qreal width,
                                   qreal height) {
  if (!(page >= 0 && page < pageCount()))
    return NULL;
  return NULL;
