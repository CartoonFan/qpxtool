...
prefReports::prefReports(QPxSettings *iset, QWidget *p, Qt::WindowFlags fl)
  ...
  connect(ck_autosave_db, SIGNAL(toggled(bool)), this,
          SLOT(box_rep_toggled(bool)));
...
prefReports::~prefReports() {
  if (ck_eject->isChecked())
    set->actions_flags |= AFLAG_EJECT_AFTER;
  else
    set->actions_flags &= ~AFLAG_EJECT_AFTER;
...
void prefReports::select_rep_path() {
  ...
  QString path = QFileDialog::getExistingDirectory(
      this, tr("Select directory..."), e_rep_path->text());
  if (path.isEmpty())
    return;
...
void prefReports::box_rep_toggled(bool) {
  ck_eject->setEnabled(box_rep->isChecked() || (box_rep_db->isChecked() &&
                                                ck_autosave_db->isChecked()));
  if (!ck_eject->isEnabled())
    ck_eject->setChecked(false);
}
