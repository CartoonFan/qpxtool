...
GraphTab::GraphTab(QPxSettings *iset, devlist *idev, QString iname, int test,
...
void GraphTab::drawGraph(QImage &img, device *dev, int ttype, int eflags) {
    ...
    graph->drawGraph(&p, s, dev, ttype, r, (ttype == TEST_ERRC) ? eflags : 0,
                     FORCE_REPAINT);
}
