...
{
    if (ck_autow->isChecked()) set->actions_flags |= AFLAG_AUTOSTART_W;
    else set->actions_flags &= ~AFLAG_AUTOSTART_W;
    if (ck_autob->isChecked()) set->actions_flags |= AFLAG_AUTOSTART_B;
    else set->actions_flags &= ~AFLAG_AUTOSTART_B;

    if (ck_rt->isChecked()) set->actions_flags |= AFLAG_DTEST_RT;
    else set->actions_flags &= ~AFLAG_DTEST_RT;
    if (ck_wt->isChecked()) set->actions_flags |= AFLAG_DTEST_WT;
    else set->actions_flags &= ~AFLAG_DTEST_WT;
    if (ck_wt_simul->isChecked()) set->actions_flags |= AFLAG_DTEST_WT_SIMUL;
    else set->actions_flags &= ~AFLAG_DTEST_WT_SIMUL;
    if (ck_errc->isChecked()) set->actions_flags |= AFLAG_DTEST_ERRC;
    else set->actions_flags &= ~AFLAG_DTEST_ERRC;
    if (ck_jb->isChecked()) set->actions_flags |= AFLAG_DTEST_JB;
    else set->actions_flags &= ~AFLAG_DTEST_JB;
    if (ck_ftw->isChecked()) set->actions_flags |= AFLAG_DTEST_FT_W;
    else set->actions_flags &= ~AFLAG_DTEST_FT_W;
    if (ck_ftb->isChecked()) set->actions_flags |= AFLAG_DTEST_FT_B;
    else set->actions_flags &= ~AFLAG_DTEST_FT_B;
    if (ck_ta->isChecked()) set->actions_flags |= AFLAG_DTEST_TA;
    else set->actions_flags &= ~AFLAG_DTEST_TA;
}
