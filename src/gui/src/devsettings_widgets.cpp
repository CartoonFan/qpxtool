...
devctlCommon::devctlCommon(device *idev, QWidget *p) : QGroupBox(p) {
    dev = idev;
...
void devctlCommon::set_prec(bool en) {
    dev->setFeature(FEATURE_POWEREC, en);
}
void devctlCommon::set_hcdr(bool en) {
    dev->setFeature(FEATURE_HIDECDR, en);
}
void devctlCommon::set_ss(bool en) {
    dev->setFeature(FEATURE_SINGLESESSION, en);
}
void devctlCommon::set_spdrd(bool en) {
    dev->setFeature(FEATURE_SPEEDREAD, en);
}
void devctlCommon::set_bitset(bool en) {
    dev->setFeature(FEATURE_BITSETR, en);
}
void devctlCommon::set_bitset_dl(bool en) {
    dev->setFeature(FEATURE_BITSETRDL, en);
}
void devctlCommon::set_simulplus(bool en) {
    dev->setFeature(FEATURE_SIMULPLUS, en);
}
...
devctlGigarec::devctlGigarec(device *idev, QWidget *p)
    ...
    bool ratio_11 = dev->dev.startsWith("DVDR   PX-755") ||
                    dev->dev.startsWith("DVDR   PX-760") ||
                    dev->dev.startsWith("CD-R   PREMIUM2");
    ...
    l_hint = new QLabel(tr("GigaRec limits maximum write speed to 8X\n"
                           "only DAO write mode can be used with GigaRec"),
                        this);
    layout->addWidget(l_hint, 10);

    // setEnabled(dev->features.supported & FEATURE_GIGAREC && dev->media.dstate
    // == "Blank");
...
devctlVarirec::devctlVarirec(device *idev, QWidget *p) : QWidget(p) {
    layout = new QVBoxLayout(this);
    layout->setMargin(0);
    layout->setSpacing(3);
    ...
    //	qDebug() << "Varirec CD  : " << (int)idev->features.vrec_cd_pwr << ", "
    //<< idev->features.vrec_cd_str; 	qDebug() << "Varirec DVD : " <<
    //(int)idev->features.vrec_dvd_pwr << ", " << idev->features.vrec_dvd_str;
}
...
devctlSecurec::devctlSecurec(device *idev, QWidget *p)
    ...
    pb_reset =
        new QPushButton(QIcon(":images/edit-clear.png"), tr("Reset"), this);
    ...
    l_hint = new QLabel(
        tr("Password length must me between 4 and 10 characters.\n\n"
           ...
           "with same password as used for writing"),
        this);
...
int devctlSecurec::validate() {
    if (e_pwd[0]->text() != e_pwd[1]->text())
        return 1;
    if (e_pwd[0]->text().length() < 4 || e_pwd[0]->text().length() > 10)
        return 2;
    return 0;
}
...
void devctlSecurec::set() {
    ...
    case 0:
        break;
    case 1:
        QMessageBox::warning(this, "Different passwords",
...
devctlSilent::devctlSilent(device *idev, QWidget *p) : QWidget(p) {
    dev = idev;
    int idx;
    bool cd = 0;
    bool dvd = 0;
    ...
    for (int i = 0; sm_rcd[i] > 0; i++)
        box_rcd->addItem(QString::number(sm_rcd[i]) + "X");
    box_wcd = new QComboBox(cw);
    for (int i = 0; sm_wcd[i] > 0; i++)
        box_wcd->addItem(QString::number(sm_wcd[i]) + "X");
    box_rdvd = new QComboBox(cw);
    for (int i = 0; sm_rdvd[i] > 0; i++)
    ...
    */
    idx = box_rcd->findText(
              QString::number(dvd ? dev->features.psm_cd_rd : dev->features.sm_cd_rd) +
              "X");
    if (idx >= 0)
        box_rcd->setCurrentIndex(idx);
    idx = box_wcd->findText(
              QString::number(dvd ? dev->features.psm_cd_wr : dev->features.sm_cd_wr) +
              "X");
    if (idx >= 0)
        box_wcd->setCurrentIndex(idx);

    idx = box_rdvd->findText(QString::number(dvd ? dev->features.sm_dvd_rd
                             : dev->features.psm_dvd_rd) +
                             "X");
    if (idx >= 0)
...
devctlAutostrategy::devctlAutostrategy(device *idev, QWidget *p) : QWidget(p) {
    dev = idev;
...
void devctlAutostrategy::run_mqck() {
    ...
    dev->features.as_mqckspd =
        bmqck_spd->currentText().remove(QRegExp(".[0-9][Xx]")).toInt();
...
devctlAutostrategyDB::devctlAutostrategyDB(device *idev, QWidget *p)
    ...
    tree->setHeaderLabels(QStringList() << "Act" << tr("Type") << tr("Media ID")
                          << tr("Speed") << tr("Writes"));
    ...
    pb_clear =
        new QPushButton(QIcon(":images/edit-clear.png"), tr("Clear"), this);
    ...
    connect(tree, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this,
            SLOT(db_toggle(QTreeWidgetItem *)));
    ...
    for (int i = 0; i < dev->asdb.size(); i++) {
        if (dev->asdb[i].present) {
            it = new QTreeWidgetItem(
                tree, QStringList() << "" << dev->asdb[i].type << dev->asdb[i].mid
                << dev->asdb[i].speed << dev->asdb[i].writes);
            if (dev->asdb[i].active)
                it->setIcon(0, QIcon(":images/ok.png"));
            tree->addTopLevelItem(it);
        }
    }

    if (idx >= 0) {
        if (idx >= dev->asdb.size())
            idx = dev->asdb.size() - 1;
...
void devctlAutostrategyDB::run_ascre() {
    ...
    //	dev->
    if (dev->media.creads || !dev->media.type.startsWith("DVD") ||
            dev->media.erasable == "yes") {
        QMessageBox::warning(
            this, tr("Can't create strategy!"),
            tr("Strategy creation does not supported on current media: ") +
            dev->media.type + "\n" +
    ...
    dev->features.as_act_mode =
        (rb_ascre_full->isChecked() ? ASCRE_FULL : 0) |
        (rb_ascre_replace->isChecked() ? ASCRE_REPLACE : 0);
    dev->startAScre();
...
devctlPioquiet::devctlPioquiet(device *idev, QWidget *p)
    ...
    ck_limit =
        new QCheckBox(tr("Limit read speed to 24X for CD and 8X for DVD"), this);
...
void devctlF1Tattoo::loadImage() {
    ...
    img = srcimg.scaled(F1TATTOOW, (r1 - r0), Qt::IgnoreAspectRatio,
...
void devctlF1Tattoo::burn() {
    ...
    // QImage timg = img.scaled(F1TATTOOW, (r1-r0+1), Qt::IgnoreAspectRatio,
    // Qt::SmoothTransformation);
    ...
    if (!tf.open(QIODevice::WriteOnly)) {
        QMessageBox::critical(this, tr("Error"),
                              tr("Can't create DiscT@2 temporary file!"));
        return;
    }
