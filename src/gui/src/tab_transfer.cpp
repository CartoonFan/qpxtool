...
#include "tab_transfer.hpp"
#include <QDebug>

tabTransfer::tabTransfer(QPxSettings *iset, devlist *idev, QString iname,
                         bool irw, QWidget *p, Qt::WindowFlags fl)
...
void tabTransfer::updateSummary(device *dev, float time) {
#ifndef QT_NO_DEBUG
    qDebug() << "tabTransfer::updateSummary(): " << (rw ? "WT" : "RT")
             << " device@" << dev;
#endif
    float avg_x;
    uint32_t avg_kb;
    ...
    l_sta_x->setText(QString::number(rw ? dev->testData.wt.first().spdx
                                     : dev->testData.rt.first().spdx,
                                     'f', 2) +
                     " X");
    l_sta_kb->setText(QString::number(rw ? dev->testData.wt.first().spdk
                                      : dev->testData.rt.first().spdk) +
                      " kB/s");

    l_end_x->setText(QString::number(rw ? dev->testData.wt.last().spdx
                                     : dev->testData.rt.last().spdx,
                                     'f', 2) +
                     " X");
    l_end_kb->setText(QString::number(rw ? dev->testData.wt.last().spdk
                                      : dev->testData.rt.last().spdk) +
                      " kB/s");

    avg_kb =
        (int)((rw ? dev->testData.wt.last().lba : dev->testData.rt.last().lba) *
              2 / time);
    if (dev->media.spd1X)
...
#ifndef QT_NO_DEBUG
    std::cout << "avg: " << avg_kb << " KB, " << avg_x << " X, "
              << dev->media.spd1X << " KB/X\n";
#endif
