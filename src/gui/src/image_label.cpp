...
ImageLabel::ImageLabel(int iminW, QString label, QImage image, int idx,
                       QWidget *parent)
    : QWidget(parent) {
#ifndef QT_NO_DEBUG
    qDebug("ImageLabel()");
...
ImageLabel::ImageLabel(int iminW, QSize is, QString label, QImage image,
                       int idx, QWidget *parent)
    : QWidget(parent) {
#ifndef QT_NO_DEBUG
    qDebug("ImageLabel()");
#endif
...
ImageLabel::ImageLabel(int iminW, int iw, int ih, QString label, QImage image,
                       int idx, QWidget *parent)
    : QWidget(parent) {
#ifndef QT_NO_DEBUG
    qDebug("ImageLabel()");
...
void ImageLabel::paintEvent(QPaintEvent *) {
    ...
    if (ck)
        p.fillRect(QRect(1, 1, width() - 2, height() - 2),
    ...
    if (focus) {
        if (ck)
            p.setPen(
                QPen(palette().color(QPalette::HighlightedText), 2, Qt::SolidLine));
        else
            p.setPen(QPen(palette().color(QPalette::Highlight), 2, Qt::SolidLine));
        p.drawRect(QRect(1, 1, width() - 2, height() - 2));
    }
#endif
    p.drawImage(((width() - img.width()) >> 1), ((iconh - img.height()) >> 1) + 2,
                img);
    ...
    p.drawText(2, iconh + 3, width() - 4, height() - iconh,
