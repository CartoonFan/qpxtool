...
TextSlider::TextSlider(QWidget *p) : QAbstractSlider(p) {
    initDefaults();
}
...
void TextSlider::addItem(QString item, bool en) {
    if (!items.contains(item))
        items << SliderItem(item, en);
    op.maximum = items.size() - 1;
    if (items.size())
        op.subControls |= QStyle::SC_SliderTickmarks;
...
bool TextSlider::prev() {
    int pos = op.sliderPosition;
    pos++;
    while (pos < op.maximum && !items[pos].enabled)
        pos++;
    if (pos > op.maximum || !items[pos].enabled)
        return 1;
...
bool TextSlider::next() {
    int pos = op.sliderPosition;
    pos--;
    while (pos > op.minimum && !items[pos].enabled)
        pos--;
    if (pos < op.minimum || !items[pos].enabled)
        return 1;
...
int TextSlider::value() {
    if (op.sliderPosition < 0 || op.sliderPosition >= items.size())
        return -1;
...
void TextSlider::setCurrentItem(const QString &text) {
    ...
    for (int i = 0; i < items.size(); i++) {
        if (items[i].enabled && items[i].text == text)
            idx = i;
    }
    if (idx < 0)
        return;
...
void TextSlider::setDisabled(bool disable) {
    setEnabled(!disable);
}

void TextSlider::mousePressEvent(QMouseEvent *) {
    oldpos = op.sliderPosition;
}
...
void TextSlider::mouseReleaseEvent(QMouseEvent *e) {
    ...
    if (op.orientation == Qt::Vertical) {
        pos = QStyle::sliderValueFromPosition(op.minimum, op.maximum,
                                              e->pos().y() - 10, height() - 21,
                                              op.upsideDown);
    } else {
...
void TextSlider::wheelEvent(QWheelEvent *e) {
    int pos = op.sliderPosition;
    if (e->delta() * (op.upsideDown ? -1 : 1) *
            (op.orientation == Qt::Vertical ? 1 : -1) <
            0) {
        prev();
    } else if (e->delta() * (op.upsideDown ? -1 : 1) *
               (op.orientation == Qt::Vertical ? 1 : -1) >
               0) {
        next();
    } else {
...
void TextSlider::keyPressEvent(QKeyEvent *e) {
    ...
    case Qt::Key_Up:
    case Qt::Key_Right:
        if ((op.upsideDown ? -1 : 1) * (op.orientation == Qt::Vertical ? 1 : -1) <
                0)
            prev();
        else
            next();
...
void TextSlider::paintEvent(QPaintEvent *) {
    ...
    // qDebug() << "Qt::WA_ForceDisabled: " <<
    // testAttribute(Qt::WA_ForceDisabled); qDebug() << "Qt::WA_Disabled: " <<
    // testAttribute(Qt::WA_Disabled);
    ...
    if (op.orientation == Qt::Vertical) {
        ...
        if (items.size())
            itemh /= items.size();
        for (int i = 0; i < items.size(); i++) {
            pos = QStyle::sliderPositionFromValue(op.minimum, op.maximum, i,
                                                  height() - VITEMSIZE - ctl,
                                                  op.upsideDown) +
                  VITEMSIZE / 2 + ctl / 2;
            p.setPen(
                QPen(palette().color((ena && items[i].enabled) ? QPalette::Active
                                     : QPalette::Disabled,
                                     QPalette::Text),
                     1));
            ...
            if (op.tickPosition == QSlider::TicksLeft ||
                ...
                p.drawText(0, pos - itemh / 2, width() - 30, itemh + 2,
                           Qt::AlignRight | Qt::AlignVCenter, items[i].text);
            else
                p.drawText(30, pos - itemh / 2, width() - 30, itemh + 2,
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (op.tickPosition == QSlider::TicksLeft ||
                    op.tickPosition == QSlider::TicksBothSides)
                p.drawText(pos - itemw / 2, 0, itemw, height() - 20,
                           Qt::AlignHCenter | Qt::AlignBottom, items[i].text);
            else
                p.drawText(pos - itemw / 2, 26, itemw, height() - 20,
                           Qt::AlignHCenter | Qt::AlignTop, items[i].text);
        }
        if (op.tickPosition == QSlider::TicksLeft ||
                op.tickPosition == QSlider::TicksBothSides)
