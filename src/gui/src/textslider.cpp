...
void TextSlider::keyPressEvent(QKeyEvent* e)
{
    ...
    case Qt::Key_Up:
    case Qt::Key_Right:
        if ((op.upsideDown ? -1:1) * (op.orientation == Qt::Vertical ? 1:-1) < 0) prev();
        else next();
        break;
    case Qt::Key_Down:
    case Qt::Key_Left:
        if ((op.upsideDown ? -1:1) * (op.orientation == Qt::Vertical ? 1:-1) < 0) next();
        else prev();
        break;
    case Qt::Key_Home:
        if ((op.upsideDown ? -1:1) * (op.orientation == Qt::Vertical ? 1:-1) < 0) first();
        else last();
        break;
    case Qt::Key_End:
        if ((op.upsideDown ? -1:1) * (op.orientation == Qt::Vertical ? 1:-1) < 0) last();
        else first();
        break;
    default:
        e->ignore();
