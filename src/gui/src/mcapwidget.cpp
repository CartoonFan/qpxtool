...
MediaCapWidget::MediaCapWidget(QString itext, bool tri, quint64 df, QWidget *p,
                               Qt::WindowFlags f)
    : QWidget(p, f) {
  tristate = tri;
  text = itext;
...
QSize MediaCapWidget::sizeHint() const {
  return QSize(MAX_ICON_SIZE + 20 + fontMetrics().width(text),
               MAX_ICON_SIZE + 2);
}
...
void MediaCapWidget::setRW() {
  ...
  icon = icon.scaled(QSize(MAX_ICON_SIZE, MAX_ICON_SIZE), Qt::KeepAspectRatio,
...
void MediaCapWidget::paintEvent(QPaintEvent *) {
  QPainter p(this);
  if (cf)
    p.drawImage(4, ((height() - icon.height()) >> 1) + 2, icon);
  p.drawText(MAX_ICON_SIZE + 9, 4, width() - MAX_ICON_SIZE - 13, height() - 4,
