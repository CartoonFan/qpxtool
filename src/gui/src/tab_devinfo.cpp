...
#include "mcapwidget.hpp"

#include "tab_devinfo.hpp"
#include <QDebug>

#include "qpx_mmc_defs.hpp"

tabDevInfo::tabDevInfo(QPxSettings *iset, devlist *idev, QWidget *p,
                       Qt::WindowFlags fl)
    : QWidget(p, fl) {
#ifndef QT_NO_DEBUG
  ...
  for (int idx = 0;
    ...
    if (rw_capabilities[idx].id) {
      cap = new MediaCapWidget(rw_capabilities[idx].name, 1,
                               rw_capabilities[idx].id, this);
  ...
  for (int idx = 0; capabilities[idx].id || strlen(capabilities[idx].name);
    ...
    if (capabilities[idx].id) {
      cap = new MediaCapWidget(capabilities[idx].name, 0, capabilities[idx].id,
                               this);
...
void tabDevInfo::selectDevice() {
  ...
  clear();
  updateData();
  QObject::connect(devices->current(), SIGNAL(doneDInfo(int)), this,
                   SLOT(updateData(int)));
#ifndef QT_NO_DEBUG
...
void tabDevInfo::updateData(int xcode) {
  ...
  l_rpc_phase->setText((dev->rpc_phase < 0) ? "-"
                                            : QString::number(dev->rpc_phase));
  l_rpc_reg->setText((dev->rpc_reg < 0)
                         ? ((dev->rpc_reg == -2) ? "not set" : "-")
                         : QString::number(dev->rpc_reg));
