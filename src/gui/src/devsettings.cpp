...
devSettings::devSettings(QPxSettings *iset, device *idev, QWidget *p,
                         Qt::WindowFlags fl)
    : QDialog(p, fl) {
    dev = idev;
    set = iset;
    ...
    if (set->show_allctl || dev->features.supported & FEATURE_SILENT)
        ilist->addLabel(tr("Silent mode"), QImage(":images/sound.png"),
                        TAB_SILENT_PLEX);
    if (set->show_allctl || dev->features.supported & FEATURE_PIOQUIET)
        ilist->addLabel(tr("PioQuiet"), QImage(":images/sound.png"),
                        TAB_SILENT_PIO);
    if (set->show_allctl || dev->features.supported & FEATURE_DESTRUCT)
        ilist->addLabel(tr("Destruction"), QImage(":images/disc-eraser.png"),
                        TAB_DESTRUCT);
