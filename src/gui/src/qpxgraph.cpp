...
int errc_logh_hres[] = {1,  2,  3,   4,   5,   10,  20,  30,
                        40, 50, 100, 200, 300, 400, 500, 0};
...
QPxGraph::QPxGraph(QPxSettings *iset, devlist *idev, QString iname, int ttype,
...
void QPxGraph::setErrcShow(int err, bool disp) {
  if (disp)
    errcList |= err;
  else
    errcList &= ~err;
...
void QPxGraph::setRightMarginHidden(bool hide) {
  margin_right = hide ? 0 : MARGIN_DEFR;
}
...
int QPxGraph::getLastX() { return lastX + margin_left + 1; }
...
//	qDebug(QString("QPxGraph::wheelEvent(): %1").arg(e->delta()));
  ...
  if (e->modifiers() == Qt::NoModifier) {
    if (!scale[0] || scale[0]->type != Scale::Linear)
      return;
    if (e->delta() > 0)
      scaleIn(0);
    else if (e->delta() < 0)
    ...
    if (test != TEST_JB || !scale[1] || scale[1]->type != Scale::Linear)
      return;
    if (e->delta() > 0)
      scaleIn(1);
    else if (e->delta() < 0)
...
void QPxGraph::paintEvent(QPaintEvent *e) {
  ...
  if (!img)
    ...
    drawGraph(&ip, size(), devices->current(), test,
              e->rect().adjusted(lastX, 0, 0, 0));
  p.drawImage(0, 0, *img);
#else
  QPainter p(this);
...
void QPxGraph::drawGraph(QPainter *p, QSize s, device *dev, int ttype,
                         const QRect &rect, int eflags, bool forceRepaint) {
  int el = errcList;
  bool ss = showspeed;
  int taL = 0, taZ = 0;
  QSize sg;
  QRect grect =
      rect.intersected(geometry().adjusted(margin_left, 0, -margin_right,
                                           -margin_bottom))
          .translated(margin_left, 0);
  ...
  if (ttype == TEST_TA) {
    ...
    if (forceAll)
      p->fillRect(1, 1, width() - 2, height() - margin_bottom - 2,
                  QBrush(settings->col_bg));
  } else {
    sg = QSize(s.width() - (margin_left + margin_right + 2),
               s.height() - margin_bottom - 1);
    if (forceAll || margin_left > rect.x())
      p->fillRect(margin_left + 1, 1,
                  width() - (margin_left + margin_right + 2),
                  height() - margin_bottom - 1, QBrush(settings->col_bg));
    else
      p->fillRect(rect.x(), 1, width() - rect.x() - margin_left,
                  height() - margin_bottom - 1, QBrush(settings->col_bg));
...
void QPxGraph::drawTransfer(QPainter *p, const QSize &s, device *dev,
                            const QRect &rect, bool rw) {
  Q_UNUSED(rect);
  ...
  if (!rw && dev->testData.rt.size()) {
    ...
    if (!forceAll) {
      ...
      for (i = dev->testData.rt.size() - 1;
           i >= 0 && dev->testData.rt[i].lba > lastLBA;)
        i--;
      if (i > 0)
        startidx = i;
    }
  ...
  if (rw && dev->testData.wt.size()) {
    ...
    if (!forceAll) {
      lastLBA = (uint64_t)(lastX * HscaleLBA);
      for (i = dev->testData.wt.size() - 1;
           i >= 0 && dev->testData.wt[i].lba > lastLBA;)
        i--;
      if (i > 0)
        startidx = i;
    }
...
void QPxGraph::drawErrc(QPainter *p, const QSize &s, device *dev,
                        const QRect &) {
  QPainterPath pps;
  IntList xerr;
  //	bool isCD;
...
//	float M,D;
  ...
  if (forceAll) {
    ...
    for (i = dev->testData.errc.size() - 1;
         i >= 0 && dev->testData.errc[i].raw.lba > lastLBA;)
      i--;
    if (i > 0)
      startidx = i;
#ifndef QT_NO_DEBUG
    ...
    if ((1 << e) & errc) {
      ...
      for (i = startidx; i < dev->testData.errc.size(); i++) {
        ...
        if (x != xo || i == (dev->testData.errc.size() - 1)) {
          ...
          p->setPen(QPen(*settings->col_errc.raw[e], 1));
          if (settings->show_simplegraph) {
            p->drawLine(xo, errc2h(s.height(), 0), xo, errc2h(s.height(), max));
          } else {
            p->drawLine(xo, errc2h(s.height(), min), xo,
                        errc2h(s.height(), max));
          }
// P=0.95
#ifdef SHOW_P95ERRC
          if (!(settings->show_simplegraph)) {
            ...
            p->drawLine(xo, errc2h(s.height(), M - D), xo,
                        errc2h(s.height(), M + D));
            xerr.clear();
          }
#endif
...
//	float M,D;
  ...
  if (forceAll) {
    ...
    lastLBA = (uint64_t)(lastX * HscaleLBA);
    for (i = dev->testData.jb.size() - 1;
         i >= 0 && dev->testData.jb[i].lba > lastLBA;)
      i--;
    if (i > 0)
      startidx = i;
#ifndef QT_NO_DEBUG
  ...
  for (i = startidx; i < dev->testData.jb.size(); i++) {
    ...
    if (dev->testData.jb[i].jitter >= -100 &&
        dev->testData.jb[i].asymm >= -100) {
#ifdef SHOW_P95JB
    ...
    if (x != xo || i == (dev->testData.jb.size() - 1)) {
      ...
      p->drawLine(xo, jitter2h(s.height(), jmin), xo,
                  jitter2h(s.height(), jmax));
// P=0.95
#ifdef SHOW_P95JB
      M = xj.M();
      D = sqrt(xj.dispers(M));
      p->setPen(QPen(settings->col_jitter.darker(), 1));
      p->drawLine(xo, jitter2h(s.height(), (M - D) / 100.0), xo,
                  jitter2h(s.height(), (M + D) / 100.0));
      xj.clear();
#endif
      ...
      p->drawLine(xo, asymm2h(s.height(), bmin), xo, asymm2h(s.height(), bmax));
// P=0.95
#ifdef SHOW_P95JB
      M = xb.M();
      D = sqrt(xb.dispers(M));
      p->setPen(QPen(settings->col_asymm.darker(), 1));
      p->drawLine(xo, asymm2h(s.height(), (M - D) / 100.0), xo,
                  asymm2h(s.height(), (M + D) / 100.0));
      xb.clear();
#endif
      xo = x;
...
void QPxGraph::drawTA(QPainter *p, const QSize &s, device *dev, const QRect &) {
  ...
  for (int i = 0; i < dev->testData.ta[taIdx].size(); i++) {
    h = ta2h(s.height(), taMode ? dev->testData.ta[taIdx][i].land
                                : dev->testData.ta[taIdx][i].pit);
#if 0
        if (h>0) {
...
void QPxGraph::drawGrid(QPainter *p, const QSize &s, device *dev, int ttype) {
  ...
  p->drawRect(margin_left, 0, s.width() - (margin_left + margin_right) - 1,
              s.height() - margin_bottom);
  if (dev->media.type == "-")
    return;
  /*
  ...
  */
  ...
  if (isCD) {
    ...
    Hscale = (s.width() - (margin_left + margin_right) - 2) / 1.0 / GBperLayer /
             (1 << 19) / dev->media.ilayers;
    spdMax = 20;
  }
  ...
  for (int i = 1; i < VGRIDS; i++)
    p->drawLine(margin_left + (int)(i * HscaleX),
                s.height() - margin_bottom - 1,
  ...
  // capacity
  if (dev->media.creads) {
    p->drawLine(margin_left + (int)(Hscale * dev->media.creads) + 2,
                s.height() - margin_bottom - 1,
                margin_left + (int)(Hscale * dev->media.creads) + 2, 1);
  }
  p->drawLine(margin_left + (int)(Hscale * dev->media.ctots) + 2,
              s.height() - margin_bottom - 1,
  ...
  if (isCD) {
    for (int i = 1; i < VGRIDS; i++) {
      if (!(i & 1))
        p->drawText(margin_left + (int)(HscaleX * (i - 1)),
                    s.height() - margin_bottom + 2, (int)(HscaleX * 2),
                    margin_bottom - 2, Qt::AlignVCenter | Qt::AlignHCenter,
    ...
    for (int i = 1; i < VGRIDS; i++) {
      if (!(i & 1))
        p->drawText(
            margin_left + (int)(HscaleX * (i - 1)),
            s.height() - margin_bottom + 2, (int)(HscaleX * 2),
            margin_bottom - 2, Qt::AlignVCenter | Qt::AlignHCenter,
            QString::number(i * (dev->media.ilayers * GBperLayer / VGRIDS)) +
                " GB");
    }
  }
  ...
  case TEST_WT:
    for (int i = (isCD ? 4 : 2); i < spdMax; i += (isCD ? 4 : 2)) {
      ...
      p->drawLine(margin_left + 1, h, s.width() - margin_right - 2, h);
      if (!(i & (isCD ? 7 : 1))) {
        p->setPen(QPen(palette().color(QPalette::Text), 1));
...
//				qDebug(QString("scale step: %1").arg(step));
    ...
    if (!label.isEmpty()) {
      ...
      p->drawText(margin_left + 5, 4, 60, 20, Qt::AlignTop | Qt::AlignLeft,
                  label);
      p->setFont(dfont);
    }
    // error limit hlines
    ...
    for (int i = (isCD ? 4 : 2); i < spdMax; i += (isCD ? 4 : 2)) {
      ...
      if (!(i & (isCD ? 7 : 1))) {
        p->drawText(s.width() - margin_right + 2, h - 10, margin_right - 4, 20,
                    Qt::AlignVCenter | Qt::AlignLeft, QString::number(i));
      }
    }
  }
...
void QPxGraph::drawGridTA(QPainter *p, const QSize &s, device *dev, int ttype) {
  ...
  */
  ...
  p->setPen(QPen(palette().color(QPalette::Text), 1));
  for (int i = 3; i <= 11; i++) {
    p->drawText((int)(Hscale * (i - 2.5)), s.height() - margin_bottom + 2,
                (int)(Hscale), margin_bottom - 2,
                Qt::AlignVCenter | Qt::AlignHCenter, QString("T%1").arg(i));
  }
  if (dev->media.type.startsWith("DVD")) {
    p->drawText((int)(Hscale * (11.5)), s.height() - margin_bottom + 2,
                (int)(Hscale), margin_bottom - 2,
                Qt::AlignVCenter | Qt::AlignHCenter, "T14");
  }

  //	p->setPen(QPen( settings->col_bginv, 1));
  if (!taMode) {
    p->setPen(QPen(settings->col_tapit, 1));
    p->drawText(10, 10, 200, 30, Qt::AlignTop | Qt::AlignLeft,
...
void QPxGraph::contextMenuEvent(QContextMenuEvent *e) {
  ...
  cmenu->addAction((scale[0]->type == Scale::Log) ? icon_ok : QIcon(),
                   tr("Logarithmic scale"), this, SLOT(setScaleTypeLog()));
  cmenu->addAction((scale[0]->type == Scale::Linear) ? icon_ok : QIcon(),
                   tr("Linear Scale"), this, SLOT(setScaleTypeLin()));
  cmenu->addSeparator();

  /*
      cmenu->addAction(
              (settings->scales.get(name).policy == Scale::Auto) ?
     QIcon(":images/ok.png") : QIcon(), tr("Auto scale"), this,
     SLOT(setScalePolicyAuto())); cmenu->addAction(
              (settings->scales.get(name).policy == Scale::Fixed) ?
     QIcon(":images/ok.png") : QIcon(), tr("Fixed scale"), this,
     SLOT(setScalePolicyFixed())); cmenu->addSeparator();
  */
  act = cmenu->addAction(QIcon(":images/plus.png"), tr("Scale in"), this,
                         SLOT(scaleIn()));
  act->setEnabled(scale[0]->type == Scale::Linear);
  act = cmenu->addAction(QIcon(":images/minus.png"), tr("Scale out"), this,
                         SLOT(scaleOut()));
  act->setEnabled(scale[0]->type == Scale::Linear);
...
void QPxGraph::setScaleTypeLog() {
  if (!scale[0] || (scale[0]->type == Scale::Log))
    return;
...
void QPxGraph::scaleIn(int idx) {
  ...
  switch (test) {
  case TEST_ERRC:
    if (!scale[idx] || scale[idx]->value <= 64)
      return;
    scale[idx]->value /= 2;
  ...
  case TEST_JB:
    if (!idx) {
      if (!scale[idx] || scale[idx]->value <= 2)
        return; // jitter
    } else {
      if (!scale[idx] || scale[idx]->value <= 8)
        return; // asymmetry
    }
    scale[idx]->value /= 2;
    settings->scales.get(name[idx]).value = scale[idx]->value;
...
void QPxGraph::scaleOut(int idx) {
  ...
  switch (test) {
  case TEST_ERRC:
    if (!scale[idx] || scale[idx]->value >= 2048)
      return;
    scale[idx]->value *= 2;
    settings->scales.get(name[idx]).value = scale[idx]->value;
    break;
  case TEST_JB:
    if (!idx) {
      ...
      if (!scale[idx] || scale[idx]->value >= 32)
        return; // asymmetry
    }
    scale[idx]->value *= 2;
    settings->scales.get(name[idx]).value = scale[idx]->value;
...
int QPxGraph::jitter2h(int h, float val) {
