...
PreviewView::PreviewView(QWidget *p, QPrinter *printer, QTextDocument *idoc,
                         AbstractPreview::PreviewMode mode)
...
void PreviewView::paintPage(QPainter *painter, int page, const QRect &rect) {
    ...
    QRectF docRect(QPointF(0, page * pgSize.height()) + rect.topLeft(),
                   rect.size());
    ...
    qDebug() << QString("Full page time: %1")
...
PrintPreview::PrintPreview(QWidget *parent, QTextDocument *document)
    ...
    connect(box_scale, SIGNAL(currentIndexChanged(QString)), this,
...
void PrintPreview::printDoc(QWidget *parent, QPrinter *printer,
                            QTextDocument *doc) {
