...
QPxSettings &QPxSettings::operator=(const QPxSettings &o) {
  ...
  for (int i = 0; i < 8; i++)
    *col_errc.raw[i] = *o.col_errc.raw[i];
...
void QPxSettings::load() {
  ...
  col_bginv = QColor((~col_bg.red()) & 0xFF, (~col_bg.green()) & 0xFF,
                     (~col_bg.blue()) & 0xFF);
  ...
  for (int i = 0; i < 8; i++)
    *col_errc.raw[i] =
        settings->value(QString("errc%1").arg(i), defColors.errc[i].rgb())
            .toInt();
...
void QPxSettings::setDefaultColors() {
  ...
  for (int i = 0; i < 8; i++)
    *col_errc.raw[i] = defColors.errc[i];
