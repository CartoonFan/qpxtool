...
prefColors::prefColors(QPxSettings *iset, QWidget *p, Qt::WindowFlags fl)
    ...
    connect(e_preset, SIGNAL(textChanged(QString)), this,
            SLOT(presetNameValidate(QString)));
...
prefColors::~prefColors() {
    ...
    set->col_bginv =
        QColor((~set->col_bg.red()) & 0xFF, (~set->col_bg.green()) & 0xFF,
               (~set->col_bg.blue()) & 0xFF);
...
void prefColors::loadPresets() {
    ...
    for (int p = 0; p < list.size(); p++) {
        ...
        for (int i = 0; i < 8; i++)
            newSet.errc[i] =
                settings->value(QString("errc%1").arg(i), defColors.errc[i]).toInt();
