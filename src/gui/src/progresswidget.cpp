...
ProgressWidget::ProgressWidget(int iblocks, int ishown, QWidget *p)
  ...
  setGeometry(offs.x() + p->width() / 2 - 150, offs.y() + p->height() / 2 - 30,
              300, 60);
  setMinimumSize(300, 60);
  setMaximumSize(300, 60);
}

ProgressWidget::~ProgressWidget() { timer.stop(); }
...
void ProgressWidget::setDirection(ProgressWidget::Direction idir) {
  dir = idir;
}
void ProgressWidget::setText(QString t) {
  text = t;
  setWindowTitle(text);
}
...
void ProgressWidget::paintEvent(QPaintEvent *) {
  ...
  if (text.isEmpty()) {
    for (int i = 0; i < shown; i++)
      p.fillRect(offs + i * blkw, (height() - blkw) >> 1, (int)(blkw * 0.8),
                 (int)(blkw * 0.8), QBrush(col_rect));
  } else {
    ...
    p.drawText(0, 0, width(), height() >> 1,
               Qt::AlignHCenter | Qt::AlignVCenter, text);
    for (int i = 0; i < shown; i++)
      p.fillRect(offs + i * blkw, height() >> 1, (int)(blkw * 0.8),
                 (int)(blkw * 0.8), QBrush(col_rect));
  }
}
