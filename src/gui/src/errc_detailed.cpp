...
#include "errc_detailed.hpp"
#include <QDebug>

static const char *labels_cd[8] = {"BLER", "E11", "E21", "E31",
                                   "E12",  "E22", "E32", "UNCR"
                                  };

static const char *labels_dvd[8] = {"",    "PIE", "PI8", "PIF",
                                    "POE", "PO8", "POF", "UNCR"
                                   };
...
ErrcDetailedDialog::ErrcDetailedDialog(QPxSettings *iset, devlist *idev,
...
void ErrcDetailedDialog::updateGraphs(device *idev) {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        l_avg[i]->setText(
            QString::number(dev->testData.errcAVG.raw.err[i], 'f', 2));
        //	graph[i]->setErrcList(1<<i, labels[i]);
        graph[i]->update();
        //		} else {
        //			graph[i]->hide();
