...
DbReportSelection::DbReportSelection(QString conn, QWidget *parent,
                                     Qt::WindowFlags fl)
    : QDialog(parent, fl) {
  report_id = -1;
...
void DbReportSelection::winit() {
  ...
  */
  ...
  connect(list, SIGNAL(clicked(const QModelIndex)), this,
...
void DbReportSelection::search() {
  ...
  while (q->next()) {
    ...
    if (!q->value(3).toInt())
      ...
      model->addRow(q->value(0).toInt(),
                    q->value(2).toString() + ", " + tr("copy") + " #" +
                        q->value(3).toString(),
                    icon);
  }
  list->setModel(model);
...
void DbReportSelection::load() { accept(); }
...
void DbReportSelection::itemActivated(const QModelIndex idx) {
  ...
  info->document()->addResource(QTextDocument::ImageResource,
                                ...
                                getMediaLogo(q->value(1).toULongLong())
  ...
  html += q->value(2).toString() + ", " + tr("copy") + "# " +
          q->value(7).toString() + "<br>";
  if (!md5.isEmpty()) {
    html += "MD5: " + md5;
  }
  html += "</td></tr>";
  ...
  html += "<tr><td bgcolor=#E0E0E0>" + tr("Device") + "</td><td>" +
          ...
          q->value(5).toString() + "</td></tr>";
  html += "<tr><td bgcolor=#E0E0E0>" + tr("Date") + "</td><td>" +
          q->value(6).toDateTime().toString("yyyy.MM.dd  hh:mm") + "</td></tr>";
...
bool DbReportSelection::eventFilter(QObject *o, QEvent *e) {
  if (o == elabel) {
    if (e->type() == QEvent::Resize) {
      int frameWidth =
          elabel->style()->pixelMetric(QStyle::PM_DefaultFrameWidth);
      elabel_clear->resize(elabel->height() - (frameWidth << 1),
                           elabel->height() - (frameWidth << 1));
      elabel_clear->move(elabel->width() - elabel_clear->width() - frameWidth,
                         frameWidth);
      /*
                          elabel_search->resize(elabel->height() -
         (frameWidth<<1), elabel->height() - (frameWidth<<1));
                          elabel_search->move(elabel->width() -
         elabel_clear->width() - frameWidth, frameWidth);
                          elabel_clear->resize(elabel->height() -
         (frameWidth<<1), elabel->height() - (frameWidth<<1));
                          elabel_clear->move(elabel->width() -
         elabel_clear->width() - elabel_search->width() - frameWidth - 10,
         frameWidth); elabel->setStyleSheet(QString("QLineEdit { padding-right:
         %1px; }").arg(elabel_clear->width()+elabel_search->width()+10));
      */
      elabel->setStyleSheet(QString("QLineEdit { padding-right: %1px; }")
                                .arg(elabel_clear->width() + 1));
      return true;
    } else {
      return false;
    }
