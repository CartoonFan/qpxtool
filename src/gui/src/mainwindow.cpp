...
#include "printpreview.hpp"
#include "progresswidget.hpp"
#include "qpxiodevice.hpp"
...
const QString errcNameCD[8] = {"BLER", "E11", "E21", "E31",
                               "E12",  "E22", "E32", "UNCR"
                              };
const QString errcNameDVD[8] = {"res", "PIE", "PI8", "PIF",
                                "POE", "PO8", "POF", "UNCR"
                               };
const QString errcNameBD[8] = {"res", "LDC", "res", "res",
                               "BIS", "res", "res", "UNCR"
                              };
...
static struct option long_options[] = {
    {"load", 1, NULL, 'l'}, {"help", 0, NULL, 'h'}, {0, 0, 0, 0}
};
...
QPxToolMW::QPxToolMW(int ac, char **av, QWidget *p, Qt::WindowFlags fl)
    ...
    while (1) {
        ...
        switch (c) {
        case 'h':
            std::cout << "qpxtool options:\n";
            std::cout << "\t-l FILE\tload data from file\n";
...
void QPxToolMW::winit() {
    ...
    */
    ...
    connect(act_sb, SIGNAL(toggled(bool)), mwidget,
            SLOT(setSidebarVisible(bool)));
...
void QPxToolMW::create_actions() {
    ...
    act_scanbus = new QAction(QIcon(":images/refresh.png"),
                              tr("Rescan local bus and network"), this);
    act_minfo = new QAction(QIcon(":images/refresh-media.png"),
                            tr("Update media info"), this);
    act_devctl =
        new QAction(QIcon(":images/device.png"), tr("Device controls"), this);
    ...
    act_pref =
        new QAction(QIcon(":images/settings.png"), tr("Preferences"), this);
    act_pref->setShortcut(QKeySequence("Ctrl+C"));
    act_print =
        new QAction(QIcon(":images/printer.png"), tr("Print test results"), this);
    act_print->setShortcut(QKeySequence("Ctrl+P"));
    act_report =
        new QAction(QIcon(":images/pdf.png"), tr("Export results to PDF"), this);
    act_export = new QAction(QIcon(":images/html.png"),
                             tr("Export results to HTML"), this);
    act_save =
        new QAction(QIcon(":images/save.png"), tr("Save results..."), this);
    act_save->setShortcut(QKeySequence("Ctrl+S"));
    act_load =
        new QAction(QIcon(":images/fileopen.png"), tr("Load results..."), this);
    act_load->setShortcut(QKeySequence("Ctrl+O"));
    act_save_db = new QAction(QIcon(":images/db_save.png"),
                              tr("Save results (DB)..."), this);
    act_load_db = new QAction(QIcon(":images/db_load.png"),
                              tr("Load results (DB)..."), this);
    ...
    connect(act, SIGNAL(triggered()), this, SLOT(selectTab()));
    act =
        act_sbgrp->addAction(QIcon(":images/test_errc.png"), "Error Correction");
    act_sblist << act;
    act->setCheckable(true);
...
void QPxToolMW::select_tests() {
    device *dev;
    if (!devices.size()) {
        QMessageBox::warning(this, tr("No devices"),
                             tr("No devices found!\nCan't run tests"));
        return;
    }
    dev = devices.current();
...
void QPxToolMW::select_tests(device *dev) {
    ...
    if (!dev->mutex->tryLock()) {
        mutex_dev.unlock();
        QMessageBox::warning(this, tr("Device busy"),
                             tr("It seems device already performing test\nor info "
...
void QPxToolMW::tests_done() {
    ...
    if (set.report_autosave || set.report_autosave_db) {
        dev->mutex->lock();
        if (set.report_autosave)
            autosave_report(dev);
        if (set.report_autosave_db)
            save_results_db(dev);
        dev->mutex->unlock();
        if (set.actions_flags & AFLAG_EJECT_AFTER)
            dev->setFeature(FEATURE_LOEJ, 0);
    }
...
void QPxToolMW::scanbus() {
    ...
    if (!mutex_dev.tryLock()) {
        QMessageBox::information(this, tr("Device busy"),
                                 tr("Can't scan bus: some device(s) busy!"));
        return;
    }
    if (scanbusio->IODevice()) {
        mutex_dev.unlock();
        return;
    }
    ...
    disconnect(c_dev, SIGNAL(currentIndexChanged(int)), this,
               SLOT(setDevice(int)));
...
void QPxToolMW::scanbus_local() {
    ...
    connect(scanbusio, SIGNAL(readyReadLine()), this,
            SLOT(qscan_process_scanbus()));
    ...
    disconnect(proc);
    disconnect(scanbusio, SIGNAL(readyReadLine()), this,
               SLOT(qscan_process_scanbus()));
    scanbusio->setIODevice(NULL);
    delete proc;
scanbus_local_end:
...
void QPxToolMW::scanbus_remote() {
    ...
    for (int i = 0; i < hosts_cnt; i++) {
        ...
        if (!dport)
            dport = 46660;
#ifndef QT_NO_DEBUG
        qDebug() << "Trying host " << i + 1 << "/" << hosts_cnt << " [" << dhost
                 << ":" << dport << "]";
#endif
        // scanning host...
        ...
        if (!sock->waitForConnected(5000)) {
            ...
            QMessageBox::warning(
                this, tr("Warning"),
                tr("Unable to connect to qscand at %1:%2").arg(dhost).arg(dport));
            if (!splash)
                progress->show();
            continue;
        }
...
#endif
    if (!scanbusio->IODevice())
        goto scanbus_process_qscan_end;
...
void QPxToolMW::setDevice(int idx) {
...
#endif
    if (idx < 0 || idx >= devices.size() || devices.idx() == idx)
        return;
    ...
    devact_en =
        (dev->type == device::DevtypeLocal) || (dev->type == device::DevtypeTCP);
        ...
        if (dev->plextor_lock) {
            QMessageBox::warning(this, tr("Warning"),
                                 ...
                                 tr("Found locked Plextor device!\nQuality check "
                                    "and some other features will not work"));
        }
    }
    if (dev->media.type == "-") {
...
void QPxToolMW::mediaUpdated(int) {
    ...
    if (testDialog)
        testDialog->mediaChanged();

    if (!testDialog) {
        if (dev->media.type == "-")
            return;
        if (((set.actions_flags & AFLAG_AUTOSTART_W) && dev->media.creads) ||
                ((set.actions_flags & AFLAG_AUTOSTART_B) &&
                 (!dev->media.creads || dev->media.type.startsWith("DVD+RW") ||
...
void QPxToolMW::update_device_info() {
    devices.current()->update_device_info();
}
void QPxToolMW::update_media_info() {
    update_media_info(devices.current());
}
...
void QPxToolMW::device_settings() {
    ...
    connect(dev, SIGNAL(doneGetFeatures(int)), this,
            SLOT(update_features_done(int)));
    ...
    delete progress;
    disconnect(dev, SIGNAL(doneGetFeatures(int)), this,
               SLOT(update_features_done(int)));
...
void QPxToolMW::save_report() {
    ...
    QString fname;
    QFileInfo finfo;
    fname = QFileDialog::getSaveFileName(this, tr("Export results to PDF"),
                                         set.last_res_path_pdf,
                                         "PDF files (*pdf *.PDF)");
    if (fname.isEmpty())
        return;
...
#warning !!! hardcoded report name structure
    ...
    if (!dir.exists(fpath) && !dir.mkpath(fpath)) {
        QMessageBox::warning(this, tr("Can't save report"),
                             tr("Direcrory not exists and I can't create it") +
                             ":\n" + fpath);
        return;
    }
...
void QPxToolMW::prepare_report(device *dev, QTextDocument *doc) {
    ...
    if (dev->testData.errc.size() > 0) {
        ...
        doc->addResource(QTextDocument::ImageResource, QUrl("images/errc.png"),
                         img);
        ...
        if (dev->media.type.startsWith("CD")) {
            for (int i = 0; i < 8; i++) {
                if (!dev->media.tdata_errc || (dev->media.tdata_errc & (1 << i))) {
                    ...
                    doc->addResource(QTextDocument::ImageResource,
                                     QUrl("images/" + errcNameCD[i] + ".png"), img);
                }
                qApp->processEvents();
            }
            ...
            for (int i = 1; i < 8; i++) {
                if (!dev->media.tdata_errc || (dev->media.tdata_errc & (1 << i))) {
                    ...
                    mwidget->drawGraph(img, dev, TEST_ERRC, (1 << i));
                    doc->addResource(QTextDocument::ImageResource,
                                     QUrl("images/" + errcNameDVD[i] + ".png"), img);
                }
                qApp->processEvents();
            }
            ...
            int residx[] = {1, 4, 7};
            for (int i = 0; i < 3; i++) {
                if (!dev->media.tdata_errc ||
                        (dev->media.tdata_errc & (1 << residx[i]))) {
                    img = QImage(HTML_GRAPH_W, HTML_GRAPH_H, QImage::Format_ARGB32);
                    mwidget->drawGraph(img, dev, TEST_ERRC, (1 << residx[i]));
                    doc->addResource(QTextDocument::ImageResource,
                                     QUrl("images/" + errcNameBD[residx[i]] + ".png"),
                                     img);
                }
                qApp->processEvents();
            }
    ...
    if (dev->test_req & TEST_TA) {
        for (int l = 0; l < dev->media.ilayers; l++) {
            for (int z = 0; z < 3; z++) {
                ...
                doc->addResource(QTextDocument::ImageResource,
                                 QUrl(QString("images/ta%1_%2_p.png").arg(l).arg(z)),
                                 img);
                ...
                doc->addResource(QTextDocument::ImageResource,
                                 QUrl(QString("images/ta%1_%2_l.png").arg(l).arg(z)),
                                 img);
...
void QPxToolMW::export_results() {
    ...
    fname = QFileDialog::getSaveFileName(this, tr("Export results to HTML"),
                                         set.last_res_path_html,
                                         "HTML files (*.html)");
    if (fname.isEmpty())
        return;
...
QString QPxToolMW::generate_html(device *dev, QString idir) {
    ...
    if (dev->host.isEmpty()) {
        ...
        r += "\t\t\t<td width=70\%>" + dev->path + "@" + dev->host + ":" +
             QString::number(dev->port) + "</td>\n";
    }
    r += "\t\t</tr>\n";
    r += "\t\t<tr>\n";
    r += "\t\t\t<td>Device ID</td>\n";
    ...
    r += "\t\t<tr>\n";
    r += "\t\t\t<td width=30\%></td>\n";
    r += "\t\t\t<td width=70\%>TLA# " + dev->tla + "<br>Serial# " + dev->sn +
         "</td>\n";
    r += "\t\t</tr>\n";
    r += "\t\t<tr>\n";
    ...
    r += "\t\t\t<td>" + QString::number(dev->media.creadm) + "MB (" +
         QString::number(dev->media.creads) + " sectors)</td>\n";
    r += "\t\t</tr>\n";
    r += "\t\t<tr>\n";
    r += "\t\t\t<td>Capacity (total):</td>\n";
    r += "\t\t\t<td>" + QString::number(dev->media.ctotm) + "MB (" +
         QString::number(dev->media.ctots) + " sectors)</td>\n";
    r += "\t\t</tr>\n";
    r += "\t</table>\n";
    r += "\t<br>\n";
    r += "<div>\n";
    ...
    if (dev->testData.rt.size() > 0) {
        r += "<p align=\"center\">\n";
        r += "\t<b>Read Transfer @ " + QString::number(dev->tspeeds.rt) +
             "X</b><br>\n";
        ...
        r += QString("\t<tr bgcolor=#e0e0e0><td align=right>%1 X</td><td "
                     "align=right>%2 X</td><td align=right>%3 X</td></tr>\n")
             .arg(dev->testData.rt.first().spdx)
             .arg(dev->testData.rt.last().spdx)
             .arg(dev->testData.rt.last().lba * 2 / dev->testData.rt_time /
                  dev->media.spd1X,
                  0, 'f', 2);
        r += QString("\t<tr bgcolor=#c0c0c0><td align=right>%1 kB/s</td><td "
                     "align=right>%2 kB/s</td><td align=right>%3 kB/s</td></tr>\n")
             .arg(dev->testData.rt.first().spdk)
             .arg(dev->testData.rt.last().spdk)
             .arg(dev->testData.rt.last().lba * 2 / dev->testData.rt_time, 0,
                  'f', 2);
        ...
        r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td "
             "align=\"center\">";
        r += "<img src=\"" + idir +
             "/rt.png\" alt=\"Read Transfer\" width = " HTML_GRAPH_WQ
             " height = " HTML_GRAPH_HQ ">";
        //		r+= "<br>Read Transfer";
        r += "</td></tr>\n</table><br>\n";
    }
    ...
    if (dev->testData.wt.size() > 0) {
        r += "<p align=\"center\">\n";
        r += "\t<b>Write Transfer @ " + QString::number(dev->tspeeds.wt) +
             "X</b><br>\n";
        r += QString("\tTest time: %1:%2<br>\n")
        ...
        r += QString("\t<tr bgcolor=#e0e0e0><td align=right>%1 X</td><td "
             ...
             .arg(dev->testData.wt.last().lba * 2 / dev->testData.rt_time /
                  dev->media.spd1X,
                  0, 'f', 2);
        r += QString("\t<tr bgcolor=#c0c0c0><td align=right>%1 kB/s</td><td "
                     "align=right>%2 kB/s</td><td align=right>%3 kB/s</td></tr>\n")
             ...
             .arg(dev->testData.wt.last().lba * 2 / dev->testData.rt_time, 0,
                  'f', 2);
        ...
        r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td "
             "align=\"center\">";
        r += "<img src=\"" + idir +
             "/wt.png\" alt=\"Write Transfer\" width = " HTML_GRAPH_WQ
             " height = " HTML_GRAPH_HQ ">";
        //		r+= "<br>Write Transfer";
        r += "</td></tr>\n</table><br>\n";
    }
    ...
    if (dev->testData.errc.size() > 0) {
        r += "<p align=\"center\">\n";
        r += "\t<b>Error Correction @ " + QString::number(dev->tspeeds.errc) +
             "X</b><br>\n";
        r += QString("\tTest time: %1:%2<br>\n")
        ...
        if (dev->media.type.startsWith("CD")) {
            for (int i = 0; i < 8; i++) {
                if (!dev->media.tdata_errc || (dev->media.tdata_errc & (1 << i))) {
                    ...
                    r += QString(
                             "\t\t<tr bgcolor=%1><td>%2</td><td align=right>%3</td><td "
        ...
        r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td "
             "align=\"center\">";
        r += "<img src=\"" + idir +
             "/errc.png\" alt=\"Error correction\" width = " HTML_GRAPH_WQ
             " height = " HTML_GRAPH_HQ ">\n";
        ...
        if (dev->media.type.startsWith("CD")) {
            for (int i = 0; i < 8; i++) {
                if (!dev->media.tdata_errc || (dev->media.tdata_errc & (1 << i))) {
                    r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td "
                         "align=\"center\">";
                    r += "\t<img src=\"" + idir + "/" + errcNameCD[i] +
                         ".png\" width = " HTML_GRAPH_WQ " height = " HTML_GRAPH_HQ ">";
                    r += "<br>" + errcNameCD[i];
                    r += "</td></tr>\n</table><br>\n";
                }
            }
            ...
            for (int i = 0; i < 3; i++) {
                if (!dev->media.tdata_errc ||
                    ...
                    r += "<table border=0 cellspacing=0 align=\"center\">\n\t<tr><td "
                         "align=\"center\">";
                    r += "\t<img src=\"" + idir + "/" + errcNameBD[residx[i]] +
                         ".png\" width = " HTML_GRAPH_WQ " height = " HTML_GRAPH_HQ ">";
    ...
    if (dev->testData.jb.size() > 0) {
        r += "<p align=\"center\">\n";
        r += "\t<b>Jitter/Asymmetry @ " + QString::number(dev->tspeeds.jb) +
             "X</b><br>\n";
        r += QString("\tTest time: %1:%2<br>\n")
        ...
        r += QString("\t\t<tr "
                     "bgcolor=#d0d0d0><td>Jitter</td><td>%1</td><td>%2</td></tr>")
             .arg(dev->testData.jbMM.jmin)
             .arg(dev->testData.jbMM.jmax);
        r +=
            QString("\t\t<tr "
                    "bgcolor=#c0c0c0><td>Asymmetry</td><td>%1</td><td>%2</td></tr>")
        ...
        r += "<table border=0 cellspacing=0 align=center>\n\t<tr><td "
             "align=center>\n";
        r += "<img src=\"" + idir +
             "/jb.png\" width = " HTML_GRAPH_WQ " height = " HTML_GRAPH_HQ ">";
        r += "</td></tr>\n</table><br>\n";
    }

    if (dev->testData.ft.size() > 0) {
        r += "<p align=\"center\">\n";
        r += "\t<b>Focus/Tracking Errors @ " + QString::number(dev->tspeeds.ft) +
             "X</b><br>\n";
        r += QString("\tTest time: %1:%2<br>\n")
        ...
        r += "\t\t<tr bgcolor=#c0c0c0><td>FE Max</td><td>" +
             QString::number(dev->testData.ftMAX.fe) + "</td></tr>";
        r += "\t\t<tr bgcolor=#d0d0d0><td>TE Max</td><td>" +
             QString::number(dev->testData.ftMAX.te) + "</td></tr>";
        r += "\t</table>\n";
        r += "<br>\n";
        r += "<table border=0 cellspacing=0 align=center>\n\t<tr><td "
             "align=center>\n";
        r += "<img src=\"" + idir +
             "/ft.png\" width = " HTML_GRAPH_WQ " height = " HTML_GRAPH_HQ ">";
        r += "</td></tr></table><br>\n";
    }
...
#warning TA analysis data
        ...
        for (int l = 0; l < dev->media.ilayers; l++) {
            for (int z = 0; z < 3; z++) {
                r += "<table border=0 cellspacing=0 align=center>\n\t<tr><td "
                     "align=center>";
                r += "<img src=\"" + idir + "/ta" + QString::number(l) + "_" +
                     QString::number(z) +
                     "_p.png\" width = " HTML_GRAPH_WQ " height = " HTML_GRAPH_HQ2 ">";
                r += "</td></tr>\n\t<tr><td align=\"center\">";
                r += "<img src=\"" + idir + "/ta" + QString::number(l) + "_" +
                     QString::number(z) +
                     "_l.png\" width = " HTML_GRAPH_WQ " height = " HTML_GRAPH_HQ2 ">";
                r += "</td></tr>\n</table><br>\n";
            }
        }
    }
    ...
    r += "\t<a "
         "href=\"http://qpxtool.sourceforge.net\">http://"
         "qpxtool.sourceforge.net</a><br>\n";
    r += "</p>\n";
    r += "</body>\n</html>\n";
    return r;
}
...
void QPxToolMW::save_results() {
    ...
    fname = QFileDialog::getSaveFileName(this, tr("Save results to file..."),
                                         set.last_res_path_native,
                                         "QPxTool data files (*.qpx)");
    if (fname.isEmpty())
        return
    ...
    if (!dev->saveResult()) {
        ...
        QMessageBox::information(
            this, tr("Info"), tr("Tests data saved in %1 sec").arg(t, 0, 'f', 2));
#endif
    }
    f.close();
}
...
#warning !!! set media label before saving to database
    ...
    // check&update model
    q->prepare(
        "SELECT id FROM dev_models WHERE name=:name AND id_vendor=:id_vendor");
    ...
    if (!q->next()) {
        //		q->prepare("INSERT INTO dev_models (id_vendor,name) VALUES
        //("+QString::number(id_vendor)+",'"+dev->dev+"') RETURNING id");
        q->prepare("INSERT INTO dev_models (id_vendor,name) VALUES "
                   "(:id_vendor,:name) RETURNING id");
        q->bindValue(":id_vendor", id_vendor);
        q->bindValue(":name", dev->dev);
        if (!q->exec()) {
            qDebug() << q->lastError().text();
...
void QPxToolMW::load_results() {
    ...
    if (!devices.size())
        return;
    fname = QFileDialog::getOpenFileName(this, tr("Load results from file..."),
                                         set.last_res_path_native,
                                         "QPxTool data files (*.qpx)");
    if (fname.isEmpty())
        return;
...
void QPxToolMW::load_results(QString fname) {
    ...
    if (!dev->loadResult()) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("Do tests data found in this file!"));
    } else {
        c_dev->setCurrentIndex(0);
#ifndef QT_NO_DEBUG
        gettimeofday(&te, NULL);
        double t = (te.tv_sec - tb.tv_sec) + (te.tv_usec - tb.tv_usec) / 1000000.0;
        QMessageBox::information(
            this, tr("Info"), tr("Tests data loaded in %1 sec").arg(t, 0, 'f', 2));
#endif
    }
    f.close();
    //	setDevice(0);
...
void QPxToolMW::load_results_db() {
    ...
    if (!dev->loadResult()) {
        QMessageBox::warning(this, tr("Warning"),
                             tr("No tests data found in this file!"));
    } else {
        c_dev->setCurrentIndex(0);
#ifndef QT_NO_DEBUG
        gettimeofday(&te, NULL);
