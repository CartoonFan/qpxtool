...
tabTA::tabTA(QPxSettings *iset, devlist *idev, QString iname, QWidget *p,
             Qt::WindowFlags fl)
    : QWidget(p, fl) {
#ifndef QT_NO_DEBUG
  qDebug("STA: tabTA()");
...
// Legend
  ...
  connect(grp_layer, SIGNAL(buttonClicked(int)), graphPit,
          SLOT(setLayerTA(int)));
  connect(grp_layer, SIGNAL(buttonClicked(int)), graphLand,
          SLOT(setLayerTA(int)));

  connect(grp_zone, SIGNAL(buttonClicked(int)), graphPit, SLOT(setZoneTA(int)));
  connect(grp_zone, SIGNAL(buttonClicked(int)), graphLand,
          SLOT(setZoneTA(int)));
