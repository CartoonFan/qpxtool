...
tabMediaInfo::tabMediaInfo(QPxSettings *iset, devlist *idev, QWidget *p,
                           Qt::WindowFlags fl)
    : QWidget(p, fl) {
#ifndef QT_NO_DEBUG
  ...
  minfo_detail =
      new QTreeWidgetItem(minfo, QStringList() << tr("Detailed info") << "");
...
void tabMediaInfo::selectDevice() {
  ...
  updateData();
  QObject::connect(devices->current(), SIGNAL(doneMInfo(int)), this,
                   SLOT(updateData(int)));
#ifndef QT_NO_DEBUG
