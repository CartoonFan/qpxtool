...
void ResultsReader::run() {
    ...
    QString name;
    QStringList data;
    bool dataFound = 0, isQpxData = 0, isDevice = 0, isMedia = 0, isTests = 0;
    int test = 0;
    int taZone = -1;
    ...
    while (!xml.atEnd()) {
            ...
            if (isQpxData) {
                if (test && (xml.name() == "d")) {
                    ...
                    case TEST_ERRC:
                        if (data.size() == 10) {
                            ...
                            if (dev->media.type.startsWith("CD")) {
                                CDErrcAVG(&dev->testData.errcAVG, &dev->testData.errcTOT,
                                          d_errc.cd.lba / 75);
                            } else if (dev->media.type.startsWith("DVD")) {
                                DVDErrcAVG(&dev->testData.errcAVG, &dev->testData.errcTOT,
                                           d_errc.dvd.lba >> 4);
                            } else if (dev->media.type.startsWith("BD")) {
                                BDErrcAVG(&dev->testData.errcAVG, &dev->testData.errcTOT,
                                          d_errc.bd.lba >> 5);
                            }
#endif
                    ...
                    case TEST_JB:
                        if (data.size() == 4) {
                            ...
                            if (!dev->testData.jb.size()) {
                                ...
                                if (dev->testData.jbMM.jmin > d_jb.jitter)
                                    dev->testData.jbMM.jmin = d_jb.jitter;
                                if (dev->testData.jbMM.jmax < d_jb.jitter)
                                    dev->testData.jbMM.jmax = d_jb.jitter;
                                if (dev->testData.jbMM.bmin > d_jb.asymm)
                                    dev->testData.jbMM.bmin = d_jb.asymm;
                                if (dev->testData.jbMM.bmax < d_jb.asymm)
                                    dev->testData.jbMM.bmin = d_jb.asymm;
                            }
#endif
...
#if RECALC_ON_LOAD
                            if (dev->testData.ftMAX.fe < d_ft.fe)
                                dev->testData.ftMAX.fe = d_ft.fe;
                            if (dev->testData.ftMAX.te < d_ft.te)
                                dev->testData.ftMAX.te = d_ft.te;
#endif
                    ...
                    if (xml.name() == "identify") {
                        ...
                        dev->media.rspeeds = attr.value("rspeeds").toString().split(
                                                 " ", QString::SkipEmptyParts);
                        dev->media.wspeedsd = attr.value("wspeedsd")
                                              .toString()
                                              .split(" ", QString::SkipEmptyParts);
                        dev->media.wspeedsm = attr.value("wspeedsm")
                                              .toString()
                                              .split(" ", QString::SkipEmptyParts);
        ...
        case QXmlStreamReader::Invalid:
            qDebug() << COL_RED << "ResultsReader:  Invalid token @"
                     << xml.characterOffset() << xml.name() << COL_NORM;
            break;
...
void ResultsWriter::run() {
    ...
    if (dev->testData.rt.size()) {
        ...
        xml.writeAttribute("time",
                           QString("%1").arg(dev->testData.rt_time, 0, 'f', 2));
    ...
    if (dev->testData.wt.size()) {
        ...
        xml.writeAttribute("time",
                           QString("%1").arg(dev->testData.wt_time, 0, 'f', 2));
    ...
    if (dev->testData.errc.size()) {
        ...
        xml.writeAttribute("time",
                           QString("%1").arg(dev->testData.errc_time, 0, 'f', 2));
        xml.writeAttribute("data", QString::number(dev->media.tdata_errc));

        for (int i = 0; i < s; i++) {
            xml.writeTextElement(
                "d", QString("%1,%2,")
                .arg(dev->testData.errc[i].raw.lba)
                .arg(dev->testData.errc[i].raw.spdx, 0, 'f', 2) +
    ...
    if (dev->testData.jb.size()) {
        ...
        xml.writeAttribute("time",
                           QString("%1").arg(dev->testData.jb_time, 0, 'f', 2));
    ...
    if (dev->testData.ft.size()) {
        ...
        xml.writeAttribute("time",
                           QString("%1").arg(dev->testData.ft_time, 0, 'f', 2));
    ...
    if (dev->testData.ta[0].size()) {
        ...
        xml.writeAttribute("time",
                           QString("%1").arg(dev->testData.ta_time, 0, 'f', 2));
