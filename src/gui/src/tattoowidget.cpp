...
TattooWidget::TattooWidget(QWidget *p) : QWidget(p) {
  r0mm = F1TATTOOR / 3;
  r1mm = F1TATTOOR;
}
...
float TattooWidget::SRC_x2ang(int x, int w) {
  return (float)((double)x * M_PI * 2 / w);
}
float TattooWidget::SRC_y2rad(int y, int h) {
  return (float)y / (float)h * (r1p - r0p + 1) + r0p;
}

void TattooWidget::resizeEvent(QResizeEvent *) { convertImage(); }
...
void TattooWidget::paintEvent(QPaintEvent *) {
  ...
  // image
  if (dimg.height())
    p.drawImage(x0 - r1p, y0 - r1p, dimg);
