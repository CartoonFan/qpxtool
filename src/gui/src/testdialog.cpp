...
TestDialog::TestDialog(QPxSettings *iset, device *idev, QWidget *p,
                       Qt::WindowFlags f)
    : QDialog(p, f) {
  settings = iset;
  dev = idev;
  ...
  if (!dev->media.creads || dev->media.type.startsWith("DVD+RW")) {
    dev->WT_simul =
        (settings->actions_flags & AFLAG_DTEST_WT_SIMUL) ? !!noSimul : 0;
  }
  if (dev->media.creads) {
    dev->test_req |= (settings->actions_flags & AFLAG_DTEST_RT) ? TEST_RT : 0;
    dev->test_req |=
        (settings->actions_flags & AFLAG_DTEST_ERRC) ? TEST_ERRC : 0;
...
void TestDialog::checkSimul() {
  ...
  if ((dev->media.type.startsWith("CD-") && (dev->cap & CAP_TEST_WRITE_CD)) ||
      (dev->media.type.startsWith("DVD-") &&
       !dev->media.type.startsWith("DVD-RAM") &&
       (dev->cap & CAP_TEST_WRITE_DVD)) ||
      (dev->media.type.startsWith("DVD+") &&
       (dev->cap & CAP_TEST_WRITE_DVD_PLUS))) {
    noSimul = 0;
...
void TestDialog::start() {
  ...
  if (elabel->text().isEmpty()) {
    QMessageBox::information(this, tr("Media label is empty!"),
                             tr("You have to define a media label!"));
    return;
  }
  if (!dev->test_req) {
    QMessageBox::information(this, tr("No tests selected!"),
                             tr("You have selected no tests!"));
    return;
  }
#ifndef QT_NO_DEBUG
...
void TestDialog::saveData() {
  dev->test_req =
      (ck_RT->isChecked() ? TEST_RT : 0) | (ck_WT->isChecked() ? TEST_WT : 0) |
      (ck_ERRC->isChecked() ? TEST_ERRC : 0) |
      (ck_JB->isChecked() ? TEST_JB : 0) | (ck_FT->isChecked() ? TEST_FT : 0) |
      (ck_TA->isChecked() ? TEST_TA : 0);
  dev->WT_simul = noSimul ? 0 : ck_WT_simul->isChecked();

  dev->tspeeds.rt =
      (int)spd_RT->currentText().remove(QRegExp("[Xx]")).toFloat();
  dev->tspeeds.wt =
      (int)spd_WT->currentText().remove(QRegExp("[Xx]")).toFloat();
  dev->tspeeds.errc =
      (int)spd_ERRC->currentText().remove(QRegExp("[Xx]")).toFloat();
  dev->tspeeds.jb =
      (int)spd_JB->currentText().remove(QRegExp("[Xx]")).toFloat();
  dev->tspeeds.ft =
...
void TestDialog::pluginChanged(int idx) {
  ...
  */
  ...
  progress =
      new ProgressWidget(10, 3, isVisible() ? this : (QWidget *)parent());
