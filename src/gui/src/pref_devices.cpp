...
prefDevices::prefDevices(QPxSettings *iset, QWidget *p, Qt::WindowFlags f)
  ...
  for (int i = 0; i < set->hosts.size(); i++) {
    ...
    hitem->setCheckState(0, (set->hosts[i][0] == '*') ? Qt::Checked
                                                      : Qt::Unchecked);
...
prefDevices::~prefDevices() {
  ...
  for (int i = 0; i < hcnt; i++) {
    ...
    set->hosts.append(((hitem->checkState(0) == Qt::Checked) ? "*" : "-") +
                      hitem->text(0));
...
void prefDevices::hostsContextMenu(QContextMenuEvent *e) {
  ...
  act = cmenu->addAction(QIcon(":images/edit.png"), tr("Edit host"), this,
                         SLOT(hostEdit()));
...
void prefDevices::hostEdit() {
  ...
  if (!hitem)
    return;
  hostEditDialog *hedit =
      new hostEditDialog(hitem->text(0), hitem->text(1).toInt(), this);
