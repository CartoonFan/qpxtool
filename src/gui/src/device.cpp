...
void ErrcADD(Errc<int64_t> *tot, const Errc<int> &o) {
    for (int i = 0; i < 8; i++)
        if (o.raw.err[i] > 0)
            tot->raw.err[i] += o.raw.err[i];
};

void ErrcMAX(Errc<int> *max, const Errc<int> &o) {
    for (int i = 0; i < 8; i++)
        if (max->raw.err[i] < o.raw.err[i])
            max->raw.err[i] = o.raw.err[i];
};
...
#warning !!! mwidget->clearMedia() call
    ...
    // if (devices.idx()<0 || devices.idx()>= devices.size() ||
    // (threadType!=infoDevice && threadType!=infoMedia)) {
    if (type == DevtypeNone) {
    ...
    QObject::connect(io, SIGNAL(readyReadLine()), this,
                     SLOT(qscan_process_info()));
    ...
    if (type == DevtypeLocal) {
        ...
        case threadMedia:
            if (plugin.isEmpty()) {
                ...
                proc->start("qscan", QStringList() << "-d" << path << "--force-plugin"
                            << plugin << "-m");
            }
            break;
        case threadGetFeatures:
        ...
        case threadMQCK: {
            ...
            cdvdopts << "-d" << path;
            cdvdopts << "--mqck"
                     << ((features.as_act_mode & ASMQCK_ADV) ? "advanced" : "quick");
        ...
        break;
        case threadDestruct:
            proc->start("cdvdcontrol",
                        QStringList() << "-d" << path << "--destruct"
                        << (features.as_act_mode ? "full" : "quick"));
            break;
        case threadTattoo:
            proc->start("f1tattoo", QStringList() << "-d" << path << "--tattoo-raw"
                        << features.tattoo_file);
            break;
        default:
            goto update_info_err;
        }
        ...
        QObject::connect(proc, SIGNAL(finished(int, QProcess::ExitStatus)), this,
        ...
        QObject::connect(sock, SIGNAL(disconnected()), this,
                         SLOT(qscan_callback_info()));
#ifndef QT_NO_DEBUG
...
void device::qscan_callback_info() {
    ...
    QObject::disconnect(io, SIGNAL(readyReadLine()), this,
                        SLOT(qscan_process_info()));
    ...
    if (type == DevtypeLocal) {
        ...
        case FEATURE_VARIREC_CD:
            if (data->enabled & FEATURE_VARIREC_CD) {
                ...
                if (data->supported & FEATURE_VARIREC_CDEXT) {
                    cdvdopts << "--varirec-cd-strategy"
                             << QString::number(data->vrec_cd_str - 1);
                }
            } else {
                cdvdopts << "--varirec-cd"
                         << "off";
        ...
        case FEATURE_VARIREC_DVD:
            if (data->enabled & FEATURE_VARIREC_DVD) {
                ...
                cdvdopts << "--varirec-dvd-strategy"
                         << QString::number(data->vrec_dvd_str - 1);
            } else {
                cdvdopts << "--varirec-dvd"
                         << "off";
            }
            break;
...
#ifndef QT_NO_DEBUG
    qDebug() << "device::next_test(): starting test " << stest << " at speed "
             << test_spd;
#endif
    /*
        run_test("rt");
...
void device::qscan_process_line(QString &qout) {
    ...
    if (qout.startsWith("ID:")) {
        ...
        if (sl.size() >= 2) {
            ...
            if (sl[0].contains("Device", Qt::CaseInsensitive) &&
                    !sl[0].contains("capabilities", Qt::CaseInsensitive)) {
//				l_dev->setText(sl[1]);
#if 0
                ...
                rpc_rst = sl[1].toInt();
            } else if (sl[0].contains("Device Generic capabilities",
                                      Qt::CaseInsensitive)) {
                cap = sl[1].toULongLong(0, 16);
            } else if (sl[0].contains("Device Read capabilities",
                                      Qt::CaseInsensitive)) {
                cap_rd = sl[1].toULongLong(0, 16);
            } else if (sl[0].contains("Device Write capabilities",
                                      Qt::CaseInsensitive)) {
...
#endif
                /*
                                        } else if (sl[0].contains("Available quality
                   tests", Qt::CaseInsensitive)) { if (sl.size()>=2) { QStringList slt =
                   sl[1].split(' ', QString::SkipEmptyParts); test_cap = 0; for (int
                   ii=0; ii<slt.size(); ii++) { test_cap |= (slt[ii] == "errc") ?
                   TEST_ERRC : 0; test_cap |= (slt[ii] == "jb")   ? TEST_JB : 0;
                                                                test_cap |= (slt[ii] ==
                   "ft")   ? TEST_FT : 0; test_cap |= (slt[ii] == "ta")   ? TEST_TA : 0;
                                                        }
                                                }
                */
            }
...
void device::cdvdcontrol_process_line(QString &qout) {
    ...
    if (sl.size() >= 2) {
        ...
        if (sl[0].contains("Lock state", Qt::CaseInsensitive)) {
        ...
        } else if (sl[0].contains("PoweRec", Qt::CaseInsensitive) &&
                   !sl[0].contains("Speed", Qt::CaseInsensitive)) {
            if (sl[1] != "-") {
            ...
            features.prec_spd =
                sl[1].remove("X").remove("(CD)").remove("(DVD)").toInt();
            ...
            if (sl2.size() >= 3) {
                ...
                qDebug() << "Yamaha DiscT@2 radius:  inner " << features.tattoo_inner
                         << ", outer " << features.tattoo_outer;
            }
...
void device::qscan_callback_test() {
    ...
    time = (int)((timeEnd.tv_sec - timeSta.tv_sec) +
                 (timeEnd.tv_usec - timeSta.tv_usec) / 1000000.0);
    ...
    while (io->linesAvailable()) {
...
#endif
        gettimeofday(&timeEnd, NULL);
        time = (timeEnd.tv_sec - timeSta.tv_sec) +
               (timeEnd.tv_usec - timeSta.tv_usec) / 1000000.0;
        switch (ctest) {
        case TEST_RT:
        ...
        case TEST_WT:
            ...
            if (qout.startsWith("lba") && qout.contains("speed")) {
                ...
                //%3").arg(di.lba).arg(di.spdx).arg(di.spdk));
            } else if (qout.startsWith("Reading blocks") ||
                       qout.startsWith("Starting write")) {
                gettimeofday(&timeSta, NULL);
            }
            break;
            ...
            if (qout.startsWith("cur")) {
                ...
                if (sl.size() >= 8) {
                    ...
                    if (!testData.jb.size()) {
                        ...
                        if (testData.jbMM.jmin > di.jitter)
                            testData.jbMM.jmin = di.jitter;
                        if (testData.jbMM.jmax < di.jitter)
                            testData.jbMM.jmax = di.jitter;
                        if (testData.jbMM.bmin > di.asymm)
                            testData.jbMM.bmin = di.asymm;
                        if (testData.jbMM.bmax < di.asymm)
                            testData.jbMM.bmin = di.asymm;
                    }
        ...
        case TEST_FT:
            if (qout.startsWith("cur")) {
                ...
                if (sl.size() >= 8) {
                    ...
                    if (testData.ftMAX.fe < di.fe)
                        testData.ftMAX.fe = di.fe;
                    if (testData.ftMAX.te < di.te)
                        testData.ftMAX.te = di.te;
...
bool device::isSaving() {
    return resWriter->isRunning();
};
bool device::saveResult() {
    return resWriter->result();
};
...
bool device::isLoading() {
    return resReader->isRunning();
};
bool device::loadResult() {
    return resReader->result();
};
