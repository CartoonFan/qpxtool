...
#define DVD_ALIGN(ptr)                                                         \
  (void *)((((uintptr_t)(ptr)) + (DVD_VIDEO_LB_LEN - 1)) / DVD_VIDEO_LB_LEN *  \
           DVD_VIDEO_LB_LEN)
...
static int GetUDFCache(udf_t *udf, UDFCacheType type, uint32_t nr, void *data) {
  int n;
  struct udf_cache *c;
  /*
...
static int SetUDFCache(udf_t *udf, UDFCacheType type, uint32_t nr, void *data) {
  int n;
  struct udf_cache *c;
  /*
...
static int UDFFileIdentifier(uint8_t *data, uint8_t *FileCharacteristics,
  ...
  *FileCharacteristics = GETN1(18);
  ...
  L_IU = GETN2(36);
  if (L_FI)
    Unicodedecode(&data[38 + L_IU], L_FI, FileName);
  else
    FileName[0] = '\0';
...
static int UDFMapICB(udf_t *udf, struct AD ICB, uint8_t *FileType,
  ...
  } while ((lbnum <= partition->Start + ICB.Location +
                         (ICB.Length - 1) / DVD_VIDEO_LB_LEN) &&
           (TagID != 261));
...
static int UDFScanDir(udf_t *udf, struct AD Dir, char *FileName,
  ...
  while (p < Dir.Length) {
    ...
    if (TagID == 257) {
      ...
      if (cache_file_info && !in_cache) {
        ...
        UDFMapICB(udf, tmpICB, &tmpFiletype, partition, &tmpFile);
      } else {
        if (!strcasecmp(FileName, filename)) {
          *FileICB = tmpICB;
          return 1;
  ...
  */
  ...
  while (p < Dir.Length) {
    ...
    if (TagID == 257) {
      p += UDFFileIdentifier(&directory[p], &filechar, filename, FileICB);
      if (!strcasecmp(FileName, filename)) {
        // dvdalign_lbfree(device, directory);
        free(directory);
        return 1;
...
static int UDFGetAVDP(udf_t *udf, struct avdp_t *avdp) {
  uint8_t *Anchor;
 ...
 */
static int UDFFindPartition(udf_t *udf, int partnum, struct Partition *part) {
  uint8_t *LogBlock;
  uint32_t lbnum, MVDS_location, MVDS_length;
  uint16_t TagID;
  int i, volvalid;
  ...
  do {
    ...
    } while ((lbnum <= MVDS_location + (MVDS_length - 1) / DVD_VIDEO_LB_LEN) &&
             (TagID != 8) && ((!part->valid) || (!volvalid)));
...
uint32_t UDFFindFile(udf_t *udf, char *filename, uint32_t *filesize) {
  uint8_t *LogBlock;
  uint32_t lbnum;
  uint16_t TagID;
  struct Partition partition;
  ...
  if (!(GetUDFCache(udf, PartitionCache, 0, &partition) &&
    ...
    } while ((lbnum < partition.Start + partition.Length) && (TagID != 8) &&
             (TagID != 256));
  ...
  }
  {
    ...
    while (token != NULL) {
      ...
      if (!UDFScanDir(udf, File, token, &partition, &ICB, cache_file_info)) {
        return 0;
      }
 ...
 */
static int UDFGetDescriptor(udf_t *udf, int id, uint8_t *descriptor,
                            int bufsize) {
  uint32_t lbnum, MVDS_location, MVDS_length;
  struct avdp_t avdp;
  uint16_t TagID;
 ...
 */
int UDFGetVolumeIdentifier(udf_t *udf, char *volid, uint32_t volid_size) {
  struct pvd_t pvd;
  uint32_t volid_len;
