...
int child_create(int idx) {
  int r;
  if ((r = thread_create(&childs[idx].tid, NULL, &child_thread,
                         (void *)&childs[idx].arg))) {
#ifdef DAEMON_EN
  ...
  if (bind(listenfd, (struct sockaddr *)&srvaddr, srvaddr_len)) {
    ...
    if (errno != EADDRINUSE || !portauto) {
      printf("%d: Can'n bind() %s:%d: [%d] %s\n", pid,
             inet_ntoa(srvaddr.sin_addr), ntohs(srvaddr.sin_port), errno,
             strerror(errno));
      return -1;
    } else {
  ...
  for (; !term;) {
    ...
    if (sret > 0) {
      if (FD_ISSET(listenfd, &rd_set)) {
        // #ifndef _WIN32
        childs[cli_idx].arg.connfd =
            accept(listenfd, (struct sockaddr *)&childs[cli_idx].arg.cliaddr,
                   &cliaddr_len);
        /*
        #else
                                        int tconnfd = accept(listenfd,(struct
        sockaddr*) &childs[cli_idx].arg.cliaddr, &cliaddr_len); #endif
        */
