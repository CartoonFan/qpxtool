...
#warning "PlexEraser DETECTION. Just assume PX755/760 and Premium-II"
  ...
  if ((dev->ven_features & YMH_TATTOO) && (flags & FL_CURRENT)) {
    if (dev->yamaha.tattoo_rows) {
      printf("Yamaha DiscT@2      : inner %d   outer %d   image 3744 x %d\n",
             dev->yamaha.tattoo_i, dev->yamaha.tattoo_o,
  ...
  if ((dev->ven_features & PX_HCDRSS) &&
      ((flags & FL_CURRENT) || (flags & FL_HCDR)))
    printf("Hide-CDR state      : %s\n", dev->plextor.hcdr ? "ON" : "OFF");
  if ((dev->ven_features & PX_HCDRSS) &&
      ((flags & FL_CURRENT) || (flags & FL_SSS)))
    printf("SingleSession state : %s\n", dev->plextor.sss ? "ON" : "OFF");
  if ((dev->ven_features & PX_SPDREAD) &&
      ((flags & FL_CURRENT) || (flags & FL_SPDREAD)))
    printf("SpeedRead state     : %s\n", dev->plextor.spdread ? "ON" : "OFF");
  if ((dev->ven_features & PX_POWEREC) &&
      ((flags & FL_CURRENT) || (flags & FL_POWEREC))) {
  ...
  if ((dev->ven_features & PX_VARIREC_CD) &&
      ((flags & FL_CURRENT) || (flags & FL_VARIREC_CD))) {
    printf("VariRec CD state    : %s\n",
           dev->plextor.varirec_state_cd ? "ON" : "OFF");
    if (dev->plextor.varirec_state_cd)
      print_varirec(dev, VARIREC_CD);
  }
  if ((dev->ven_features & PX_VARIREC_DVD) &&
      ((flags & FL_CURRENT) || (flags & FL_VARIREC_DVD))) {
    printf("VariRec DVD state   : %s\n",
           dev->plextor.varirec_state_dvd ? "ON" : "OFF");
    if (dev->plextor.varirec_state_dvd)
      print_varirec(dev, VARIREC_DVD);
  }
  if ((dev->ven_features & PX_SECUREC) &&
      ((flags & FL_CURRENT) || (flags & FL_SECUREC)))
    printf("SecuRec state       : %s\n", dev->plextor.securec ? "ON" : "OFF");
  if ((dev->ven_features & PX_SILENT) &&
      ((flags & FL_CURRENT) || (flags & FL_SILENT))) {
    printf("Silent mode         : %s\n",
           dev->plextor_silent.state ? "ON" : "OFF");
    if (dev->plextor_silent.state)
      plextor_print_silentmode_state(dev);
  }
  if ((dev->ven_features & PX_BITSET_R) &&
      ((flags & FL_CURRENT) || (flags & FL_BOOK_R)))
    printf("DVD+R bitsetting    : %s\n", dev->book_plus_r ? "ON" : "OFF");
  if ((dev->ven_features & PX_BITSET_RDL) &&
      ((flags & FL_CURRENT) || (flags & FL_BOOK_RDL)))
    printf("DVD+R DL bitsetting : %s\n", dev->book_plus_rdl ? "ON" : "OFF");
  if ((dev->ven_features & PX_SIMUL_PLUS) &&
      ((flags & FL_CURRENT) || (flags & FL_TESTWRITE)))
    printf("DVD+R(W) testwrite  : %s\n",
           dev->plextor.testwrite_dvdplus ? "ON" : "OFF");
  if ((dev->ven_features & PX_ASTRATEGY) &&
      ((flags & FL_CURRENT) || (flags & FL_AS)))
    plextor_print_autostrategy_state(dev);
  if (dev->ven_features & PX_ERASER)
    printf("PlexEraser          : supported\n");

  if ((dev->ven_features & PIO_QUIET) &&
      ((flags & FL_CURRENT) || (flags & FL_PIOQUIET)))
    printf("QuietMode setting   : %s\n",
           pioneer_silent_tbl[(int)dev->pioneer.silent]);
  if ((dev->ven_features & PIO_QUIET) &&
      ((flags & FL_CURRENT) || (flags & FL_PIOLIMIT)))
...
void usage(char *bin) {
  ...
  printf(
      "\t--loej                       load/eject media (toggle tray state)\n");
  printf("\t--loej-immed                 don't wait for media is loaded or "
         "ejected\n");

  printf("\noptions for Plextor devices:\n");
  printf(
      "\t--spdread [on|off]           turn SpeedRead on/off (default: off)\n");
  printf("\t--sss [on|off]               turn SingleSession on/off (default: "
         "off)\n");
  printf(
  ...
  printf("\t                             values: 0.6, 0.7, 0.8, 0.9, 1.1, 1.2, "
         "1.3, 1.4, off\n");
  printf("\t--varirec-cd <pwr>           set VariREC power for CD or turn "
         "VariREC off\n");
  printf("\t                             values: +4, +3, +2, +1, 0, -1, -2, "
         "-3, -4, off\n");
  printf("\t--varirec-cd-strategy <str>  set VariREC strategy for CD\n");
  printf("\t                             values: default, azo, cya, pha, phb, "
         "phc, phd\n");
  printf("\t--varirec-dvd <pwr>          set VariREC power for DVD or turn "
         "VariREC off\n");
  printf("\t                             values: +4, +3, +2, +1, 0, -1, -2, "
         "-3, -4, off\n");
  printf("\t--varirec-dvd-strategy <str> set VariREC strategy for DVD\n");
  printf("\t                             values: default, 0, 1, 2, 3, 4, 5, 6, "
         "7\n");
  printf(
      "\t--securec <passwd>           set SecuREC password and turn it on\n");
  printf("\t                             passwd must be 4..10 characters "
         "length\n");
  printf("\t--nosecurec                  turn SecuRec off\n");
  printf("\t--bitset+r [on|off]          turn on/off setting DVD-ROM book on "
         "DVD+R media\n");
  printf("\t--bitset+rdl [on|off]        turn on/off setting DVD-ROM book on "
         "DVD+R DL media\n");
  printf("\t--mqck <quick|advanced>      run MediaQuality Check\n");
  printf("\t--mqck-speed #               set MQCK speed (default: maximum)\n");
  printf("\t--as-mode <mode>             AS: select AutoStrategy mode: "
         "forced,auto,on,off,\n");
  ...
  printf("\t--as-create <q|f> <a|r>      AS: create new strategy for inserted "
         "DVD media (PX-755/PX-760 only)\n");
  printf("\t                             mode:    q = quick, f = full\n");
  printf("\t                             action:  a = add,   r = replace\n");
  printf("\t--as-save <file>             AS: save Strategies            / "
         "EXPERIMENTAL /\n");
  printf("\t--as-load <file>             AS: load Strategy from file    / "
         "EXPERIMENTAL /\n");
  printf("\t--as-clear                   AS: remove ALL strategies from "
         "database\n");
  printf("\t--dvd+testwrite [on|off]     turn on/off testwrite on DVD+R(W) "
         "media\n");
  printf("\t--silent [on|off]            just turn Silent Mode on/off "
         "(default: on)\n");
  printf("\t--sm-nosave                  don't save Silent Mode settings\n");
  printf(
      "\t--sm-cd-rd #                 set max CD READ speed (default: 32X)\n");
  printf("\t                             values: 4, 8, 24, 32, 40, 48\n");
  printf(
      "\t--sm-cd-wr #                 set max CD WRITE speed (default: 32X)\n");
  ...
  printf("\t--sm-eject #                 set tray eject speed. spd can be 0 to "
         "80 (default: 0)\n");
  printf("\t--sm-access <slow|fast>      set access time, has effect only with "
         "CD/DVD speed setting\n");
  printf("\t--destruct <quick|full>      perform PlexEraser function\n");
  printf("\t                             WARNING! data on inserted CD-R/DVD-R "
         "will be lost!\n");
  printf(
      "\t                             works on PX-755/PX-760/Premium2 only\n");

  printf("\noptions for Pioneer devices:\n");
  printf("\t--pio-limit [on|off]         limit (or not) read speed by 24x for "
         "CD and 8x for DVD\n");
  printf("\t--pio-quiet [quiet|perf|std] select QuietMode setting: Quiet, "
         "Performance or Standard (default: quiet)\n");
  printf("\t--pio-nosave                 don't save settings to drive (changes "
...
int main(int argc, char *argv[]) {
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (!strcmp(argv[i], "-d")) {
      ...
      flags |= FL_CURRENT;
    else if (!strcmp(argv[i], "--current"))
      flags |= FL_CURRENT;
    else if (!strcmp(argv[i], "-s"))
      flags |= FL_SUPPORTED;
    else if (!strcmp(argv[i], "--supported"))
      flags |= FL_SUPPORTED;
    else if (!strcmp(argv[i], "-l"))
      flags |= FL_SCAN;
    else if (!strcmp(argv[i], "--scanbus"))
      flags |= FL_SCAN;
    //  ************   Lock
    ...
    //  ************   GigaREC
      ...
      if (argc > (i + 1)) {
        i++;
        flags |= FL_GIGAREC;
        if (!strcmp(argv[i], "0.6"))
          gigarec = GIGAREC_06;
        else if (!strcmp(argv[i], "0.7"))
          gigarec = GIGAREC_07;
        else if (!strcmp(argv[i], "0.8"))
          gigarec = GIGAREC_08;
        else if (!strcmp(argv[i], "0.9"))
          gigarec = GIGAREC_09;
    ...
    //  ************   VariREC  CD
      ...
      if (argc > (i + 1)) {
        ...
        if (!errno && val >= -1 && val < 6) {
          ...
          varirec_cd_str = 0;
        else if (!strcmp(argv[i], "azo"))
          varirec_cd_str = 1;
        else if (!strcmp(argv[i], "cya"))
          varirec_cd_str = 2;
        else if (!strcmp(argv[i], "pha"))
          varirec_cd_str = 3;
        else if (!strcmp(argv[i], "phb"))
          varirec_cd_str = 4;
        else if (!strcmp(argv[i], "phc"))
          varirec_cd_str = 5;
        else if (!strcmp(argv[i], "phd"))
          varirec_cd_str = 6;
        else {
    ...
    //  ************   VariREC  DVD
      ...
      if (argc > (i + 1)) {
        ...
        if (!strcmp(argv[i], "q"))
          ...
          printf(
              "invalid --as-create parameter 1: '%s'. Only 'q|f' are valid\n",
              argv[i]);
      }
      if (argc > (i + 1)) {
        i++;
        if (!strcmp(argv[i], "a"))
          ...
          printf(
              "invalid --as-create parameter 2: '%s'. Only 'a|r' are valid\n",
              argv[i]);
      }
